...
* **Cách hoạt động:**
  ...
  2. Khi bạn **dừng cuộn chuột trong 2 giây** ➔ Bảng Action Notes sẽ **tự động trồi lên lại vị trí ban đầu** (khi chế độ Auto đang BẬT).
  ...
  4. **Nút Phóng To / Thu Nhỏ Timeline (Expand / Collapse):** Trên thanh tiêu đề của bảng Action Notes bên dưới có nút **`Expand`** (icon `Maximize2`) — click vào để phóng to chiều cao drawer lên `480px` tại chỗ giúp hiển thị nhiều dòng nhật ký (activity notes) hơn cùng lúc mà không phải cuộn nhiều; click lại **`Collapse`** (icon `Minimize2`) để thu về chiều cao chuẩn `260px`. Khi bạn click nút **`Hide`**, drawer sẽ tự động đặt lại trạng thái thu gọn cho lần mở kế tiếp.
  5. **Nút Chuyển Đổi Auto / Manual (Auto-Show Toggle):** Trên thanh tiêu đề Action Timeline có nút **`Auto` / `Manual`** (icon `Pin` / `PinOff`). Mặc định là chế độ **`Auto`** (tự động hiện lại sau 1.8 giây khi dừng cuộn). Khi bấm chuyển sang **`Manual`**, khi bạn cuộn bảng trên thì Timeline vẫn tự ẩn gọn nhưng **sẽ KHÔNG tự động trồi lên lại**, giúp người dùng tập trung duyệt bảng danh sách không bị gián đoạn. Để mở lại Timeline trong chế độ Manual, chỉ cần bấm nút nổi **`▲ Action Timeline`** ở góc dưới bên phải. Trạng thái lựa chọn (Auto/Manual) được tự động lưu vào `localStorage` (`ats_action_menu_auto_slide_enabled`) và giữ nguyên qua các lần tải lại trang.
  _Cập nhật bởi: Antigravity (Implementer) — 2026-09-17_
* *[📷 Vị trí ảnh chụp 1.2: Trạng thái bảng dưới tự trượt xuống, nút nổi Quick Peek, nút Auto/Manual toggle và nút Expand/Collapse Timeline]*
...
### 3.6 Ẩn Mềm Đơn Tuyển Dụng (Archive Application) & Khôi Phục (Restore)
* **Cách hoạt động:**
  * **Archive (Ẩn đơn):** Khi muốn ẩn một hồ sơ ứng tuyển (ví dụ gán nhầm hoặc kết thúc mà không muốn xuất hiện trong các báo cáo pipeline/Action Menu), Recruiter nhấn nút **`Archive`** trên thẻ đơn. Xác nhận trên hộp thoại cảnh báo để ẩn đơn.
  * **Tự động loại trừ:** Đơn đã ẩn sẽ tự động biến mất khỏi Action Menu, giảm 1 số lượng ứng viên (`applicant_count`) trên trang Jobs & Clients Workbench.
  * **Show Archived & Restore (Khôi phục):** Bên dưới danh sách đơn active, bấm **`Show Archived (N)`** để xem lại các đơn đã ẩn. Nhấn nút **`Restore`** để đưa đơn trở lại pipeline hoạt động bình thường bất cứ lúc nào.
* _Cập nhật bởi: Antigravity (Implementer) — 2026-09-18_
* *[📷 Vị trí ảnh chụp 3.6: Nút Archive/Restore trên thẻ đơn và danh sách Show Archived]*

---

### 3.7 Đổi Vị Trí Tuyển Dụng (Edit Job Assignment)
* **Cách hoạt động:**
  * Khi cần sửa vị trí tuyển dụng do gán nhầm Job Order, Recruiter nhấn vào icon cây bút chì **`✏️`** ngay cạnh tên Job ở đầu thẻ đơn ứng tuyển.
  * Modal **Edit Job Assignment** mở ra hiển thị Job hiện tại và 2 ô chọn tìm kiếm:
    1. **Filter by Client Company (Optional):** Tìm kiếm và lọc nhanh công ty khách hàng.
    2. **Select New Target Position * (Required):** Tìm kiếm và chọn vị trí tuyển dụng mới.
  * **Bảo vệ chống trùng lặp:** Hệ thống tự động kiểm tra và chặn đổi sang Job mà ứng viên đã có hồ sơ trong pipeline.
  * Nhấn **`Update Job`** để cập nhật vị trí tuyển dụng mới. Số đếm ứng viên của cả Job cũ và Job mới được tự động tính toán lại.
* _Cập nhật bởi: Antigravity (Implementer) — 2026-09-18_
* *[📷 Vị trí ảnh chụp 3.7: Modal Edit Job Assignment với hộp tìm kiếm vị trí mới]*
...
### 7.1 Phân Loại & Quản Lý Danh Sách Chiến Dịch (Campaign Types, Filters & Master Table)
...
1. **Job Posting (Đăng bài tuyển dụng):**
   ...
   - Gắn với một hoặc nhiều **Job Orders** liên kết (`Linked Jobs`), hỗ trợ nội dung bài đăng, ảnh đính kèm, tính năng **AI Spin Content** (chống trùng lặp nội dung), và giới hạn bài đăng tối đa mỗi lượt chạy.
...
*Bộ Lọc Trạng Thái Thông Minh (Default Hide Completed):*
- Bộ lọc Status mặc định chọn **`Active (Hide Completed)`** nhằm tự động ẩn các chiến dịch đã kết thúc (`Completed`), giữ cho bảng danh sách luôn gọn gàng và tập trung vào các chiến dịch đang hoạt động.
- Người dùng có thể dễ dàng chuyển sang **`All Statuses`** hoặc từng trạng thái cụ thể (`Draft`, `Ready`, `Running`, `Paused`, `Completed`, `Needs Review`) để tra cứu.

*Cột "Linked Job" Tối Giản Theo Nhu Cầu (On-Demand Clean UX):*
- Khi một campaign liên kết nhiều job orders (≥3 jobs), bảng chỉ hiển thị mặc định tối đa 2 badges đầu tiên kèm nút bấm **`+N more`** để tránh vỡ chiều cao hàng.
- Bấm vào **`+N more`** để bung toàn bộ danh sách badge kèm thanh cuộn mượt nội bộ và nút **`See less`** để thu gọn lại. Nhấn vào nút xem thêm/thu gọn không làm kích hoạt chọn dòng / mở Campaign Details Panel.
- Nhấn trực tiếp vào từng badge Job vẫn điều hướng nhanh sang chi tiết Job tại `/jobs?job_id=...`.

*Cập Nhật Trạng Thái Chiến Dịch (Status Save & Validation):*
- Cho phép chỉnh sửa và lưu trực tiếp trạng thái chiến dịch (`Draft`, `Ready`, `Running`, `Paused`, `Completed`, `Failed`, `Archived`) thông qua **Edit Campaign Modal**, được xác thực an toàn qua server action.

_Cập nhật bởi: Antigravity (Implementer) — 2026-09-15_

* *[📷 Vị trí ảnh chụp 7.1: Modal tạo/chỉnh sửa Campaign với lựa chọn loại Job Posting vs Warming & Auto-Join, bộ lọc trạng thái và cột Linked Job thu gọn]*
...
4. **Nút Phóng To / Thu Nhỏ Detail Panel (Expand / Collapse):**
   - Trên thanh tiêu đề của Campaign Details Panel có nút **`Expand`** (icon `Maximize2`) — click vào để mở rộng chiều cao panel (khoảng ~54% viewport, responsive), đồng thời Master Table phía trên tự động co gọn lại (`max-h-36`) giúp hiển thị nhiều dòng lịch sử chạy (**Run History**) hoặc danh sách tài khoản & nhóm mục tiêu (**Overview & Groups**) mà không bị cắt đáy hay tràn cuộn kép.
   - Click nút **`Collapse`** (icon `Minimize2`) để thu về kích thước ban đầu.
   - Khi đóng panel bằng nút `✕` (Close), hệ thống tự động reset trạng thái thu gọn cho các lần chọn chiến dịch kế tiếp.

_Cập nhật bởi: Antigravity (Implementer) — 2026-09-15_

* *[📷 Vị trí ảnh chụp 7.3: Giao diện Detail Panel của Warming Campaign với tab Overview, nút Run Warm & Join, nút Expand/Collapse và bảng Run History mở rộng]*
