...
| **Phase 10: Unified Campaign Stop Conditions, Per-Nick Circuit Breaker & Configurable Batch Size (1..15)** | Thống nhất điều kiện dừng tự động cho Job Posting (`end_date` / pool coverage) & Warming (`end_date` / full coverage), Per-Nick Circuit Breaker (tự động ngắt nick khi ≥3 Failed run items), điều chỉnh dynamic batch size `max_posts_per_run` (1..15, mặc định 15) loại bỏ target_quota cũ, và sửa cron starvation cho Warming Campaign (`MAX(completed_at) ASC NULLS FIRST`). | ✅ Hoàn thành | 100% |
...
### v3.0-RC197 (15/09/2026)
* 🛑🔄 **Unified Campaign Stop Conditions, Per-Nick Circuit Breaker, Configurable Batch Size (1..15)**:
  * **Database Migration**: Bổ sung cột `max_posts_per_run integer NOT NULL DEFAULT 15 CHECK (max_posts_per_run > 0 AND max_posts_per_run <= 15)` trên cả 2 schema `public.campaigns` và `sandbox.campaigns`.
  * **Per-Nick Circuit Breaker**: Tự động nhận diện và loại trừ các tài khoản có ≥3 kết quả `Failed` trong campaign (`_getDisabledAccountIdsForJobCampaign` và `_getDisabledAccountIdsForWarmingCampaign`). Áp dụng nhất quán ở `_getEligibilityState` (cả preview lẫn execution) và Warming run lock resolution.
  * **Dynamic Pool Quota & Safe Dispatch Batching**: Target Quota được tính động = toàn bộ Target Pool (`COUNT DISTINCT campaign_social_groups.social_group_id`); dispatch limit cho mỗi lượt chạy = `Math.min(quotaRemaining, max_posts_per_run)` với trần cứng an toàn `SAFE_DISPATCH_BATCH_SIZE_MAX = 15` (chống quá tải/timeout VPS Bridge 55 phút).
  * **Unified Auto-Completion**:
    * Job Posting (`checkCampaignAutoCompletion`): Kiểm tra `end_date` trước (`completedReason: 'end_date'`); sau đó kiểm tra độ phủ pool `totalProcessed >= totalTargetPool` (`completedReason: 'quota'`).
    * Warming (`checkWarmingCampaignAutoCompletion`): Kiểm tra `end_date` trước; sau đó kiểm tra số nhóm chưa join còn lại của các nick active không bị circuit breaker (`remaining_work === 0` -> `completedReason: 'full_coverage'`). Gọi tự động trong `warm-join-run-callback/route.js`.
  * **Warming Cron Starvation Fix**: Trong `_acquireWarmJoinRunLock`, khi cron trigger (`campaignId = null`), sắp xếp chọn Warming campaign theo `(SELECT MAX(completed_at) FROM warm_join_runs wr WHERE wr.campaign_id = campaigns.id) ASC NULLS FIRST` và lọc `(end_date IS NULL OR end_date >= CURRENT_DATE)`.
  * **UI Updates**:
    * `CampaignEditModal.js`: Thay ô `targetQuota` bằng `maxPostsPerRun` (min 1, max 15, default 15) và cập nhật mô tả Auto-Scheduler.
    * `page.js`: Overview Panel hiển thị "Target Pool (toàn bộ nhóm mục tiêu)" với `totalGroupsInPool`.
    * `CampaignDispatchPreviewModal.js`: Đổi label stats bar thành "Dispatched This Run".
...
### Version 3.0-RC215 (2026-09-18) — Archive Application Record & Edit Job Assignment (Candidate 360°)
- **Vai trò triển khai:** Antigravity (Implementer). Spec & QA: Claude (Architect/QA).
- **Nội dung hoàn thành:**
  1. **DB Migration (`public.activity` & `sandbox.activity`):** Bổ sung cột `is_active boolean NOT NULL DEFAULT true` vào bảng `activity` trên cả 2 schema.
  2. **Server Actions (`src/app/actions.js`):**
     - Thêm `setApplicationActive(applicationId, isActive)` độc lập với guard khoá "Closed", cập nhật `is_active` và revalidate `/`, `/candidates`, `/jobs`.
     - Thêm `changeApplicationJob(applicationId, newJobId)` sửa lỗi gán nhầm Job, tích hợp logic chống trùng pipeline (`id != applicationId`) và kiểm tra trạng thái Job đích.
     - Cập nhật 4 query đọc (`getActionMenuData`, `getCandidateProfile`, `getClientWorkbenchData`, `getJobSearchData`) loại trừ `is_active = false` khỏi kết quả mặc định.
  3. **Candidate 360° UI (`src/app/candidates/page.js`):**
     - Thêm nút `Archive` / `Restore` trên mỗi application card.
     - Thêm nút `✏️ Pencil` và modal `EditJobModal` (kế thừa trọn vẹn Searchable Combobox Client/Job từ `AssignToJobModal`).
     - Tách danh sách Active và Archived, tích hợp khối toggle `Show Archived (N)`.
  4. **Kiểm Tra & Xác Minh:** `node --check` PASS, Next.js build Turbopack PASS 100% (31/31 routes), chạy test suite độc lập PASS 100% 3/3 kịch bản, dọn sạch dữ liệu test.

_Cập nhật bởi: Antigravity (Implementer) — 2026-09-18_

### Version 3.0-RC155 (2026-09-07) — Khắc Phục Lỗi Tường Lửa Vercel 403 Forbidden Khi Nạp CV & Bổ Sung Google OAuth Session Guard
- **Vai trò triển khai:** Antigravity (Implementer).
- **Nội dung hoàn thành:**
  1. **Khắc phục Lỗi Vercel Firewall 403 Forbidden Khi Nạp CV:** Vercel Firewall Rule 2 (`Path starts with /api/webhooks/` AND `IP != 127.0.0.1` -> `Deny 403`) tự động chặn request của người dùng khi nạp tệp qua route cũ `/api/webhooks/cv-upload-proxy`. Đã di dời toàn bộ route xử lý sang `/api/cv-upload-proxy` nằm ngoài phạm vi chặn IP của webhook, giúp trình duyệt người dùng kết nối thông suốt.
