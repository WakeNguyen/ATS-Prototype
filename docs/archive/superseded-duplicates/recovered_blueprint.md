...
> **Phiên bản:** v3.0-RC46 (Quản Lý Đa Chi Nhánh Multi-Branch JSONB & Liên Kết Gợi Ý Vị Trí Làm Việc)  
> **Hệ thống kết nối:** Next.js 15 (App Router / Server Actions), React 19, TailwindCSS v4, Supabase Singapore PostgreSQL (Transaction Pooler port 6543), Lucide Icons  
...
### 6.6 Quy Chuẩn Bắt Buộc Về Lọc Dữ Liệu & Xin Phép User (Filtering Strategy & Explicit Approval Protocol)
  1. 💎 **� iểm Mạnh (Pros & Strengths):** Lợi ích UX, tốc độ, tính năng, độ mở rộng.
  2. ⚠�  **� iểm Yếu & Thách Thức (Cons & Challenges):** Rủi ro ti� m ẩn, độ phức tạp bảo trì.
  3. 🎯 **Giá Trị � ạt � ược (Gains):** Tiết kiệm th� i gian, năng suất và độ chính xác cho Recruiter.
  4. ⚖�  **Cái Giá � ánh � ổi (Trade-offs):** Những yếu tố kỹ thuẙt hoặc nghiệp vụ cần cân nhắc đánh đổi.
  5. � � **Khuyến Nghị Của Architect:** � ưa ra phương án tối ưu nhất kèm lý do để ngư� i dùng tự tin ra quyết định.

### 6.6 Quy Chuẩn Bắt Buộc V�  L� c Dữ Liệu & Xin Phép User (Filtering Strategy & Explicit Approval Protocol)
* **Tôn chỉ tối thượng:** � �  cao Bảo mật (Security), Toàn vẹn dữ liệu (Data Integrity) và � ộ ổn định (Stability) lên hàng đầu.
* **Quy tắc bắt buộc:** M� i giải pháp liện quan đến logic tìm kiếm / l� c dữ liệu (giữa Backend Server-Side Query và Frontend In-Memory Filter) BẮT BUỘC phải được phân tích điểm mạnh - điểm yếu và **phải có sự đồng ý rõ ràng của Ngư� i dùng trước khi thực hiện**. Cấm tự ý tải dữ liệu lớn (dump records) v�  client để l� c trên bộ nhớ trình duyệt nếu chưa được phê duyệt.

---

## 7. Nhật K� Cập Nhật (Changelog)

### v3.0-RC75-QA (31/08/2026)
* 🛡️ **Kiểm thử Nghiệm thu & V� Lỗi Database (QA Verification & Integrity Fixes)**:
  * **Tạo API Route Kiểm Thử Tự Động:** X�y dựng endpoint độc lập (`src/app/api/qa-test/route.js`) để bắn 5 kịch bản �c liệt nhất (Destructive Testing, Race Condition, Deduplication).
  * **V� lỗi Connection Pooler (PgBouncer):** Bổ sung `prepare: false` v�o Postgres client (`db.js`) để tương th�ch ho�n to�n với chế độ Transaction Mode tr�n Port 6543 của Supabase, khắc phục triệt để lỗi `prepared statement does not exist`.
  * **Row-level Transaction Lock:** Th�m `pg_advisory_xact_lock` v�o c�c Server Actions (`actions.js`) để loại bỏ ho�n to�n đụng độ Race Condition khi sinh `display_number`.
  * **Khắc phục lỗi Not-Null Constraint:** V� lỗi thiếu trường `summary` trong qu� tr�nh INSERT v�o bảng `activity`.
  * **V� lỗi Zod Schema Validation:** Fix crash `Cannot read properties of undefined (reading 'map')` khi format lỗi trả về từ Zod.

### v3.0-RC46 (30/08/2026)
* � ��  **Quản Lý � a Chi Nhánh Multi-Branch JSONB & Liên Kết Gợi �  Vị Trí Làm Việc**:
