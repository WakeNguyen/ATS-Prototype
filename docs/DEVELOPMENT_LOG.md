...
| **`SNAP-20260918-190`** | 18/09/2026 01:25 | `v3.0-RC216` | 🛑🩺 **Circuit Breaker Chỉ Trip Khi Lỗi Phản Ánh Tài Khoản Có Vấn Đề (Session / Checkpoint) (FIX_SPEC_2026-09-18b)**: (1) **Helper (`src/app/campaign_actions.js`)**: Thêm hàm `isAccountHealthFailure(errorMessage)` nhận diện các lỗi session hết hạn/đăng nhập lại (`session expired`, `login required`, `fb-session.json not found`), loại bỏ các lỗi UI selector, network timeout, chưa join group; (2) **Webhooks (`campaign-run-progress/route.js` & `warm-join-run-progress/route.js`)**: Cập nhật điều kiện gọi circuit breaker và câu truy vấn đếm sự kiện chỉ tính `Checkpoint` và `Failed` do lỗi session; (3) **Verify**: `node --check` PASS 100% 3 files, Next.js Turbopack `npm run build` PASS 100% (31/31 routes), full HTTP E2E integration test suite sandbox PASS 100% 5/5 kịch bản (Kịch bản A: UI selector 5 fails không trip; Kịch bản B: Session expired 3 fails trip đúng lần 3; Kịch bản C: Checkpoint 3 lần trip đúng lần 3; Kịch bản D: Trộn 2 UI + 2 Checkpoints chưa trip, thêm 1 Checkpoint trip đúng lần 3; Kịch bản E: Warming trộn 1 UI + 3 session/checkpoint errors trip đúng lần 3), `/api/qa-test` (6/6 PASS 100%), `/api/biz-test` (28/28 PASS 100%). Dọn dẹp sạch toàn bộ dữ liệu test. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260918-189`** | 18/09/2026 00:35 | `v3.0-RC215` | 🗄️✏️ **Archive (Ẩn Mềm) Application Record & Edit Job (Sửa Job Gán Nhầm) Trên Candidate 360° (FIX_SPEC_2026-09-18)**: (1) **DB Migration (`public` & `sandbox`)**: Thêm cột `is_active boolean NOT NULL DEFAULT true` vào bảng `activity`; (2) **Server Actions (`src/app/actions.js`)**: Thêm `setApplicationActive(applicationId, isActive)` độc lập với guard khoá "Closed", thêm `changeApplicationJob(applicationId, newJobId)` tích hợp logic chống trùng pipeline (`id != applicationId`) và kiểm tra trạng thái Job đích; Sửa 4 query đọc (`getActionMenuData`, `getCandidateProfile`, `getClientWorkbenchData`, `getJobSearchData`) loại trừ `is_active = false` khỏi kết quả mặc định; (3) **UI Candidate 360° (`src/app/candidates/page.js`)**: Thêm nút `Archive` / `Restore`, nút `✏️ Pencil` và modal `EditJobModal` (Searchable Combobox Client/Job), refactor thẻ card dùng chung `renderApplicationCard`, tách danh sách Active / Archived và thêm toggle `Show Archived (N)`; (4) **Verify**: Chạy isolated test suite sandbox PASS 100% 3/3 kịch bản, `node --check` PASS 100%, Turbopack `npm run build` PASS 100% (31/31 routes). Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo + DB |
| **`SNAP-20260918-188`** | 18/09/2026 00:20 | `v3.0-RC214` | 🔧🗄️ **Backfill `link` Cho 58 Notification "Hoàn Tất Batch CV" Cũ (Data Fix, đi kèm SNAP-186)**: Thực hiện trực tiếp bởi Claude (Architect/QA) qua Supabase MCP (`public` schema, production), theo yêu cầu PO sau khi phát hiện notification cũ (tạo trước khi publish fix SNAP-186 lúc 17/09 16:53 UTC) vẫn còn `link = '/'`. Đã UPDATE đúng 58/60 notification `type = 'cv_batch_progress'` có `title LIKE 'Hoàn tất batch CV%'` và `link = '/'`, tính lại `link` theo đúng công thức đã dùng trong node `Finalize Batch` (SNAP-186): batch 1 file có đúng 1 item `done`/`failed` với `candidate_id` xác định → `/candidates?id=<candidate_id>`, còn lại → `/candidates`. 2 notification loại "Đang xử lý..." (progress, chưa hoàn tất) được loại trừ có chủ đích, giữ nguyên `link = '/'` theo đúng thiết kế. Verify: query xác nhận `remaining_stale_finalize = 0` sau khi chạy. KHÔNG đụng bất kỳ bảng/cột nào khác, không xoá dữ liệu. | ✅ Stable | Supabase (public) |
| **`SNAP-20260917-187`** | 18/09/2026 00:05 | `v3.0-RC214` | 📇⚡ **Sửa Lỗi Tạo Trùng Lặp Google Contact Khi Sync & Chống Double Click (FIX_SPEC_2026-09-17c)**: (1) **n8n Workflow (`O659fZyN2uGaaLyL`)**: Sửa node `Match Logic` (`n007`) — xoá bỏ early-return `if (!data.hasMatchableContacts)` và bổ sung fallback match qua `atsUrl` nhúng trong `biographies`, cho phép match chính xác 100% hồ sơ không có Email/Phone/LinkedIn và tự chữa lành các bản trùng cũ; (2) **`src/app/actions.js`**: Dòng 2272 sửa `atsUrl` chuẩn hoá định dạng `${baseUrl}/candidates?id=${c.id}`; (3) **UI (`src/app/candidates/page.js` & `src/app/search/page.js`)**: Thêm state `isSyncingGoogleContacts`, disable nút "Save to Google Contacts" và hiển thị "Syncing..." khi đang sync để chống race condition do bấm nhanh/double-click; (4) **Verify**: `node --check` PASS 100%, workflow n8n published & active (`e37fde22-9b66-433b-99c0-608e7b7cb71f`), test isolated sandbox candidate không có phone/email PASS 100% (Run 1: Create, Run 2: Update qua ATS URL biography match), dữ liệu test đã dọn dẹp sạch sẽ. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo + n8n |
| **`SNAP-20260917-186`** | 17/09/2026 23:55 | `v3.0-RC213` | 🔔🎯 **Deep Link Notification "Hoàn Tất Batch CV" Tới Hồ Sơ Candidate (n8n Workflow `fofSZKkdyhlVd9Lc`) (FIX_SPEC_2026-09-17b)**: (1) **n8n Workflow (`fofSZKkdyhlVd9Lc`)**: Cập nhật duy nhất node `Finalize Batch` (`finalize_batch_node`) — thêm logic tính `link`: nếu batch đúng 1 file (`total === 1 && processed.length === 1`) và item có `candidate_id` xác định thì `link = '/candidates?id=' + processed[0].candidate_id`, ngược lại fallback về `link = '/candidates'`; gán `link` động vào payload POST `/api/webhooks/notifications` thay vì hardcode `'/'`; (2) **Verify**: `validate_node_config` PASS 100%, `update_workflow` PASS 100%, `publish_workflow` active version `5d0bed8a-41bc-4fe6-90f3-23d294926f3c`, `get_workflow_versions_diff` xác nhận 100% chỉ có đúng `parameters.jsCode` của node `Finalize Batch` thay đổi, 0 node thêm/bớt, 0 connection thay đổi. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | n8n |
| **`SNAP-20260917-185`** | 17/09/2026 08:55 | `v3.0-RC212` | 🎛️⚡ **Toggle Auto-Show Cho Action Timeline & Chẩn Đoán Đồng Bộ Status Candidates ➔ Action Menu (FIX_SPEC_2026-09-17)**: (1) **Bug 2 (Feature Auto-Show Toggle Timeline)**: Thêm nút toggle `Auto` / `Manual` (`Pin` / `PinOff`) trên header Action Timeline (`src/app/page.js`), lưu preference `ats_action_menu_auto_slide_enabled` vào `localStorage`, gate logic tự-trồi-lên sau 1.8s trong `handleMasterWheel` (giữ nguyên auto-hide khi cuộn); (2) **Bug 1 (Chẩn đoán đồng bộ Status/Stage)**: Thực hiện đầy đủ quy trình chẩn đoán 3 kịch bản (a)/(b)/(c) trên Next.js 16.3.0 dev server thật với browser automation: Cả 3 kịch bản đều PASS 100% (Stage/Status cập nhật live ngay lập tức từ DB). Loại trừ bug cache client. Xác nhận cơ chế filter mặc định `statusFilter = "In progress"` tự động ẩn các application chuyển sang "Closed" trừ khi chọn filter Closed/All Status; (3) **Verify**: `node --check src/app/page.js` PASS, Next.js Turbopack `npm run build` PASS 100% (31/31 routes). | ✅ Stable | Repo |
| **`SNAP-20260915-184`** | 15/09/2026 17:55 | `v3.0-RC211` | 🍪🔄 **Lưu Cookie Xoay Vòng Trong `post-to-group.js` (Khối `finally`) + Tăng Thời Gian Chờ Reset IP 4G Lên 15s (FIX_SPEC_2026-09-15)**: (1) **`scripts/post-to-group.js`**: Thêm logic `context.storageState({ path: SESSION_PATH })` trong khối `finally` trước khi đóng browser — lưu lại cookie/token mới (xs/fr/sb) cho MỌI nhánh return (kể cả Failed do lỗi UI), chống gửi lặp cookie cũ hết hạn làm Facebook khóa session / Checkpoint; (2) **`scripts/run-batch.js`**: Tăng thời gian chờ sau khi gọi URL reset IP 4G từ 10s lên 15s (`setTimeout 15000`) để modem 4G ổn định hoàn toàn IP mới giữa các lượt chạy multi-account; (3) **Verify**: `node --check` PASS 100% 2/2 files, Next.js Turbopack `npm run build` PASS 100% (31/31 routes), `/api/qa-test` (6/6 PASS 100%), `/api/biz-test` (28/28 PASS 100%). Theo Implementation Spec từ Claude (Architect/QA) dựa trên RCA đã thẩm định độc lập. | ✅ Stable | Repo |
| **`SNAP-20260915-183`** | 15/09/2026 16:45 | `v3.0-RC210` | 🛑🗑️ **Xóa Hẳn Bộ Đếm Failed Cũ Thừa Của Circuit Breaker (Thay Thế Hướng Vá Mốc Thời Gian) (FIX_SPEC_2026-09-15)**: (1) **`src/app/campaign_actions.js`**: Xóa hoàn toàn 2 hàm helper `_getDisabledAccountIdsForJobCampaign` và `_getDisabledAccountIdsForWarmingCampaign`; Xóa lớp lọc thừa `disabledAccountIds` trong `_getEligibilityState` (Job Posting) và khối resolve `targetAccounts` (Warming); Xóa `disabledIds` trong `checkWarmingCampaignAutoCompletion`; (2) **Lý do & Cơ chế**: Khi account fail >= 3 lần, `_maybeTripCircuitBreaker` đã xóa dòng vật lý khỏi `campaign_fb_accounts` (nguồn xác định duy nhất). Việc giữ lại bộ đếm Failed cũ là thừa và gây ra lỗi double-jeopardy chặn account vừa được PO tick lại. Sau khi xóa lớp lọc thừa, PO tick lại account vào campaign sẽ có hiệu lực ngay lập tức; (3) **Verify**: `node --check` PASS 100%, `grep` xác nhận không còn bất kỳ tham chiếu nào tới 2 hàm cũ, full sandbox E2E test suite PASS 100% (Job Posting & Warming: Failed 3 lần -> Tripped & xóa gán -> PO tick lại -> Xuất hiện ngay trong Dispatch Preview / Warming Lock -> Failed tiếp 3 lần -> Tripped lần 2 thành công), `/api/qa-test` (6/6 PASS 100%), `/api/biz-test` (28/28 PASS 100%), Next.js Turbopack `npm run build` PASS 100% (31/31 routes). Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260915-182`** | 15/09/2026 16:20 | `v3.0-RC209` | 🛑⚡ **Circuit Breaker Tự Gỡ Account Khỏi Campaign (Thật) + Chặn Fallback Khi Đã Bị Gỡ (FIX_SPEC_2026-09-15)**: (1) **`src/app/campaign_actions.js`**: Thêm `_maybeTripCircuitBreaker(sqlTx, campaignId, fbAccountId, failedCount, triggerType)` xóa dòng `campaign_fb_accounts` và ghi log `campaign_account_circuit_breaker_log` khi `failedCount >= 3`; Trong `_getEligibilityState` (Job Posting) và warming targetAccounts resolve, kiểm tra bảng log để CHẶN fallback sang "tất cả account Active" khi campaign đã từng bị trip breaker; `getCampaignDetail` nạp `circuitBreakerLog`; `computeCampaignDispatchPreview` trả `circuitBreakerRemovedAccounts` vào `stats`; (2) **Webhooks (`campaign-run-progress/route.js` & `warm-join-run-progress/route.js`)**: Kiểm tra khi có status/action `Failed`, đếm tổng Failed của account trong campaign và gọi `_maybeTripCircuitBreaker` trong cùng transaction; (3) **UI (`CampaignEditModal.js` & `CampaignDispatchPreviewModal.js`)**: Edit Modal hiển thị cảnh báo amber khi có account bị breaker tự gỡ ("Auto-removed due to repeated failures: {name} — {N} failed items..."); Dispatch Preview Modal hiển thị cảnh báo amber kèm tên account khi không còn target group hợp lệ do bị breaker; (4) **Verify**: `node --check` PASS 100% 5/5 files, Turbopack `npm run build` PASS 100% (31/31 routes), `/api/qa-test` (6/6 PASS 100%), `/api/biz-test` (28/28 PASS 100%), full HTTP E2E sandbox verification script (Job Posting & Warming x3 fails -> DB deleted assignment, breaker log created, fallback blocked, multi-account isolation verified). Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260915-181`** | 15/09/2026 15:25 | `v3.0-RC208` | 🎯⚡ **Bảng Run History Tự Cập Nhật Live + Fix Giật Do Polling Toàn Cục & Fix Phantom FB Account Selection (`/campaigns`) (FIX_SPEC_2026-09-15)**: (1) **Hạng mục 1 (`src/app/campaigns/page.js` & `src/app/components/RunHistoryTable.js`)**: Di chuyển `setTargetGroupIds(tIds)` vào trong nhánh `if (!isSilent)` và bỏ silent refetch `fetchTargetGroups` trong `loadCampaignDetailData`; thu hẹp polling `useEffect` chỉ load detail khi `viewedCampaign?.latest_run_status === 'Running'`; thêm auto-poll 10s trong `RunHistoryTable` cho expanded run đang `Running`; (2) **Hạng mục 2 (`src/app/components/CampaignEditModal.js`)**: Cập nhật `setAvailableAccounts` nạp các assigned accounts kể cả khi không còn `Active` để hiển thị checkbox đầy đủ, loại bỏ hiện tượng đếm phantom selection; (3) **Verify**: `node --check` 3/3 files PASS, `npm run build` PASS 100% 31/31 routes, `/api/qa-test` (6/6 PASS 100%), `/api/biz-test` (28/28 PASS 100%). Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260915-180`** | 15/09/2026 14:50 | `v3.0-RC207` | 🎯🔄 **Redesign Campaign Status: 4 Giá Trị (3 User-Chọn + 1 Hệ Thống) & Tách Rời Trạng Thái Chạy (FIX_SPEC_2026-09-15)**: (1) **DB Migration (`sandbox.campaigns`)**: Cập nhật constraint `campaigns_status_check` thành `status IN ('Draft', 'Active', 'Archived', 'Needs Review')`; chuẩn hóa dữ liệu cũ (`Ready`/`Running`/`Failed` -> `Active`, `Paused` -> `Draft`, `Completed` -> `Archived`), đồng bộ `is_active = (status = 'Active')`, reset `auto_run_enabled = false` cho Archived; (2) **`src/app/campaign_actions.js`**: `createCampaign` default `status = 'Draft'`, server-derive `is_active`, validate whitelist `['Draft','Active','Archived']`; `updateCampaign` chặn client gửi `Needs Review` hoặc invalid status, derive `is_active` qua CASE WHEN; `getCampaigns` hỗ trợ sentinel `HIDE_ARCHIVED` (`status <> 'Archived'`); gỡ bỏ `status = 'Running'` khỏi UPDATE trong `_executeCampaignRunInternal` và `triggerWarmJoinRun`; `checkCampaignAutoCompletion` và `checkWarmingCampaignAutoCompletion` set `status = 'Archived'`; (3) **`src/app/components/CampaignEditModal.js`**: `CAMPAIGN_STATUS_OPTIONS` rút gọn còn 3 giá trị (`Draft`, `Active`, `Archived`); render option disabled `Needs Review` khi status là Needs Review; loại bỏ field `is_active` khỏi form submit payload; cảnh báo "Note: the run currently in progress will continue to completion — pausing only prevents future runs." khi chuyển away từ Active trong khi run đang Running; (4) **`src/app/campaigns/page.js`**: Default filter `HIDE_ARCHIVED`, dropdown filter có option `Needs Review`; `hasRunningCampaign` và `isRunning` kiểm tra `latest_run_status === 'Running'`; `renderCampaignStatusBadge` nhận full campaign object ưu tiên `latest_run_status === 'Running'` > `Needs Review` > `Active`/`Archived`/`Draft`; render warning toast khi pause campaign đang có run chạy; (5) **Webhooks**: `campaign-run-callback` set `status = 'Active'` khi xong, `status = 'Needs Review'` + `is_active = false` + `auto_run_enabled = false` khi systemic failure; `warm-join-run-callback` set `status = 'Active'`; `auto-run-eligible-campaigns` bỏ lọc redundant; (6) **Verify**: `node --check` PASS 100% 6 files, Turbopack `npm run build` PASS 100% 31/31 routes, `/api/qa-test` (6/6 PASS 100%), `/api/biz-test` (28/28 PASS 100%), sandbox test suite 7/7 scenarios PASS 100%. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo + DB |
| **`SNAP-20260915-179`** | 15/09/2026 13:55 | `v3.0-RC206` | 🎯✨ **Thêm "Select All / Deselect All" Cho Target Social Groups Trong Campaign Edit Modal (`CampaignEditModal.js`) (FIX_SPEC_2026-09-15)**: (1) **`src/app/components/CampaignEditModal.js`**: Import `getSocialGroupIdsMatchingFilter` từ `../campaign_actions`; Thêm 2 handlers `handleSelectAllFilteredGroups` và `handleDeselectAllFilteredGroups` gọi server action `getSocialGroupIdsMatchingFilter({ search: groupSearch })` để union hoặc subtract toàn bộ IDs khớp filter vào `selectedGroupIds`; Thêm 2 nút "Select All ({groupTotalCount})" và "Deselect All" cạnh ô search groups; Thu nhỏ ô search từ `w-48` thành `w-40` để vừa hàng; (2) **Verify**: `node --check` PASS 100%, `npm run build` PASS 100% (31/31 routes). Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260915-178`** | 15/09/2026 13:40 | `v3.0-RC205` | 📇🌐 **Bảo Toàn Query Param `?id=` Cho Facebook `profile.php` Trong `normalizeContactValue` (`src/lib/utils.js`) (FIX_SPEC_2026-09-15)**: (1) **`src/lib/utils.js`**: Thêm nhánh riêng xử lý Facebook `profile.php` trước logic URL chung, trích xuất `idMatch = val.match(/id=(\d+)/)` và trả về `${pathOnly}?id=${idMatch[1]}` (thay vì cắt bỏ toàn bộ query string làm mất định danh profile); (2) **Verify**: `node --check` PASS 100%, standalone unit tests (Facebook profile.php with/without noise `%0d`, regular vanity Facebook profile, LinkedIn tracking params, Email, Phone) PASS 100%, `git diff --stat` chỉ `src/lib/utils.js` và `docs/DEVELOPMENT_LOG.md`, `/api/biz-test` (28/28 PASS 100%), Next.js Turbopack `npm run build` PASS 100% (31/31 routes). Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260915-177`** | 15/09/2026 11:10 | `v3.0-RC204` | 📇✨ **Chuẩn Hóa Giá Trị Contact Point Trước Khi Insert Trong 2 Luồng HITL (`hitl_actions.js`) (FIX_SPEC_2026-09-15)**: (1) **`src/app/hitl_actions.js`**: `createCandidateFromPayload` lọc `cp && cp.value` và chuẩn hóa giá trị `const normVal = normalizeContactValue(cp.type, cp.value)` trước khi INSERT; Luồng `MERGE` tính `const normVal = normalizeContactValue(cp.type, cp.value)` và lưu `value: normVal || cp.value` thay vì raw `cp.value`; Giữ nguyên logic fallback giá trị gốc khi không chuẩn hóa được; (2) **Verify**: `node --check` PASS 100%, sandbox test suite PASS 3/3 kịch bản (FORCE_CREATE Phone/LinkedIn URL, MERGE Phone/LinkedIn URL/Email dedup, Invalid phone fallback), `/api/biz-test` (28/28 PASS 100%), `/api/qa-test` (6/6 PASS 100%), Next.js Turbopack `npm run build` PASS 100% (31/31 routes). Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260915-176`** | 15/09/2026 10:55 | `v3.0-RC203` | 📇🛡️ **Chặn Trùng Contact Point Trong Cùng 1 Candidate Hồ Sơ (Candidate 360°) (FIX_SPEC_2026-09-15)**: (1) **Server Actions (`src/app/actions.js`)**: Sửa `addContactPoint` và `updateContactPoint` gộp query dedup trên bảng `contact_points`, bỏ điều kiện `cp.candidate_id != ${candidateId}`, SELECT thêm `cp.candidate_id` và phân biệt rõ 2 thông báo lỗi: cùng candidate báo `"This contact already exists on this candidate's profile."`, khác candidate báo `"Duplicate Contact Detected: This contact is already associated with another candidate."`; (2) **Test Suite (`src/app/api/biz-test/route.js`)**: Bổ sung test case `BIZ-26` tự động hoá 5 bước kiểm tra cách ly sandbox (Add new contact, Add duplicate same-candidate, Add duplicate cross-candidate, Update duplicate same-candidate, Update duplicate cross-candidate); (3) **Verify**: `node --check` PASS 100%, `/api/biz-test` PASS 28/28 tests 100% (gồm `BIZ-26`), Next.js Turbopack `npm run build` PASS 100% (31/31 routes). Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260915-175`** | 15/09/2026 10:15 | `v3.0-RC202` | 🎯🛠️ **Sửa Lỗi Lưu Campaign Status, Default Filter Ẩn Completed, và On-Demand Collapse Cột Linked Job (`/campaigns`) (FIX_SPEC_2026-09-15)**: (1) **Hạng mục 1 (Bug Fix Status Save)**: `src/app/campaign_actions.js` destructure `status` trong `updateCampaign`, whitelist 7 trạng thái (`Draft, Ready, Running, Paused, Completed, Failed, Archived`), cập nhật `status` qua CASE WHEN trong UPDATE SQL và gán fallback null cho `max_posts_per_run` chống lỗi `UNDEFINED_VALUE`; (2) **Hạng mục 2 (Feature Default Hide Completed Filter)**: `campaign_actions.js` (`getCampaigns`) xử lý `status === 'HIDE_COMPLETED'` (`c.status <> 'Completed'`), `src/app/campaigns/page.js` khởi tạo `campaignStatusFilter = 'HIDE_COMPLETED'` và thêm option `Active (Hide Completed)`; (3) **Hạng mục 3 (UI On-Demand Clean UX Linked Job)**: `src/app/campaigns/page.js` thêm state `expandedJobsRowIds`, hằng số `LINKED_JOB_BADGES_VISIBLE_LIMIT = 2`, nút `+N more` / `See less` với `e.stopPropagation()`; (4) **Tài liệu**: Cập nhật `docs/USER_MANUAL_DRAFT.md` mục 7.1; (5) **Verify**: `node --check` PASS 100%, sandbox test suite PASS 7/7 kịch bản (Status Save, Bogus reject, HIDE_COMPLETED/ALL/Completed filtering), `/api/qa-test` (6/6 PASS 100%), `/api/biz-test` (27/27 PASS 100%), Next.js `npm run build` PASS 100% 31/31 routes. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260915-174`** | 15/09/2026 10:00 | `v3.0-RC201` | 🎛️📐 **Thêm Nút "Expand/Collapse" Cho Campaign Details Panel (`/campaigns`) (FIX_SPEC_2026-09-15)**: (1) **`src/app/campaigns/page.js`**: Import `Maximize2, Minimize2` từ `lucide-react`, thêm state `isDetailPanelExpanded`, áp dụng chiều cao responsive `h-[clamp(380px,54vh,760px)]` cho Campaign Details panel khi Expand (thu về `flex-1 min-h-[360px]` khi Collapse), tự động co Master Table về `shrink-0 max-h-36` khi Expand để chống clipping ở màn hình thấp 1366x768 (768px viewport); Thêm nút toggle Expand/Collapse trước nút Close 'X' và tự động reset `isDetailPanelExpanded=false` khi đóng panel; (2) **`docs/USER_MANUAL_DRAFT.md`**: Cập nhật mục 7.3 bổ sung hướng dẫn sử dụng nút Expand/Collapse; (3) **Verify**: `node --check` PASS 100%, Chrome DevTools MCP browser test (1440x900 & 1366x768) PASS 100% không clipping (mainScrollHeight = mainClientHeight = 653px), regression Action Menu (`/`) PASS 100%, Next.js Turbopack `npm run build` PASS 100% (31/31 routes). Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260915-173`** | 15/09/2026 01:30 | `v3.0-RC200` | 🛡️🔥 **Auto-clamp Warming "Max Joins Per Run" Theo Số Account Thật (Chống Lặp Lại Sự Cố Timeout 55 Phút VPS) (Spec 2026-09-15)**: (1) **`src/app/campaign_actions.js`**: Thêm `SAFE_TOTAL_WARM_ACTIONS = 20` (~2.5 phút/action, quỹ 50 phút trước ngưỡng cứng 55 phút `BRIDGE_TIMEOUT_MS`). Trong `_acquireWarmJoinRunLock`, tự động clamp `maxPostsPerRun = Math.max(1, Math.min(requestedMax, Math.floor(SAFE_TOTAL_WARM_ACTIONS / safeAccountCount)))` dựa trên `targetAccounts.length` thực tế sau khi lọc busy/disabled, gắn `wasClampedForSafety: true` và `requestedMaxPostsPerRun` khi có clamp; Trong `triggerWarmJoinRun`, áp cùng công thức clamp cho manual override `maxGroupsPerAccount` dựa trên `lockRes.accounts.length` và log cảnh báo khi bị clamp; (2) **`src/app/campaigns/page.js`**: Thêm cảnh báo phụ màu amber trong Modal "Confirm Warm & Join" khi `activeCount * effectiveVal > 20` cảnh báo người dùng hệ thống sẽ tự động giảm số lượng để tránh vượt timeout hạ tầng; (3> **Verify**: `node --check` PASS 100%, Next.js Turbopack `npm run build` PASS 100% 31/31 routes, bổ sung test case `WARM-CLAMP` vào `/api/qa-test` (test tự động 5 account bị clamp xuống 4, 1 account giữ nguyên 15 -> PASS 100%), `/api/qa-test` (6/6 PASS 100%), `/api/biz-test` (27/27 PASS 100%). Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260915-172`** | 15/09/2026 01:25 | `v3.0-RC199` | ⏰🎯 **Enforce start_time/end_time Daily Window Cho Job Posting Auto-Scheduler (`auto-run-eligible-campaigns`) (Spec 2026-09-15)**: (1) **`src/app/api/webhooks/auto-run-eligible-campaigns/route.js`**: Bổ sung điều kiện lọc theo giờ VN `AND (now() AT TIME ZONE 'Asia/Ho_Chi_Minh')::time BETWEEN c.start_time AND c.end_time` vào WHERE clause của câu SELECT `eligibleCampaigns`, đặt ngay sau `end_date` và trước `AND NOT EXISTS`, đảm bảo đúng múi giờ Asia/Ho_Chi_Minh bất kể timezone máy chủ; (2) **Giữ nguyên**: Không can thiệp `_executeCampaignRunInternal`, `computeCampaignDispatchPreview`, hay `triggerCampaignRun` — đảm bảo đường "Run" thủ công từ UI luôn hoạt động bất kể giờ nào; (3) **Verify**: `node --check` PASS, Next.js Turbopack `npm run build` PASS 100% 31/31 routes, verify trực tiếp qua HTTP endpoint `/api/webhooks/auto-run-eligible-campaigns` với test campaign sandbox (loại trừ chính xác ngoài khung 08:00-20:00 lúc 01:22 VN, nhận diện chính xác khi nằm trong khung giờ), `/api/qa-test` (5/5 PASS), `/api/biz-test` (27/27 PASS). Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260915-171`** | 15/09/2026 01:35 | `v3.0-RC198` | 🔄🔥 **Unify "Max Posts Per Run" Cho Warming Campaign & Khôi Phục Per-Run Override (Round 1 & Round 2 Spec 2026-09-15)**: (1) **`src/app/campaign_actions.js`**: `_acquireWarmJoinRunLock` chọn thêm `max_posts_per_run` trong cả 2 nhánh (campaignId cụ thể & cron auto-pick) và trả về `maxPostsPerRun: campaign?.max_posts_per_run || 2`; `triggerWarmJoinRun` khôi phục nhận `maxGroupsPerAccount` từ params (không đặt default cứng = 2) và ưu tiên override khi gọi manual từ UI: `maxGroupsPerAccount != null ? Number(maxGroupsPerAccount) : (Number(lockRes.maxPostsPerRun) || 2)`; (2) **`src/app/api/webhooks/warm-join-cron-register/route.js`**: Bổ sung `maxGroupsPerAccount: lockRes.maxPostsPerRun` vào response JSON; (3) **`src/app/components/CampaignEditModal.js`**: Thống nhất lưu `max_posts_per_run` (1..15) cho cả Job Posting & Warming, hiển thị input `Max Posts / Joins Per Run` với label & helper description động theo loại campaign; (4) **Verify**: `node --check` PASS, `npm run build` PASS 31/31 routes, UI test trên Chrome DevTools MCP (edit campaign Warming -> lưu max_posts_per_run = 7 -> DB ghi nhận 7 -> mở lại load đúng 7 -> trigger run với picker override 4 group(s) -> Server log xác nhận dispatch payload đúng `maxGroupsPerAccount: 4`), `/api/qa-test` (5/5 PASS), `/api/biz-test` (27/27 PASS). Theo Implementation Spec & Round 2 Fix Instruction từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260915-170`** | 15/09/2026 01:10 | `v3.0-RC197` | 🛑🔄 **Unified Campaign Stop Conditions, Per-Nick Circuit Breaker, Configurable Batch Size (1..15) (Spec 2026-09-15)**: (1) **DB Migration**: Thêm `max_posts_per_run integer NOT NULL DEFAULT 15 CHECK (max_posts_per_run > 0 AND max_posts_per_run <= 15)` trên cả `public.campaigns` và `sandbox.campaigns`; (2) **Backend & Circuit Breaker (`campaign_actions.js`)**: Triển khai per-nick circuit breaker (tự động loại trừ nick khi ≥3 `Failed` run items trong campaign ở cả Job Posting và Warming), dynamic pool quota (Target Quota = toàn bộ target pool), safe dispatch batching `Math.min(quotaRemaining, max_posts_per_run)` với `SAFE_DISPATCH_BATCH_SIZE_MAX = 15`, thống nhất điều kiện dừng tự động cho Job Posting (`end_date` / pool coverage) và Warming (`end_date` / full coverage), sửa cron starvation cho Warming Campaign (`MAX(completed_at) ASC NULLS FIRST`); (3) **UI (`CampaignEditModal.js`, `page.js`, `CampaignDispatchPreviewModal.js`)**: Thay targetQuota bằng `maxPostsPerRun` (1..15), Overview Panel hiển thị "Target Pool (toàn bộ nhóm mục tiêu)", Preview Modal stats bar đổi thành "Dispatched This Run"; (4) **Verify**: `node --check` PASS, `npm run build` PASS 31/31 routes, isolated sandbox test suite 6/6 PASS, `/api/qa-test` (5/5 PASS), `/api/biz-test` (28/28 PASS). | ✅ Stable | Repo + DB |
| **`SNAP-20260915-169`** | 15/09/2026 00:10 | `v3.0-RC196` | ⚡🚀 **Dùng `after()` (next/server) Cho Webhook Dispatch Trong `_executeCampaignRunInternal` & `triggerWarmJoinRun` (FIX_SPEC_2026-09-15)**: Khắc phục sự cố campaign dispatch webhook n8n bị rớt ngẫu nhiên trên Vercel Serverless Function (production incident "WINPRO - 3 JOBS" kẹt `Running` do container đóng băng trước khi unawaited `fetch()` kịp hoàn tất). (1) **`src/app/campaign_actions.js`**: Import `after` từ `next/server`, bọc toàn bộ khối `fetch()` gọi n8n webhook ở cả 2 luồng (Job Posting dispatch trong `_executeCampaignRunInternal` và Warming dispatch trong `triggerWarmJoinRun`) vào `after(async () => { ... })`, trích xuất `process.env.N8N_*` và `INTERNAL_WEBHOOK_SECRET` ra biến cục bộ trước callback, thêm `signal: AbortSignal.timeout(8000)`, giữ nguyên `try...catch` an toàn; (2) **Verify**: `node --check` PASS 100%, Next.js `npm run build` Turbopack PASS 100% 31/31 routes, trigger thật trong sandbox UI dispatch campaign Job Posting chuyển trạng thái "Running" và lưu DB thành công, `/api/biz-test` (27 tests) + `/api/qa-test` (5 tests) PASS 100%. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260914-168`** | 14/09/2026 23:45 | `v3.0-RC195` | 🔍📱 **Search Column Responsive Fix & Campaigns Detail Panel Tablet Audit (Spec 2026-09-14 Batch 2)**: (1) **Search Social Column Fix**: Xoá `max-w-[260px]` cứng khỏi container badge social ở `src/app/search/page.js` (dòng 623), cho phép div tự mở rộng theo độ rộng thực tế của `<td>` (`min-w-[260px]`), loại bỏ hiện tượng wrap dòng sai khi có ≥3 profiles; (2) **Campaigns Tablet QA Audit**: Thực hiện QA audit tablet có mục tiêu trên 4 mốc độ rộng (768px, 834px, 1024px, 1440px) qua Chrome DevTools MCP: `scrollWidth === clientWidth` 100%, 0 console error, popover Group Type Tag Editor & Join Status Accounts Breakdown tự động lật `data-side` và đóng mở chính xác; (3) **Verify**: `npm run build` PASS 31/31 routes (17.2s). Theo Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260914-167`** | 14/09/2026 23:15 | `v3.0-RC194` | 🛠️ **Batch Fix Campaign, Search, Location & Popover (FIX_SPEC_2026-09-14 - Round 1 & Round 2 QA)**: (1) **Blacklist Sync**: Auto-clear `blacklist_note = ''` khi uncheck Blacklisted trong Server Action & UI; tách `isBlocked` ở `/search` chỉ phụ thuộc `c.blocked`; (2) **Job Branch Location Link**: Bổ sung `branch_id text` trên bảng `jobs` (sandbox & public), dynamic display resolver `getJobLocationDisplay` ở `/jobs`, atomic update `location` + `branch_id`; (3) **Portal Popover Migration**: Chuyển inline popover trong `JoinStatusBadge.js` & `GroupTypeTagEditor.js` sang `@base-ui/react/popover` (`src/components/ui/popover.jsx`) chống tràn/cắt bởi `overflow-hidden`, gỡ bỏ hoàn toàn manual self-toggle tránh double-toggle; (4) **Unified Contact Aggregation**: Refactor `syncCandidateAggregatedContacts` dùng chung cho `addContactPoint`, `updateContactPoint` và `hitl_actions.js` MERGE block; (5) **Verify**: Bổ sung BIZ-24, BIZ-25, toàn bộ 28 tests (BIZ-01..25, PERF-01..03) PASS 100%, Next.js build PASS 31/31 routes. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo + DB |
| **`SNAP-20260913-166`** | 13/09/2026 23:25 | `v3.0-RC193` | 📇⚡ **Dùng `after()` (next/server) Cho Webhook Dispatch Trong `syncCandidatesToGoogleContacts` (FIX_SPEC_2026-09-13)**: Khắc phục lỗi 80% lần dispatch "Save to Google Contacts" bị rớt ngẫu nhiên trên Vercel Serverless Function do container đóng băng/ngắt ngay sau khi hàm `return` trước khi `fetch()` không `await` kịp hoàn tất. (1) **Server Actions (`src/app/actions.js`)**: Import `after` từ `next/server`, bọc dispatch webhook n8n vào `after(async () => { ... })` (`waitUntil` trên Vercel), đọc `process.env.*` ra biến cục bộ trước callback, thêm `signal: AbortSignal.timeout(8000)`, giữ `try...catch` an toàn không chặn response trả về User; (2) **Verify**: `node --check` PASS 100%, Turbopack `npm run build` PASS 100% 31/31 routes. Theo FIX_SPEC từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260913-165`** | 13/09/2026 21:05 | `v3.0-RC192` | 📄🗑️ **Chỉ Cho Phép Delete CV Version, Bỏ Hẳn Add/Edit Trực Tiếp Trên Candidate 360 (FIX_SPEC_2026-09-13-e)**: Rút gọn tính năng Add/Edit CV version trực tiếp theo quyết định của PO (nhằm loại bỏ rủi ro gõ/dán nhầm link CV không qua xác nhận). Từ nay CV mới CHỈ được nạp qua luồng "+ Parse CV (AI)" có kiểm duyệt HITL review. Candidate 360 CHỈ còn giữ lại quyền Delete 1 version cụ thể. (1) **Server Actions (`src/app/actions.js`)**: Xoá bỏ 3 hàm `appendCvVersion`, `updateCvVersion`, `triggerCvReparse`. Giữ nguyên hàm `deleteCvVersion`; (2) **Candidate 360 UI (`src/app/candidates/page.js`)**: Xoá import 3 hàm trên, xoá các state modal append CV, xoá hàm `handleAppendCvVersion`, bỏ gọi `triggerCvReparse` trong `handleSaveProfile`, khoá input "CV Link" thành `readOnly` + `disabled`, xoá nút Add Version và nút Edit ở toolbar CV Viewer, xoá modal Append New CV Version; (3) **Môi trường**: Xoá biến `N8N_CV_REPARSE_WEBHOOK_URL` khỏi `.env.local`; (4) **Verify**: `node --check` PASS 100% 2 files, `git grep` 0 kết quả thừa, Turbopack `npm run build` PASS 100% 31/31 routes. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260913-164`** | 13/09/2026 20:45 | `v3.0-RC191` | 🔒 **Khắc Phục Sự Cố Lộ `BRIDGE_INTERNAL_SECRET` Qua "AI Collab History"**: Phát hiện qua đối chiếu độc lập (Gemini): file snapshot tham khảo `AI Collab History/2026-09-13_...md` bị commit nhầm lên `master` (`d1a69ff`) chứa plaintext secret VPS đang active. Xử lý: (1) Rotate lại `BRIDGE_INTERNAL_SECRET` lần 2 trên VPS (xem SNAP-163); (2) Gỡ hẳn file + folder khỏi Git (`50edb8b`); (3) Thêm rule `AI Collab History/` vào `.gitignore` gốc làm hàng rào kỹ thuật; (4) Sửa lại đoạn hướng dẫn sai trong `bridge/HUONG_DAN.md` từng gợi ý commit thư mục này vào repo (`73d81fa`); (5) Merge toàn bộ vào `development`. Quyết định PO: log trao đổi Claude↔Antigravity từ nay CHỈ lưu cục bộ ngoài repo, không qua Git. | ✅ Stable | Repo (Git history) |
| **`SNAP-20260913-163`** | 13/09/2026 20:40 | `v3.0-RC191` | 🔑 **Rotate Lại `BRIDGE_INTERNAL_SECRET` Lần 2 (VPS Production)**: Giá trị rotate lần 1 cùng ngày (13:50) bị lộ qua sự cố ở SNAP-164 — rotate lại ngay để vô hiệu hoá. Sinh secret mới trên VPS (`openssl rand -hex 32`), cập nhật `/opt/n8n/facebook auto posting 2.0/.env`, restart `pm2 fb-bridge`, verify: secret cũ bị lộ → `401`, secret mới → `200`. PO đã tự cập nhật giá trị mới vào n8n Credential "ATS 3.0 VPS Bridge Secret". Giá trị secret không được ghi vào bất kỳ file nào trong repo. | ✅ Stable | VPS + n8n |
| **`SNAP-20260913-162`** | 13/09/2026 20:05 | `v3.0-RC190` | 📄✏️ **Edit & Delete Từng CV Version Cụ Thể Trong Candidate 360° (Embedded CV Viewer)**: (1) **Server Actions (`src/app/actions.js`)**: Thêm `updateCvVersion(candidateId, index, { url, filename })` và `deleteCvVersion(candidateId, index)` bọc guard `assertRealRequestContext`, bảo toàn bất biến `cv_url` đồng bộ với version cuối cùng của `cv_urls`, chặn xoá version cuối cùng còn lại; (2) **Candidate 360 UI (`src/app/candidates/page.js`)**: Import 2 actions mới, bổ sung `arrayIndex` và `filename` vào useMemo `allCvUrls`, thêm state `editingCvVersionIndex`, nâng cấp `handleAppendCvVersion` dùng chung cho cả Add/Edit (không trigger re-parse khi edit), thêm handler `handleDeleteCvVersion`, gắn nút Edit (`Pencil`) và Delete (`Trash2`) cạnh dropdown version (chỉ hiện cho version có thật trong mảng), nâng cấp modal Add/Edit CV Version hiển thị tiêu đề và nút tương ứng theo chế độ. **Polish sau đó (Claude, theo phản hồi PO xem UI thật)**: bỏ dấu "+" thừa trên nút "Add Version", đồng bộ style 4 nút trong toolbar, bỏ hiển thị URL thô (chỉ còn trong modal Edit), đổi toolbar sang `flex-wrap` để nút không bị cắt/ẩn khi hẹp; (3) **Verify**: `node --check` PASS 100% 2 files, `npm run build` Turbopack PASS 100% 31/31 routes. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260913-161`** | 13/09/2026 19:30 | `v3.0-RC189` | 📄🔄 **Tự Động Re-parse CV Khi Cập Nhật (Qua HITL) + Báo Lỗi Rõ Ràng (Mục #14)**: (1) **Server Action (`src/app/actions.js`)**: Thêm `triggerCvReparse(candidateId)` kèm guard `assertRealRequestContext`, trích xuất `driveFileId` từ Google Drive URL, tạo Notification `cv_reparse` ban đầu, và dispatch webhook sang n8n kèm `baseUrl` sandbox/production; (2) **Candidate 360 UI (`src/app/candidates/page.js`)**: Import `triggerCvReparse`, tự động gọi khi append CV version thành công trong `handleAppendCvVersion` và khi field `cv_url` thực sự thay đổi trong `handleSaveProfile`; (3) **n8n Workflow A5 (`8sdwRFG354PTL8Oj`)**: Tạo mới workflow "A5: CV Re-parse (Existing Candidate)" trong thư mục "ATS 3.0" gồm 8 nodes (Webhook HeaderAuth, Prepare Context, Create Batch Record, Insert Batch Item, Build SubWorkflow Input, Process Single Item `WfSingle00000001`, Build Reparse Notification, POST Final Notification), phân loại thông báo rõ ràng (`UPDATE` success, `ERROR` failure, `CONFLICT`/`NEW` warning), publish `active: true`; (4) **Verify**: `node --check` PASS 100% 2 files, n8n active version verify PASS 100%, Turbopack `npm run build` PASS 100% 31/31 routes. Theo Implementation Spec #14 từ Claude (Architect/QA). | ✅ Stable | Repo + n8n |
| **`SNAP-20260913-160`** | 13/09/2026 19:25 | `v3.0-RC188` | 🔔📍 **Click Notification Dẫn Thẳng Tới Nơi Cần Điền (Mục #8) & Focus / Ghim Social Group Trên Library**: (1) **Notification Center Navigation (`PendingCVClientWrapper.js`)**: Kích hoạt `router.push(notif.link)` khi click notification có `link`, tự động gọi `handleMarkRead(notif.id)` và `setIsOpen(false)`, thêm `e.stopPropagation()` trên nút check; (2) **Ghim Group Trong Library (`campaign_actions.js`)**: Thêm tham số `focusGroupId` vào `getSocialGroupsLibrary`, bổ sung `OR sgu.id = focusGroupId` vào WHERE và `ORDER BY (sgu.id = focusGroupId) DESC` sắp xếp group mục tiêu lên đầu trang 1; (3) **Deep Linking Social Groups Tab (`campaigns/page.js`)**: Sửa `useEffect` đọc `group_id` query param lưu vào state `focusGroupId` (thay vì nhét vào ô search), truyền `focusGroupId` xuống `fetchSocialGroupsLibrary` và truyền prop `autoOpenAnswer={g.id === focusGroupId}` vào `JoinStatusBadge`; (4) **Auto Open Answer Popover & Scroll (`JoinStatusBadge.js`)**: Thêm prop `autoOpenAnswer = false`, khởi tạo `showAnswerPopover(autoOpenAnswer)` và `useEffect` tự động cuộn `scrollIntoView({ behavior: 'smooth', block: 'center' })` vào đúng badge; (5) **Verify**: `node --check` PASS 100% 4 files, Turbopack `npm run build` PASS 100% 31/31 routes. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |

---

## 📝 Chi Tiết Từng Snapshot

### [2026-09-18 01:25] Circuit Breaker chỉ trip khi lỗi phản ánh tài khoản có vấn đề (session/Checkpoint) (FIX_SPEC_2026-09-18b)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/campaign_actions.js`, `src/app/api/webhooks/campaign-run-progress/route.js`, `src/app/api/webhooks/warm-join-run-progress/route.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Bối cảnh & Quyết định PO**:
     - Circuit breaker (tự gỡ account khỏi campaign khi gặp >=3 lỗi) trước đây áp dụng cho mọi loại lỗi `Failed` bất kể nguyên nhân (lỗi UI selector, timeout mạng, selector không tìm thấy ô nhập).
     - Thống nhất theo FIX_SPEC_2026-09-18b: Circuit breaker CHỈ trip cho 2 loại sự kiện: (1) `status/action = 'Checkpoint'`, (2) `status/action = 'Failed'` VÀ `error_message` chứa lỗi session/đăng nhập (`session expired`, `login required`, `fb-session.json not found`). KHÔNG trip cho lỗi UI selector, network/timeout, chưa là thành viên group.
     - Giữ nguyên ngưỡng `>= 3` và không sửa hàm `_maybeTripCircuitBreaker`, không can thiệp bảng breaker log cũ.
  2. **Chi tiết code**:
     - **`src/app/campaign_actions.js`**: Thêm và export hàm `isAccountHealthFailure(errorMessage)`.
     - **`src/app/api/webhooks/campaign-run-progress/route.js`**:
       * Import `isAccountHealthFailure`.
       * Sửa điều kiện kích hoạt breaker: `const isAccountHealthEvent = status === 'Checkpoint' || (status === 'Failed' && await isAccountHealthFailure(errorMessage));`
       * Sửa câu SQL đếm `failedCount` chỉ đếm `status = 'Checkpoint' OR (status = 'Failed' AND (error_message ILIKE '%session expired%' OR error_message ILIKE '%login required%' OR error_message ILIKE '%fb-session.json not found%'))`.
     - **`src/app/api/webhooks/warm-join-run-progress/route.js`**:
       * Import `isAccountHealthFailure`.
       * Sửa `isAccountHealthEventThisCall` kiểm tra cả accountAction lẫn `groupItems` với `isAccountHealthFailure`.
       * Sửa SQL đếm `failedCount` tương tự với `action = 'Checkpoint' OR (action = 'Failed' AND ...)`.
- Verify:
  - `node --check src/app/campaign_actions.js src/app/api/webhooks/campaign-run-progress/route.js src/app/api/webhooks/warm-join-run-progress/route.js`: PASS 100%.
  - Full HTTP E2E integration test suite trên Sandbox schema (`test_circuit_breaker_scenarios.mjs`): PASS 100% 5/5 kịch bản:
    * **Kịch bản A (UI Selector Errors - Không trip)**: Gửi 5 lần lỗi UI selector (`Could not find text editor...`) -> `campaign_fb_accounts` vẫn tồn tại, `circuit_breaker_log` không có bản ghi nào -> PASS.
    * **Kịch bản B (Session Expired - Trip ở lần 3)**: Gửi 3 lần lỗi `Facebook session expired or login required...` -> Lần 1 & 2 chưa trip, lần 3 gỡ khỏi `campaign_fb_accounts` và ghi log `failed_count = 3, trigger_type = 'job_posting'` -> PASS.
    * **Kịch bản C (Checkpoint - Trip ở lần 3)**: Gửi 3 lần `status: 'Checkpoint'` -> Lần 1 & 2 chưa trip, lần 3 gỡ khỏi `campaign_fb_accounts` và ghi log `failed_count = 3` -> PASS.
    * **Kịch bản D (Trộn lẫn UI Errors & Checkpoints - Đếm đúng sự kiện hợp lệ)**: Gửi 2 lỗi UI selector + 2 Checkpoints -> Chưa trip (mới có 2 sự kiện hợp lệ); Gửi tiếp 1 Checkpoint thứ 3 -> Trip thành công ở lần 3 -> PASS.
    * **Kịch bản E (Warming Route - Trộn lẫn lỗi non-health và session/checkpoint)**: Gửi 1 lỗi UI warming + 1 session expired + 1 checkpoint -> Chưa trip (mới 2 sự kiện); Gửi tiếp 1 lỗi `fb-session.json not found` -> Trip thành công với `trigger_type = 'warming'` -> PASS.
    * Dọn dẹp sạch toàn bộ dữ liệu test trong sandbox schema.
  - Next.js Turbopack `npm run build`: PASS 100% (31/31 routes).
  - `/api/qa-test`: PASS 100% (6/6 test suites).
  - `/api/biz-test`: PASS 100% (28/28 tests).
- ⚠️ Sai lệch so với spec:
  - Spec gốc yêu cầu: `export function isAccountHealthFailure(errorMessage)`
  - Lỗi gặp phải: `src/app/campaign_actions.js` có chỉ thị `'use server'` ở đầu file, nên Next.js Turbopack compiler bắt buộc mọi exported function phải là `async function` (nếu khai báo `export function` sẽ bị lỗi build: `Error: Server Actions must be async functions.`).
  - Giải pháp thay thế đã dùng: Khai báo `export async function isAccountHealthFailure(errorMessage)` và dùng `await isAccountHealthFailure(...)` trong 2 webhook route. Logic nghiệp vụ và các từ khóa nhận diện giữ nguyên 100% đúng theo spec.
  - Đã báo Claude/User: Có, ghi rõ trong DEVELOPMENT_LOG.md và báo cáo hoàn thành task.

### [2026-09-18 00:35] Archive (ẩn) Application record + Edit Job (sửa job gán nhầm) trên Candidate 360° (FIX_SPEC_2026-09-18)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `docs/testing/SQL_2026-09-18_activity_is_active-column.sql`, `src/app/actions.js`, `src/app/candidates/page.js`, `docs/features/candidates-hub.md`, `docs/USER_MANUAL_DRAFT.md`, `docs/architecture/ATS_3.0_UI_Modernization_Blueprint.md`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Bối cảnh & Quyết định PO**:
     - PO cần tính năng xử lý các đơn ứng tuyển gán nhầm Job hoặc không còn nhu cầu theo dõi mà không muốn DELETE cứng làm mất dữ liệu.
     - Triển khai giải pháp kép: Archive (ẩn mềm qua `is_active = false`) + Edit Job Assignment (đổi Job trực tiếp cho đơn đã tồn tại).
  2. **Chi tiết triển khai**:
     - **DB Migration (`public` & `sandbox`)**: Thêm cột `is_active boolean NOT NULL DEFAULT true` vào bảng `activity`. Lưu file `docs/testing/SQL_2026-09-18_activity_is_active-column.sql`.
     - **Server Actions (`src/app/actions.js`)**:
       * Thêm `setApplicationActive(applicationId, isActive)`: Độc lập với guard "Closed", cập nhật `is_active` và gọi `revalidatePath` cho `/`, `/candidates`, `/jobs`.
       * Thêm `changeApplicationJob(applicationId, newJobId)`: Đổi Job Order cho đơn hiện có, kiểm tra trạng thái Job đích (chặn nếu Closed/Filled/On Hold), kiểm tra chống trùng pipeline (`candidate_id = app.candidate_id AND job_id = newJobId AND id != applicationId`).
       * Sửa 4 query đọc:
         - `getActionMenuData`: Thêm `AND app.is_active = true` vào cả 2 khối WHERE (`countRes` và `data`).
         - `getCandidateProfile`: Thêm `app.is_active` vào SELECT list.
         - `getClientWorkbenchData`: Sửa JOIN condition `LEFT JOIN activity a ON j.id = a.job_id AND a.is_active = true`.
         - `getJobSearchData`: Sửa JOIN condition `LEFT JOIN activity app ON j.id = app.job_id AND app.is_active = true`.
     - **UI Candidate 360° (`src/app/candidates/page.js`)**:
       * Import `setApplicationActive`, `changeApplicationJob` từ `../actions` và `Archive`, `ArchiveRestore` từ `lucide-react`.
       * Thêm state `showArchived` và `editJobTarget`.
       * Tạo component `EditJobModal`: Tái sử dụng trọn vẹn pattern Searchable Combobox Client và Job từ `AssignToJobModal`, hiển thị Job hiện tại và gọi `changeApplicationJob`.
       * Refactor JSX card thành hàm `renderApplicationCard(app)` dùng chung cho Active và Archived.
       * Thêm nút `Archive` / `Restore` trên mỗi card.
       * Thêm nút icon `✏️ Pencil` cạnh tên Job để kích hoạt `EditJobModal`.
       * Tách `activeApplications` và `archivedApplications`, render khối toggle `Show/Hide Archived (N)`.
  3. **Tài liệu & Hướng dẫn sử dụng**:
     - Cập nhật `docs/features/candidates-hub.md` (mục 4 & 5).
     - Cập nhật `docs/USER_MANUAL_DRAFT.md` (mục 3.6 & 3.7).
     - Cập nhật `docs/architecture/ATS_3.0_UI_Modernization_Blueprint.md` (Version 3.0-RC215).
- Verify:
  - `node --check src/app/actions.js src/app/candidates/page.js` PASS 100%.
  - Migration verify: Query `information_schema.columns` xác nhận `is_active` tồn tại trên cả `public.activity` và `sandbox.activity` (`data_type: boolean, column_default: true`).
  - Isolated test suite `scratch/test-archive-and-edit-job.mjs` PASS 100% 3/3 kịch bản:
    * Kịch bản 1: Archive & Restore (ẩn khỏi Action Menu, giảm count Job A, ẩn khỏi active list, hiện trong Show Archived, restore phục hồi lại tất cả).
    * Kịch bản 2: Edit Job duplicate prevention (chặn đổi sang Job B khi candidate đã có application ở Job B).
    * Kịch bản 3: Edit Job sang Job C thành công (cập nhật `job_title`, applicant_count Job A = 0, Job C = 1).
    * Dọn dẹp sạch toàn bộ dữ liệu test.
  - Next.js Turbopack `npm run build` PASS 100% 31/31 routes.

### [2026-09-18 00:05] Sửa lỗi tạo trùng lặp Google Contact mỗi lần "Save to Google Contacts" (FIX_SPEC_2026-09-17c)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: n8n workflow `O659fZyN2uGaaLyL` (node `Match Logic`), `src/app/actions.js`, `src/app/candidates/page.js`, `src/app/search/page.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Bối cảnh & Root Causes (FIX_SPEC_2026-09-17c)**:
     - Google Contacts xuất hiện nhiều bản trùng lặp cho cùng 1 candidate khi người dùng bấm "Save to Google Contacts" nhiều lần.
     - Root Cause 1 (chính): Node `Match Logic` (`n007`) trong workflow `O659fZyN2uGaaLyL` có early-return `if (!data.hasMatchableContacts)` cho candidate không có Email/Phone/LinkedIn, dẫn tới luôn route sang nhánh Create.
     - Root Cause 2 (phụ): Nút "Save to Google Contacts" trên Candidate 360° và Search Hub thiếu state loading/disable, gây race condition khi người dùng click nhanh/double-click.
     - Phát hiện phụ: `atsUrl` trong `src/app/actions.js:2272` sai định dạng `/candidates/${c.id}` thay vì query param `/candidates?id=${c.id}`.
  2. **Chi tiết thay đổi**:
     - **n8n Workflow `O659fZyN2uGaaLyL` (node `Match Logic` - `n007`)**:
       * Xoá hoàn toàn khối `if (!data.hasMatchableContacts) { ... }`.
       * Thêm logic match qua `atsUrl` trong biography:
         ```js
         ...
         ```
       * Workflow đã publish active (version `e37fde22-9b66-433b-99c0-608e7b7cb71f`).
     - **`src/app/actions.js`**: Dòng 2272 đổi `atsUrl: `${baseUrl}/candidates/${c.id}`` thành `atsUrl: `${baseUrl}/candidates?id=${c.id}``.
     - **`src/app/candidates/page.js`**: Thêm state `isSyncingGoogleContacts`, disable nút và hiển thị "Syncing..." khi đang xử lý.
     - **`src/app/search/page.js`**: Thêm state `isSyncingGoogleContacts`, disable nút và hiển thị "Syncing..." khi đang xử lý.
- Verify:
  - `node --check src/app/actions.js src/app/candidates/page.js src/app/search/page.js` -> PASS 100%.
  - `get_workflow_details` xác nhận CHỈ node `Match Logic` (`n007`) thay đổi, active version `e37fde22-9b66-433b-99c0-608e7b7cb71f`.
  - Test cách ly sandbox candidate `f9999999-0000-0000-0000-000000000001` (chỉ có Facebook, không Phone/Email/LinkedIn):
    * Lần sync 1 (Execution 12185): `matchCount: 0` -> Route nhánh Create -> Google Contact `people/c1547478484384854029` được tạo thành công kèm `biographies` chứa ATS URL.
    * Lần sync 2 (Execution 12186): `matchCount: 1`, `matchedOn: ["ATS URL (biography)"]` -> Route nhánh Update -> Google Contact `people/c1547478484384854029` được update, KHÔNG tạo thêm bản trùng.
    * Dọn dẹp: Đã xoá contact test `people/c1547478484384854029` khỏi Google Contacts (Execution 12190) và xoá candidate test khỏi Postgres DB.
- ✅ Không có sai lệch so với spec.

### [2026-09-17 23:55] Deep Link Notification "Hoàn Tất Batch CV" Tới Hồ Sơ Candidate (n8n Workflow fofSZKkdyhlVd9Lc)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (n8n workflow active version `5d0bed8a-41bc-4fe6-90f3-23d294926f3c`; file docs chờ Claude review & commit theo Rule 10.9)
- Files: n8n workflow `fofSZKkdyhlVd9Lc` (node `Finalize Batch`), `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Bối cảnh & Root Cause (FIX_SPEC_2026-09-17b)**:
     - Khi người dùng click notification "Hoàn tất batch CV: N/N" trên Notification Center, hệ thống điều hướng về `/` (Action Menu) thay vì mở thẳng hồ sơ candidate vừa xử lý.
     - Root cause: Node `Finalize Batch` (`finalize_batch_node`) trong workflow n8n `fofSZKkdyhlVd9Lc` hardcode trường `link: '/'` khi gọi webhook notification.
  2. **Chi tiết thay đổi trên n8n Workflow (`fofSZKkdyhlVd9Lc`)**:
     - Sửa đúng 1 node `Finalize Batch` (id `finalize_batch_node`):
       * Thêm khối xác định deep link:
         ```js
         let link = '/candidates';
         ...
         ```
       * Đổi `link: '/'` thành `link,` trong payload POST `/api/webhooks/notifications`.
       * Giữ nguyên 100% logic còn lại và toàn bộ cấu trúc kết nối của workflow.
- Verify:
  - `validate_node_config`: PASS 100%.
  - `update_workflow`: PASS 100% (1 applied operation).
  - `publish_workflow`: Active version `5d0bed8a-41bc-4fe6-90f3-23d294926f3c`.
  - `get_workflow_versions_diff`: PASS 100% (xác nhận `nodesAdded: []`, `nodesRemoved: []`, `connectionsAdded: []`, `connectionsRemoved: []`, duy nhất node `finalize_batch_node` được cập nhật `parameters.jsCode`).

### [2026-09-17 23:40] Đổi Status Filter Mặc Định Của Action Menu Sang "All Status" (Round 2)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/page.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Bối cảnh & Quyết định PO (Follow-up SNAP-20260917-185)**:
     - Sau khi PHẦN A xác nhận Bug 1 thực chất do Action Menu mặc định lọc `statusFilter = "In progress"` làm ẩn các application `Closed`, PO đã chính thức phê duyệt đổi giá trị mặc định sang `"All Status"`.
  2. **Chi tiết code (`src/app/page.js`)**:
     - Sửa đúng 1 dòng: `const [statusFilter, setStatusFilter] = useState("ALL");` (thay cho `"In progress"`).
     - Giữ nguyên toàn bộ logic dropdown filter, `handleStatusFilterChange` và `handleInlineUpdate`.
- Verify:
  - `node --check src/app/page.js`: PASS 100%.
  - `git diff --stat`: Chỉ `src/app/page.js` và `docs/DEVELOPMENT_LOG.md`.
  - Browser verification (`npm run dev`): Mở `/` lần đầu ➔ Dropdown Status Filter hiển thị mặc định `"All Status"` ➔ Bảng nạp 80 rows gồm cả 26 In Progress và 54 Closed ➔ Đổi thủ công sang "In Progress" (80 rows), "Closed" (80 rows) và "ALL" hoạt động hoàn toàn chính xác.
  - Next.js Turbopack `npm run build`: PASS 100% (31/31 routes).

### [2026-09-17 08:55] Toggle Auto-Show Cho Action Timeline & Chẩn Đoán Đồng Bộ Status Candidates ➔ Action Menu
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/page.js`, `docs/USER_MANUAL_DRAFT.md`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Bug 1 (Chẩn đoán đồng bộ Stage/Status giữa Candidates Hub và Action Menu)**:
     - Thực thi quy trình chẩn đoán bắt buộc Bước 1 (PHẦN A) trên `npm run dev` qua Chrome DevTools MCP browser automation trên candidate thật `#11856 Vu Van G`:
       * **Kịch bản (a)** (Click tab "Action Menu" trên NavbarTabs): PASS (Stage cập nhật từ Talent Mapping ➔ Contact / Reach Out, `ActionMenuPage mounted` log xuất hiện, nạp live DB data).
       * **Kịch bản (b)** (Click link "Open in Action Menu" trên application card): PASS (Stage cập nhật sang Waiting for CV, `ActionMenuPage mounted` log xuất hiện, nạp live DB data).
       * **Kịch bản (c)** (Mở sẵn `/` từ trước, sang `/candidates` đổi sang Send CV To AH/Client, bấm nút Browser Back quay lại `/`): PASS (Stage cập nhật sang Send CV to Client, Next.js 16.3.0 remount và nạp live DB data).
       * **Phân tích nguyên nhân PO báo lỗi**: Khi đổi Status sang `Closed` trên Candidates, Action Menu có `statusFilter` mặc định là `"In progress"` nên application đó bị ẩn khỏi view mặc định (đúng thiết kế). Khi chuyển filter sang `Closed` hoặc `All Status`, hồ sơ lập tức xuất hiện đúng trạng thái `Closed`. Không có hiện tượng stale data/cache client trong cả 3 kịch bản. Theo đúng spec (mục 5 Bước 1 & Bước 2), giữ nguyên code logic fetch hiện tại, không can thiệp cache global.
  2. **Bug 2 (Thêm toggle tắt Auto-Show cho Action Timeline — Smart Auto-Slide)**:
     - `src/app/page.js`: Import `Pin, PinOff` từ `lucide-react`.
     - Thêm state `autoSlideEnabled` (default `true`) và nạp từ `localStorage.getItem("ats_action_menu_auto_slide_enabled")` khi mount.
     - Thêm hàm `toggleAutoSlide()` cập nhật state và lưu `localStorage`.
     - Sửa `handleMasterWheel`: chỉ kích hoạt `setIsDetailVisible(true)` sau 1800ms khi `autoSlideEnabled === true` (vẫn giữ nguyên auto-hide khi cuộn).
     - Thêm nút bấm toggle `Auto` / `Manual` trên header của sub-table Action Timeline (trước nút Expand/Collapse).
  3. **Tài liệu**: Cập nhật `docs/USER_MANUAL_DRAFT.md` mục 1.2 mô tả chi tiết nút Auto/Manual toggle mới.
- Verify:
  - `node --check src/app/page.js`: PASS 100%.
  - `git diff --stat`: Chỉ `src/app/page.js` (+ 2 file docs).
  - Test browser Bug 2:
    * Auto mode: Cuộn bảng trên ➔ Timeline ẩn ngay ➔ Tự hiện lại sau 1.8s.
    * Chuyển sang Manual ➔ Cuộn bảng trên ➔ Timeline ẩn ngay và KHÔNG tự hiện lại.
    * Bấm nút nổi `Action Timeline` (Quick Peek) ➔ Timeline mở lại đúng cách.
    * Reload trang (F5) ➔ Chế độ Manual được bảo toàn từ localStorage.
    * Toggle về Auto ➔ Hoạt động tự trồi lên phục hồi bình thường.
  - Next.js Turbopack `npm run build`: PASS 100% (31/31 routes).

### [2026-09-15 17:55] Lưu cookie xoay vòng trong post-to-group.js (khối finally) + tăng thời gian chờ reset IP 4G lên 15s
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `scripts/post-to-group.js`, `scripts/run-batch.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Lưu cookie mới nhất trong khối `finally` (`scripts/post-to-group.js`)**:
     - Thêm `if (context) { try { await context.storageState({ path: SESSION_PATH }); ... } catch (e) { ... } }` trong khối `finally` trước `browser.close()`.
     - Áp dụng cho MỌI nhánh kết thúc (kể cả Failed do lỗi UI không phải checkpoint) nhằm lưu lại các token xoay vòng của Facebook (xs/fr/sb), loại bỏ hiện tượng gửi lại session cookie cũ gây khóa phiên/checkpoint lặp lại.
  2. **Tăng thời gian chờ reset IP 4G (`scripts/run-batch.js`)**:
     - Tăng delay sau khi gọi URL reset IP từ 10s (`10000ms`) lên 15s (`15000ms`), giúp modem 4G có đủ thời gian thiết lập hoàn tất IP cellular mới trước khi đổi tài khoản trong batch.
  3. **Bối cảnh & Thẩm định**:
     - Fix dựa trên báo cáo RCA sự cố văng session/checkpoint trên acc_01/acc_02 đã được Claude (Architect/QA) thẩm định độc lập qua kiểm tra code và đối chiếu dữ liệu chạy thật (run #9437).
- Verify:
  - `node --check scripts/post-to-group.js scripts/run-batch.js`: PASS 100%.
  - `git diff --stat`: Đúng 2 files `scripts/` (+ doc).
  - Next.js Turbopack `npm run build`: PASS 100% (31/31 routes, 19.2s).
  - `/api/qa-test`: PASS 100% (6/6 test suites).
  - `/api/biz-test`: PASS 100% (28/28 tests).

### [2026-09-15 16:45] Xóa hẳn bộ đếm Failed cũ thừa của Circuit Breaker (thay thế hướng vá mốc thời gian)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/campaign_actions.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Xóa 2 hàm helper cũ (`src/app/campaign_actions.js`)**:
     - Xóa hoàn toàn `_getDisabledAccountIdsForJobCampaign` và `_getDisabledAccountIdsForWarmingCampaign`.
  2. **Gỡ bỏ lớp lọc thừa tại 3 vị trí gọi (`src/app/campaign_actions.js`)**:
     - `_getEligibilityState` (Job Posting): Xóa đoạn lọc `accountState.delete(disabledId)`.
     - `_acquireWarmJoinRunLock` (Warming resolve): Xóa đoạn lọc `targetAccounts = targetAccounts.filter(...)`.
     - `checkWarmingCampaignAutoCompletion`: Xóa điều kiện `disabledIds` trong query SQL và metadata notification.
  3. **Lý do & Nguyên tắc hoạt động**:
     - Fix thay thế hướng tiếp cận ban đầu (vá bằng mốc thời gian). Theo phân tích của PO: khi account fail >= 3 lần, `_maybeTripCircuitBreaker` đã xóa dòng gán thật trong `campaign_fb_accounts` và log lại (đủ để chặn fallback). Do đó `campaign_fb_accounts` là NGUỒN XÁC ĐỊNH DUY NHẤT cho việc account có thuộc campaign hay không. Bộ đếm cũ đếm toàn bộ lịch sử `Failed` trong quá khứ là thừa và gây bug chặn lại account vừa được PO kiểm tra và tick lại.
     - Sau khi xóa, khi PO tick lại account vào campaign, account sẽ xuất hiện lại ngay lập tức trong Dispatch Preview / Lock acquisition mà không cần xử lý thêm.
- Verify:
  - `node --check src/app/campaign_actions.js`: PASS 100%.
  - `grep -rn "_getDisabledAccountIdsForJobCampaign\|_getDisabledAccountIdsForWarmingCampaign" src/`: Trả về rỗng (0 matches).
  - Isolated E2E test trong sandbox:
    - Job Posting: Acc A failed 3 lần -> Tripped & xóa khỏi `campaign_fb_accounts` + tạo cbLog -> PO tick lại Acc A -> Acc A xuất hiện lại ngay lập tức trong Dispatch Preview -> Acc A failed tiếp 3 lần -> Tripped lần 2 thành công -> PASS 100%.
    - Warming: Acc A failed 3 lần -> Tripped & xóa khỏi `campaign_fb_accounts` -> PO tick lại Acc A -> `_acquireWarmJoinRunLock` nạp lại Acc A ngay lập tức -> `checkWarmingCampaignAutoCompletion` tính đúng `remainingWork` khi còn group chưa join và auto-archive khi tất cả group đã join -> PASS 100%.
    - Multi-account isolation: Acc A bị trip không ảnh hưởng tới tính hợp lệ của Acc B -> PASS 100%.
  - `/api/qa-test`: PASS 100% (6/6 test suites).
  - `/api/biz-test`: PASS 100% (28/28 test suites).
  - Next.js Turbopack `npm run build`: PASS 100% (31/31 routes).

### [2026-09-15 16:20] Circuit breaker tự gỡ account khỏi campaign (thật) + chặn fallback khi đã bị gỡ
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/campaign_actions.js`, `src/app/api/webhooks/campaign-run-progress/route.js`, `src/app/api/webhooks/warm-join-run-progress/route.js`, `src/app/components/CampaignEditModal.js`, `src/app/components/CampaignDispatchPreviewModal.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Helper dùng chung `_maybeTripCircuitBreaker` (`src/app/campaign_actions.js`)**:
     - Export hàm `_maybeTripCircuitBreaker(sqlTx, campaignId, fbAccountId, failedCount, triggerType)`: khi `failedCount >= 3`, DELETE bản ghi gán trong `campaign_fb_accounts` và INSERT / ON CONFLICT UPDATE vào `campaign_account_circuit_breaker_log` (lưu `campaign_id, fb_account_id, trigger_type, failed_count, disabled_at`).
  2. **Chặn fallback "tất cả account Active" (`src/app/campaign_actions.js`)**:
     - Trong `_getEligibilityState` (Job Posting) và khối resolve `targetAccounts` của Warming, khi `cfa` không có account nào, kiểm tra `SELECT 1 FROM campaign_account_circuit_breaker_log WHERE campaign_id = ...`. Nếu đã từng có log breaker, CHẶN fallback sang "tất cả account Active" (tránh campaign âm thầm đổi sang account khác ngoài ý định của PO).
  3. **Truy xuất Circuit Breaker Log cho UI (`src/app/campaign_actions.js`)**:
     - `getCampaignDetail`: Query `campaign_account_circuit_breaker_log` join `fb_accounts` và trả về mảng `circuitBreakerLog` trong response data.
     - `computeCampaignDispatchPreview`: Query `circuitBreakerLog` và gắn `circuitBreakerRemovedAccounts` vào cả 2 đối tượng `stats` (early-return và main return).
  4. **Trip Circuit Breaker trong Webhook Progress**:
     - `src/app/api/webhooks/campaign-run-progress/route.js`: Khi `status === 'Failed'`, đếm lại tổng `Failed` của `fbAccountId` trong campaign từ `campaign_run_items` và gọi `_maybeTripCircuitBreaker(sqlTx, breakerCampaignId, fbAccountId, failedCount, 'job_posting')` trong cùng transaction.
     - `src/app/api/webhooks/warm-join-run-progress/route.js`: Khi có action `Failed` (ở cấp account hoặc group), đếm lại tổng `Failed` của `fbAccountId` trong campaign từ `warm_join_run_items` và gọi `_maybeTripCircuitBreaker(sqlTx, run.campaign_id, fbAccountId, failedCount, 'warming')` trong cùng transaction.
  5. **Cảnh báo UI**:
     - `src/app/components/CampaignEditModal.js`: Thêm state `circuitBreakerLog`, nạp từ `detailRes.data.circuitBreakerLog`, hiển thị banner amber cảnh báo "Auto-removed due to repeated failures: {account_name} — {failed_count} failed items. Check this account on Facebook before re-adding it to the pool below." ngay phía trên danh sách account pool.
     - `src/app/components/CampaignDispatchPreviewModal.js`: Trong empty dispatch view ("No target groups are currently eligible..."), hiển thị thêm dòng cảnh báo màu amber liệt kê các account bị breaker gỡ và gợi ý gán account khác để tiếp tục.
- Verify:
  - `node --check` 5/5 files JS: PASS 100%.
  - `git diff --stat`: Đúng 5 files code nguồn và `docs/DEVELOPMENT_LOG.md`.
  - Next.js Turbopack `npm run build`: PASS 100% (31/31 routes, 15.9s).
  - `/api/qa-test`: PASS 100% (6/6 test suites gồm DB-06/07, DB-11/12, DB-20, BIZ-13, SMOKE, WARM-CLAMP).
  - `/api/biz-test`: PASS 100% (28/28 tests gồm BIZ-01..26 và PERF-01..03).
  - **Sandbox HTTP E2E Verification (`test_circuit_breaker_http.mjs` & `verify_circuit_breaker.mjs`)**:
    * **Job Posting Scenario**: Gán 1 account -> Gọi webhook 3 lần `status: 'Failed'` -> Lần 1 & 2: giữ nguyên gán, không có log -> Lần 3: dòng gán trong `campaign_fb_accounts` bị xóa thật, `campaign_account_circuit_breaker_log` tạo 1 dòng `failed_count=3, trigger_type='job_posting'`, `getCampaignDetail` trả về `circuitBreakerLog`, preview modal hiển thị `circuitBreakerRemovedAccounts` và chặn hoàn toàn fallback sang account Active khác.
    * **Warming Scenario**: Gán 1 account -> Gọi webhook warming 3 lần `action: 'Failed'` (cả account-level và group-level) -> Lần 3: dòng gán trong `campaign_fb_accounts` bị xóa thật, log ghi nhận `trigger_type='warming', failed_count=3`, warming dispatch resolve chặn hoàn toàn fallback sang account Active khác.
    * **Multi-Account Isolation**: Gán Account 1 và Account 2 -> Account 1 fail 3 lần -> Account 1 bị gỡ và ghi log, Account 2 vẫn được giữ nguyên và preview dispatch sang Account 2 bình thường.
    * Dọn dẹp sạch toàn bộ dữ liệu test sau khi hoàn tất.

### [2026-09-15 15:25] Bảng Run History tự cập nhật live + Fix giật do polling toàn cục & Fix phantom FB Account selection (/campaigns)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/campaigns/page.js`, `src/app/components/RunHistoryTable.js`, `src/app/components/CampaignEditModal.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Hạng mục 1: Bảng Run History tự cập nhật live + Fix giật do polling toàn cục**:
     - `src/app/campaigns/page.js` (`loadCampaignDetailData`): Di chuyển `setTargetGroupIds(tIds)` vào bên trong nhánh `if (!isSilent)`, loại bỏ hoàn toàn việc gọi `fetchTargetGroups` khi background silent refresh. Khi poll ngầm trong lúc có run đang chạy, chỉ cập nhật `campaignDetail` (để refresh `runs` cho Run History), không đè lại state `targetGroupIds` hay refetch group list, triệt để chấm dứt hiện tượng giật/reset checkbox khi user đang chọn nhóm.
     - `src/app/campaigns/page.js` (polling `useEffect`): Thu hẹp điều kiện gọi `loadCampaignDetailData(selectedCampaignId, true)` — chỉ gọi khi chính campaign đang xem có `latest_run_status === 'Running'`, bổ sung `campaigns` vào dependency array.
     - `src/app/components/RunHistoryTable.js`: Bổ sung `React.useEffect` tự động poll lại dữ liệu chi tiết từng nhóm (`fetchRunDetail(expandedRunId)`) mỗi 10 giây nếu dòng đang mở rộng (`expandedRunId`) có `status === "Running"`. Khi run hoàn tất hoặc đóng dòng, poll tự động dừng.
  2. **Hạng mục 2: Fix "phantom" FB Account đã gán nhưng không hiện checkbox**:
     - `src/app/components/CampaignEditModal.js`: Ngay sau khi khởi tạo `setSelectedAccountIds(new Set(assignedAccs.map((a) => a.id)))`, bổ sung cập nhật `setAvailableAccounts` kiểm tra và nạp thêm các account trong `assignedAccs` chưa có trong `availableAccounts` (kể cả account không còn `Active` như `Checkpoint`/`Disabled`).
     - Đảm bảo toàn bộ account đã được gán trước đó đều có checkbox tương ứng trên modal checklist, số lượng `(N selected)` hiển thị khớp chính xác 100% với số checkbox đang tick, cho phép user nhìn thấy và bỏ chọn qua UI bình thường.
- Verify:
  - `node --check` cả 3 files (`src/app/campaigns/page.js`, `src/app/components/RunHistoryTable.js`, `src/app/components/CampaignEditModal.js`): PASS 100%.
  - `git diff --stat`: Chỉ 3 file code nguồn trên và `docs/DEVELOPMENT_LOG.md`.
  - Next.js Turbopack `npm run build`: PASS 100% 31/31 routes (28.7s).
  - `/api/qa-test`: PASS 100% (6/6 test suites gồm DB-06/07, DB-11/12, DB-20, BIZ-13, SMOKE, WARM-CLAMP).
  - `/api/biz-test`: PASS 100% (28/28 tests gồm BIZ-01..26 và PERF-01..03).

### [2026-09-15 14:50] Redesign Campaign Status: 3 user-chọn (Draft, Active, Archived) + 1 hệ thống (Needs Review) & Tách rời trạng thái Running
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/campaign_actions.js`, `src/app/components/CampaignEditModal.js`, `src/app/campaigns/page.js`, `src/app/api/webhooks/campaign-run-callback/route.js`, `src/app/api/webhooks/warm-join-run-callback/route.js`, `src/app/api/webhooks/auto-run-eligible-campaigns/route.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **DB Migration (`sandbox.campaigns`)**:
     - Cập nhật constraint `campaigns_status_check` thành `status IN ('Draft', 'Active', 'Archived', 'Needs Review')`.
     - Chuyển đổi dữ liệu cũ: `Ready`, `Running`, `Failed` -> `Active`; `Paused` -> `Draft`; `Completed` -> `Archived`.
     - Đồng bộ `is_active = (status = 'Active')` và reset `auto_run_enabled = false` khi `status = 'Archived'`.
  2. **Server Actions (`src/app/campaign_actions.js`)**:
     - `createCampaign`: Gán mặc định `status = 'Draft'`, server-derive `is_active = (status === 'Active')`, validate whitelist `['Draft', 'Active', 'Archived']`.
     - `updateCampaign`: Bỏ tin cậy `is_active` từ client; validate whitelist user-choosable (`['Draft', 'Active', 'Archived']`), chặn client tự set `Needs Review`; cập nhật `is_active` nguyên tử qua `CASE status WHEN 'Active' THEN true ELSE false END`.
     - `getCampaigns`: Thay thế sentinel `HIDE_COMPLETED` bằng `HIDE_ARCHIVED` (`c.status <> 'Archived'`).
     - `_executeCampaignRunInternal` & `triggerWarmJoinRun`: Gỡ bỏ hoàn toàn việc UPDATE `status = 'Running'` vào bảng `campaigns` (chỉ cập nhật `last_updated`), giữ nguyên status gốc của campaign trong khi run được ghi nhận là `Running` trong `campaign_runs` / `warm_join_runs`.
     - `checkCampaignAutoCompletion` & `checkWarmingCampaignAutoCompletion`: Cập nhật `status = 'Archived'`, `is_active = false`, `auto_run_enabled = false`.
  3. **Campaign Edit Modal (`src/app/components/CampaignEditModal.js`)**:
     - Rút gọn `CAMPAIGN_STATUS_OPTIONS` từ 7 xuống 3: `Draft`, `Active`, `Archived`.
     - Render option disabled `<option value="Needs Review" disabled>` khi campaign đang ở trạng thái `Needs Review` (giúp user nhận biết và chọn sang trạng thái khác nhưng không thể quay lại `Needs Review`).
     - Loại bỏ `is_active` khỏi payload submit (được server tự tính).
     - Bổ sung cảnh báo dưới ô status: `"Note: the run currently in progress will continue to completion — pausing only prevents future runs."` khi user đổi từ Active sang Draft/Archived trong lúc có run đang Running.
  4. **Campaigns UI (`src/app/campaigns/page.js`)**:
     - Đổi default filter sang `HIDE_ARCHIVED` ("Active (Hide Archived)").
     - Bổ sung option `Needs Review` vào dropdown filter.
     - Sửa `hasRunningCampaign` và `isRunning` kiểm tra qua `c.latest_run_status === 'Running'`.
     - Nâng cấp `renderCampaignStatusBadge` nhận full campaign object `c`: ưu tiên `c.latest_run_status === 'Running'` (hiển thị badge Running xanh dương nhấp nháy), kế đến `Needs Review` (badge cam), và các trạng thái tĩnh `Active` (xanh lá), `Archived` (xám), `Draft` (xám nhạt).
     - Render toast warning banner khi pause campaign có run đang chạy.
  5. **Webhooks (`campaign-run-callback`, `warm-join-run-callback`, `auto-run-eligible-campaigns`)**:
     - `campaign-run-callback`: Cập nhật `status = 'Active'` khi hoàn tất (hoặc `status = 'Needs Review'`, `is_active = false`, `auto_run_enabled = false` khi systemic failure).
     - `warm-join-run-callback`: Cập nhật `status = 'Active'` khi hoàn tất.
     - `auto-run-eligible-campaigns`: Loại bỏ điều kiện lọc redundant `AND c.status NOT IN ('Running', 'Needs Review')`.
- Verify:
  - `node --check` 6/6 files: PASS 100%.
  - Next.js Turbopack `npm run build`: PASS 100% 31/31 routes (7.3s).
  - `/api/qa-test`: PASS 100% (6/6 tests).
  - `/api/biz-test`: PASS 100% (28/28 tests).
  - Sandbox isolated test suite: PASS 7/7 scenarios (DB constraint check, invalid status rejection, Draft default, Active update, Archived auto_run disable, run status separation, systemicFailure -> Needs Review).

### [2026-09-15 13:55] Thêm "Select All / Deselect All" cho Target Social Groups trong Campaign Edit Modal
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/components/CampaignEditModal.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **`src/app/components/CampaignEditModal.js`**:
     - Import server action `getSocialGroupIdsMatchingFilter` từ `../campaign_actions`.
     - Thêm 2 hàm xử lý `handleSelectAllFilteredGroups` và `handleDeselectAllFilteredGroups` áp dụng trên toàn bộ kết quả khớp `groupSearch` (qua server action `getSocialGroupIdsMatchingFilter({ search: groupSearch })`).
     - Thêm 2 nút `Select All ({groupTotalCount})` và `Deselect All` vào header mục Target Social Groups (giữa label và input search).
     - Điều chỉnh độ rộng ô search từ `w-48` xuống `w-40` để đảm bảo vừa khít 1 hàng trên modal.
- Verify:
  - `node --check src/app/components/CampaignEditModal.js`: PASS 100%.
  - `git diff --stat`: chỉ `src/app/components/CampaignEditModal.js` và `docs/DEVELOPMENT_LOG.md`.
  - Next.js Turbopack `npm run build`: PASS 100% 31/31 routes (24.3s).

### [2026-09-15 13:40] Bảo toàn query param `?id=` cho Facebook `profile.php` trong `normalizeContactValue`
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/lib/utils.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Facebook `profile.php` Query Preservation (`normalizeContactValue` trong `src/lib/utils.js`)**:
     - Bổ sung nhánh nhận diện `if (/facebook\.com\/profile\.php/i.test(val))` ngay trước đoạn xử lý URL chung.
     - Trích xuất Facebook ID qua `const idMatch = val.match(/id=(\d+)/)`.
     - Chuẩn hóa base path và trả về `${pathOnly}?id=${idMatch[1]}` nếu có ID, giữ nguyên định danh duy nhất của Facebook kiểu cũ thay vì cắt bỏ toàn bộ query string.
     - Giữ nguyên toàn bộ logic cho Phone, Email và các URL nền tảng khác (LinkedIn, Github, regular vanity Facebook URLs).
- Verify:
  - `node --check src/lib/utils.js`: PASS 100%.
  - Unit tests độc lập:
    - `normalizeContactValue('Facebook', 'https://www.facebook.com/profile.php?%0did=100045121424813')` → `'facebook.com/profile.php?id=100045121424813'` (PASS).
    - `normalizeContactValue('Facebook', 'https://www.facebook.com/profile.php?id=61581122866182')` → `'facebook.com/profile.php?id=61581122866182'` (PASS).
    - `normalizeContactValue('Facebook', 'https://www.facebook.com/phamgiang19/')` → `'facebook.com/phamgiang19'` (PASS - giữ nguyên hành vi vanity profile).
    - `normalizeContactValue('LinkedIn', 'https://www.linkedin.com/in/john-doe?trackingId=123')` → `'linkedin.com/in/john-doe'` (PASS - tracking query param bị lược bỏ đúng chuẩn).
    - `normalizeContactValue('Email', 'Test@Example.COM')` → `'test@example.com'` (PASS).
    - `normalizeContactValue('Phone', '0912345678')` → `'+84912345678'` (PASS).
  - `git diff --stat`: chỉ `src/lib/utils.js` và `docs/DEVELOPMENT_LOG.md`.
  - `/api/biz-test`: PASS 28/28 tests 100% (gồm BIZ-01..26 và PERF-01..03, không regression).
  - Next.js Turbopack `npm run build`: PASS 100% 31/31 routes (32.4s).

### [2026-09-15 11:10] Chuẩn hóa giá trị Contact Point trước khi insert trong 2 luồng HITL (`hitl_actions.js`)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/hitl_actions.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **FORCE_CREATE Contact Normalization (`createCandidateFromPayload` trong `src/app/hitl_actions.js`)**:
     - Thêm `.filter(cp => cp && cp.value)` loại bỏ contact rỗng/undefined.
     - Áp dụng `const normVal = normalizeContactValue(cp.type, cp.value)` và lưu `value: normVal || cp.value` trước khi insert xuống bảng `contact_points`.
     - Bọc `if (contactInserts.length > 0)` chống lỗi postgres.js khi mảng insert rỗng.
  2. **MERGE Flow Contact Normalization (`resolvePendingCVImport` MERGE branch trong `src/app/hitl_actions.js`)**:
     - Trong khối `dedupedContactPoints.map(...)`, áp dụng `const normVal = normalizeContactValue(cp.type, cp.value)` và lưu `value: normVal || cp.value` thay vì lưu `cp.value` thô.
     - Giữ nguyên logic filter dedup ở dòng 197-202 phía trên.
- Verify:
  - `node --check src/app/hitl_actions.js`: PASS 100%.
  - `git diff --stat`: chỉ `src/app/hitl_actions.js` và `docs/DEVELOPMENT_LOG.md`.
  - Sandbox isolated test suite (tự tạo/tự dọn):
    - Kịch bản 1 (FORCE_CREATE với Phone `'0901234567'` & LinkedIn URL `'https://www.linkedin.com/in/test-user/'`): Phone lưu `'+84901234567'`, LinkedIn lưu `'linkedin.com/in/test-user'`.
    - Kịch bản 2 (MERGE với Phone `'0987654321'` & LinkedIn URL `'https://www.linkedin.com/in/test-user/'` vào candidate đã có sẵn Email): Phone lưu `'+84987654321'`, LinkedIn lưu `'linkedin.com/in/test-user'`, Email trùng bị dedup bỏ qua chính xác.
    - Kịch bản 3 (Invalid phone fallback `'123'`): Lưu giá trị gốc `'123'`, không làm mất dữ liệu.
  - `/api/biz-test`: PASS 28/28 tests 100% (không regression).
  - `/api/qa-test`: PASS 6/6 test suites 100% (DB-06/07, DB-11/12, DB-20, BIZ-13, SMOKE, WARM-CLAMP).
  - Next.js Turbopack `npm run build`: PASS 100% 31/31 routes (13.6s).

### [2026-09-15 10:55] Chặn trùng Contact Point trong cùng 1 Candidate (Candidate 360° - Contact Points Hub)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/actions.js`, `src/app/api/biz-test/route.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Bug Fix Same-Candidate Contact Point Deduplication (`src/app/actions.js`)**:
     - `addContactPoint`: Gộp câu query dedup thành 1, bỏ điều kiện loại trừ `cp.candidate_id != ${candidateId}`, SELECT thêm `cp.candidate_id`. Nếu phát hiện trùng và `dups[0].candidate_id === candidateId`, trả về lỗi `"This contact already exists on this candidate's profile."`. Nếu trùng với candidate khác, trả về lỗi `"Duplicate Contact Detected: This contact is already associated with another candidate."`.
     - `updateContactPoint`: Áp dụng logic tương tự bên trong khối `if (candidateId) { ... }`, bỏ điều kiện `cp.candidate_id != ${candidateId}` (vẫn giữ `cp.id != ${contactId}`), phân biệt rõ lỗi trùng với chính mình và lỗi trùng với candidate khác.
  2. **Automated Test Suite (`src/app/api/biz-test/route.js`)**:
     - Thêm test case `BIZ-26` (`Contact Point Deduplication within Same Candidate & Cross Candidate`) kiểm thử tự động toàn diện 5 bước cách ly sandbox (tự sinh UUID, tự dọn dẹp sạch sẽ).
- Verify:
  - `node --check src/app/actions.js src/app/api/biz-test/route.js` PASS 100%.
  - Sandbox isolated verification test:
    - Kịch bản 1 (Add initial email `dedup-test-A@example.com` cho candidate A): `{"success": true}`.
    - Kịch bản 2 (Add lại cùng email `dedup-test-A@example.com` cho candidate A): Thất bại với message `"This contact already exists on this candidate's profile."`.
    - Kịch bản 3 (Add cùng email `dedup-test-A@example.com` cho candidate B): Thất bại với message `"Duplicate Contact Detected: This contact is already associated with another candidate."`.
    - Kịch bản 4 (Update contact point thứ 2 của candidate A thành `dedup-test-A@example.com` đã có của A): Thất bại với message `"This contact already exists on this candidate's profile."`.
    - Kịch bản 5 (Update contact point của candidate B thành `dedup-test-A@example.com` của candidate A): Thất bại với message `"Duplicate Contact Detected: This contact is already associated with another candidate."`.
  - `/api/biz-test`: PASS 28/28 tests 100% (bao gồm `BIZ-26` mới, không regression).
  - Next.js Turbopack `npm run build`: PASS 100% 31/31 routes (33.0s).

### [2026-09-15 10:15] Sửa lỗi lưu Campaign Status, Default Filter ẩn Completed, và On-Demand Collapse cột Linked Job (/campaigns)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/campaign_actions.js`, `src/app/campaigns/page.js`, `docs/USER_MANUAL_DRAFT.md`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **Hạng mục 1 (BUG - Campaign Status Save & Whitelist Validation)**:
     - `src/app/campaign_actions.js` (`updateCampaign`): Bổ sung `status` vào khối destructure dữ liệu đầu vào. Thêm validate whitelist 7 trạng thái hợp lệ (`['Draft', 'Ready', 'Running', 'Paused', 'Completed', 'Failed', 'Archived']`) đồng bộ với `CAMPAIGN_STATUS_OPTIONS` của `CampaignEditModal.js`. Bổ sung mệnh đề cập nhật `status = CASE WHEN ${status !== undefined} THEN ${status || null} ELSE status END` vào câu lệnh UPDATE SQL.
     - Sửa lỗi tiềm ẩn `UNDEFINED_VALUE` trong `postgres.js` template tag: bổ sung fallback null cho `${validatedMaxPostsPerRun !== undefined ? validatedMaxPostsPerRun : null}`.
  2. **Hạng mục 2 (Feature - Default Filter "Active (Hide Completed)")**:
     - `src/app/campaign_actions.js` (`getCampaigns`): Xử lý giá trị filter `status === 'HIDE_COMPLETED'` bằng mệnh đề SQL server-side `AND c.status <> 'Completed'`.
     - `src/app/campaigns/page.js`: Đổi giá trị khởi tạo state `campaignStatusFilter` từ `"ALL"` thành `"HIDE_COMPLETED"`. Bổ sung option `<option value="HIDE_COMPLETED">Active (Hide Completed)</option>` vào đầu dropdown Status filter.
  3. **Hạng mục 3 (UI On-Demand Clean UX - Linked Job Column Expand/Collapse)**:
     - `src/app/campaigns/page.js`: Khởi tạo state `expandedJobsRowIds` (`new Set()`) và hằng số `LINKED_JOB_BADGES_VISIBLE_LIMIT = 2` cùng helper `toggleJobsRowExpanded(campaignId)`.
     - Cột Linked Job trong bảng Master Campaigns mặc định chỉ hiển thị tối đa 2 badge Job đầu tiên, kèm nút bấm `+N more` / `See less` (`e.stopPropagation()` để không làm kích hoạt chọn dòng / mở Campaign Details panel, đồng thời click trực tiếp badge Job vẫn điều hướng sang `/jobs?job_id=...`).
  4. **Tài liệu**: Cập nhật `docs/USER_MANUAL_DRAFT.md` mục 7.1.
- Verify:
  - `node --check src/app/campaign_actions.js src/app/campaigns/page.js` PASS 100%.
  - Sandbox test suite (`test-campaign-spec`):
    - Status Save: Tạo campaign test, cập nhật sang `Paused` -> DB cập nhật `status = 'Paused'`, trả về `success: true`.
    - Whitelist Validation: Cập nhật sang status không hợp lệ `"Bogus"` -> trả về lỗi `Invalid status: "Bogus"`, DB giữ nguyên `status = 'Paused'`.
    - Filter `HIDE_COMPLETED`: Trả về 5 campaign (chứa `Paused`, loại trừ `Completed`).
    - Filter `ALL`: Trả về toàn bộ 6 campaign (chứa cả `Paused` và `Completed`).
    - Filter `Completed`: Trả về đúng 1 campaign `Completed` (loại trừ `Paused`).
    - Dọn dẹp sạch toàn bộ test data trong sandbox.
  - Regression test suites:
    - `/api/qa-test`: PASS 6/6 test suites 100% (DB-06/07, DB-11/12, DB-20, BIZ-13, SMOKE, WARM-CLAMP).
    - `/api/biz-test`: PASS 27/27 tests 100% (BIZ-01..25, PERF-01..03).
  - Next.js Turbopack `npm run build`: PASS 100% 31/31 routes (25.7s).

### [2026-09-15 10:00] Thêm nút Expand/Collapse cho Campaign Details Panel (`/campaigns`)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/campaigns/page.js`, `docs/USER_MANUAL_DRAFT.md`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **`src/app/campaigns/page.js`**:
     - Import `Maximize2, Minimize2` từ `lucide-react`.
     - Thêm state `isDetailPanelExpanded` (mặc định `false`).
     - Đổi chiều cao của container Campaign Details Panel sang dynamic responsive `h-[clamp(380px,54vh,760px)] shrink-0` khi `isDetailPanelExpanded = true` và `flex-1 min-h-[360px]` khi `false`.
     - Điều chỉnh Master Campaigns Table tự động thu gọn về `shrink-0 max-h-36` khi đang Expand (áp dụng mục 6 tuỳ chọn của spec) để đảm bảo toàn bộ layout nằm trọn vẹn trong viewport 768px mà không bị cắt đáy.
     - Thêm nút Expand/Collapse (`Maximize2`/`Minimize2`, text "Expand"/"Collapse", tooltip động) trên Header của Detail Panel đặt trước nút Close "X".
     - Đặt lại `setIsDetailPanelExpanded(false)` khi bấm nút Close "X" để đảm bảo mỗi lần mở campaign mới đều bắt đầu ở trạng thái thu gọn mặc định.
  2. **`docs/USER_MANUAL_DRAFT.md`**:
     - Cập nhật mục 7.3 bổ sung hướng dẫn sử dụng nút Expand/Collapse trên Campaign Details Panel.
- Verify:
  - `node --check src/app/campaigns/page.js` PASS 100%.
  - `git diff --stat` xác nhận chỉ các file trong phạm vi được chỉnh sửa.
  - Browser testing qua Chrome DevTools MCP:
    - Viewport chuẩn desktop (1440x900): Panel mở rộng đạt 486px (~54vh), hiển thị toàn bộ 11 dòng Run History rõ ràng; nút Collapse thu về 447px; chuyển sang tab "Overview & Groups" hoạt động chính xác; nút Close "X" đóng panel và mở lại campaign khác luôn ở trạng thái thu gọn mặc định.
    - Viewport thấp (1366x768): Panel mở rộng đạt 415px, Master Table co về 144px, `mainScrollHeight = mainClientHeight = 653px` (0px overflow, panel bottom cách mép dưới 19.3px, không bị clipping, không thanh cuộn kép).
    - Regression test: Action Menu Dashboard (`/`) drawer "Interview & Activity Timeline" nút Expand/Collapse vẫn hoạt động chính xác 100%.
  - `npm run build` (Turbopack) PASS 100% 31/31 routes (18.0s).
- ⚠️ Sai lệch so với spec:
  - Spec gốc yêu cầu: Áp dụng chiều cao `h-[clamp(420px,60vh,760px)]` cho panel.
  - Lỗi gặp phải: Trên màn hình 1366×768 (viewport height 768px), `60vh` (461px) kết hợp với filter bar và Master Table `max-h-64` (235px) làm tổng chiều cao (779px) vượt quá `main` clientHeight (653px), khiến đáy panel bị cắt ngoài màn hình khoảng 35-120px.
  - Giải pháp thay thế đã dùng: Áp dụng mục 6 tuỳ chọn của spec (Master Table thu về `shrink-0 max-h-36` khi expand) kết hợp tinh chỉnh clamp sang `h-[clamp(380px,54vh,760px)]`. Kết quả verify thực tế tại 768px: `mainScrollHeight` = `mainClientHeight` = 653px, đáy panel cách viền dưới 19.3px, không clipping, không tràn thanh cuộn.
  - Đã báo Claude/User: Có, ghi rõ trong báo cáo hoàn thành.

---

### [2026-09-15 01:30] Auto-clamp Warming "Max Joins Per Run" theo số account thật (Chống lặp lại sự cố timeout 55 phút VPS)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/campaign_actions.js`, `src/app/campaigns/page.js`, `src/app/api/qa-test/route.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **`src/app/campaign_actions.js`**:
     - Thêm hằng số `SAFE_TOTAL_WARM_ACTIONS = 20` (~2.5 phút/action, quỹ 50 phút trước ngưỡng timeout cứng 55 phút `BRIDGE_TIMEOUT_MS` của VPS bridge).
     - `_acquireWarmJoinRunLock`: Tính toán `clampedMaxPostsPerRun = Math.max(1, Math.min(requestedMax, Math.floor(SAFE_TOTAL_WARM_ACTIONS / safeAccountCount)))` theo số lượng `targetAccounts.length` thực tế đã resolve (sau khi đã lọc busy/disabled). Gán vào `result.maxPostsPerRun` và đính kèm `wasClampedForSafety: true`, `requestedMaxPostsPerRun: requestedMax` khi có clamp xảy ra.
     - `triggerWarmJoinRun`: Áp dụng cùng công thức clamp cho trường hợp override thủ công `maxGroupsPerAccount` dựa trên `(lockRes.accounts || []).length`, log `console.warn` cảnh báo khi clamp được kích hoạt.
  2. **`src/app/campaigns/page.js`**:
     - Thêm cảnh báo phụ trong modal "Confirm Warm & Join" (đặt dưới Dynamic Risk Indicator Badge): khi `activeCount * effectiveVal > 20`, hiển thị banner amber thông báo ước tính tổng thao tác và lưu ý hệ thống sẽ tự động giảm số lượng để tránh vượt timeout hạ tầng 55 phút.
  3. **`src/app/api/qa-test/route.js`**:
     - Bổ sung test suite `WARM-CLAMP` kiểm thử tự động với dữ liệu sandbox độc lập: xác minh 5 accounts với `max_posts_per_run=15` bị clamp xuống chính xác 4 (`wasClampedForSafety=true`), và 1 account với `max_posts_per_run=15` giữ nguyên 15 (`wasClampedForSafety=undefined`). Dọn dẹp sạch sẽ toàn bộ dữ liệu test sau khi chạy.
- Verify:
  - `node --check src/app/campaign_actions.js` PASS 100%.
  - `npm run build` (Turbopack) PASS 100% (31/31 routes).
  - `/api/qa-test` PASS 100% (6/6 tests gồm `DB-06/07`, `DB-11/12`, `DB-20`, `BIZ-13`, `SMOKE`, `WARM-CLAMP`).
  - `/api/biz-test` PASS 100% (27/27 tests gồm `BIZ-01..25`, `PERF-01..03`).
- ✅ Không có sai lệch so với spec.

---

### [2026-09-15 01:25] Enforce start_time/end_time Daily Window Cho Job Posting Auto-Scheduler
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/api/webhooks/auto-run-eligible-campaigns/route.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **`src/app/api/webhooks/auto-run-eligible-campaigns/route.js`**:
     - Thêm điều kiện lọc `AND (now() AT TIME ZONE 'Asia/Ho_Chi_Minh')::time BETWEEN c.start_time AND c.end_time` vào WHERE clause của câu SELECT `eligibleCampaigns`.
     - Đặt ngay sau điều kiện `end_date` (~dòng 46), trước `AND NOT EXISTS`.
     - Sử dụng `AT TIME ZONE 'Asia/Ho_Chi_Minh'` tường minh để đảm bảo đúng múi giờ Việt Nam bất kể múi giờ máy chủ/container.
  2. **Bảo toàn tính năng Run thủ công**:
     - Tuyệt đối không can thiệp vào `_executeCampaignRunInternal`, `computeCampaignDispatchPreview`, hay `triggerCampaignRun`. Người dùng bấm "Run" từ UI vẫn luôn hoạt động tức thì bất kể khung giờ cấu hình.
- Verify:
  - `node --check src/app/api/webhooks/auto-run-eligible-campaigns/route.js` PASS 100%.
  - `npm run build` (Turbopack) PASS 100% (31/31 routes).
  - Test trực tiếp qua route thật `GET /api/webhooks/auto-run-eligible-campaigns` với header `x-internal-secret` và dữ liệu độc lập trên schema `sandbox`:
    - Test Case 1: Cấu hình `start_time='08:00', end_time='20:00'`, thời điểm kiểm tra là 01:22 VN (ngoài khung giờ) -> Kết quả: `isEligible = false`, campaign bị loại trừ chính xác.
    - Test Case 2: Cấu hình `start_time='00:00', end_time='23:59'` (bao gồm thời điểm hiện tại) -> Kết quả: `isEligible = true`, count = 1, campaign xuất hiện chính xác.
    - Test Case 3: Cấu hình `start_time='00:00', end_time='00:05'` (khung giờ quá khứ trong ngày) -> Kết quả: `isEligible = false`, campaign bị loại trừ chính xác.
    - Dọn dẹp sạch sẽ toàn bộ dữ liệu test sau khi hoàn tất.
  - `/api/qa-test` (5/5 PASS 100%).
  - `/api/biz-test` (27/27 PASS 100%).
- ✅ Không có sai lệch so với spec.

---

### [2026-09-15 01:35] Unify "Max Posts Per Run" Cho Warming Campaign & Khôi Phục Per-Run Override (Round 1 & 2)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/campaign_actions.js`, `src/app/api/webhooks/warm-join-cron-register/route.js`, `src/app/components/CampaignEditModal.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  1. **`src/app/campaign_actions.js`**:
     - `_acquireWarmJoinRunLock`: Thêm `max_posts_per_run` vào câu SELECT campaign khi có `campaignId` cụ thể và câu SELECT pick oldest active Warming campaign khi cron (`campaignId=null`).
     - `_acquireWarmJoinRunLock`: Thêm field `maxPostsPerRun: campaign?.max_posts_per_run || 2` vào object `result` trả về.
     - `triggerWarmJoinRun`: Khôi phục `maxGroupsPerAccount` trong destructure `params` (không default cứng = 2), ưu tiên override khi gọi manual từ UI: `maxGroupsPerAccount != null ? Number(maxGroupsPerAccount) : (Number(lockRes.maxPostsPerRun) || 2)`.
  2. **`src/app/api/webhooks/warm-join-cron-register/route.js`**:
     - Thêm `maxGroupsPerAccount: lockRes.maxPostsPerRun` vào response JSON thành công để n8n Workflow C nhận và forward đúng.
  3. **`src/app/components/CampaignEditModal.js`**:
     - Bỏ điều kiện `isJobPosting ?` ép Warming luôn nhận `max_posts_per_run: 15` khi lưu payload — áp dụng `Math.min(15, Math.max(1, parseInt(maxPostsPerRun, 10) || 15))` cho cả 2 loại campaign.
     - Di chuyển input `Max Posts / Joins Per Run` ra khu vực chung (trước `Target FB Accounts Pool`), hiển thị cho cả Warming và Job Posting.
     - Cập nhật label & mô tả helper động:
       - Job Posting: Label "Max Posts Per Run", mô tả trần cứng 15, an toàn trước timeout hạ tầng 55 phút/lượt.
       - Warming: Label "Max Joins Per Run", mô tả trần cứng 15, mặc định 2 nếu không đổi.
- Verify:
  - `node --check` PASS 100% các file sửa đổi.
  - Next.js Turbopack `npm run build` PASS 100% 31/31 routes.
  - Thử nghiệm giao diện thật qua Chrome DevTools MCP: mở modal Warming campaign -> thay đổi Max Joins Per Run thành 7 -> click "Update Campaign" -> DB xác nhận lưu `max_posts_per_run = 7` -> mở lại modal xác nhận input load đúng `7` -> bấm Run chọn override `4 group(s)` -> modal feedback & Server Action dispatch payload đúng `maxGroupsPerAccount: 4`.
  - API regression tests: `/api/qa-test` (5/5 PASS), `/api/biz-test` (27/27 PASS).
- ✅ Không có sai lệch so với spec.

---

### [2026-09-15 01:10] Unified Campaign Stop Conditions, Per-Nick Circuit Breaker, Configurable Batch Size (1..15) (Spec 2026-09-15)
- Viết bởi: Antigravity (Implementer)
- Commit: uncommitted (chờ Claude review & commit theo Rule 10.9)
- Files: `src/app/campaign_actions.js`, `src/app/api/webhooks/auto-run-eligible-campaigns/route.js`, `src/app/api/webhooks/campaign-data/route.js`, `src/app/api/webhooks/warm-join-run-callback/route.js`, `src/app/campaigns/page.js`, `src/app/components/CampaignEditModal.js`, `src/app/components/CampaignDispatchPreviewModal.js`, `docs/DEVELOPMENT_LOG.md`, `docs/architecture/ATS_3.0_UI_Modernization_Blueprint.md`
- Nội dung:
  1. **PHẦN A (DB Migration)**: Đã migrate DDL trên cả 2 schema `public` và `sandbox` (Supabase Singapore `mock-supabase-project`): `ALTER TABLE campaigns ADD COLUMN max_posts_per_run integer NOT NULL DEFAULT 15 CHECK (max_posts_per_run > 0 AND max_posts_per_run <= 15);`.
  2. **PHẦN B (Backend & Circuit Breaker `campaign_actions.js`)**:
     - Duy trì static imports nguyên bản: `import { revalidatePath } from 'next/cache';` và `import { after } from 'next/server';` (không dynamic import fallback).
     - Thêm internal helpers `_getDisabledAccountIdsForJobCampaign` (≥3 `Failed` run items trong campaign) và `_getDisabledAccountIdsForWarmingCampaign` (≥3 `Failed` items trong warm join runs).
     - Tích hợp loại trừ disabled accounts trong `_getEligibilityState` (tự động áp dụng cho preview và execution).
     - Job Posting: Quota động = toàn bộ Target Pool (`COUNT DISTINCT campaign_social_groups.social_group_id`); dispatch limit = `Math.min(quotaRemaining, max_posts_per_run)` với `SAFE_DISPATCH_BATCH_SIZE_MAX = 15`.
     - Tự động hoàn thành Job Posting (`checkCampaignAutoCompletion`): kiểm tra `end_date` trước (`completedReason: 'end_date'`); sau đó kiểm tra pool coverage `totalProcessed >= totalTargetPool` (`completedReason: 'quota'`).
     - Tự động hoàn thành Warming (`checkWarmingCampaignAutoCompletion`): kiểm tra `end_date` trước; sau đó kiểm tra remaining unjoined groups cho active non-disabled accounts (`remaining_work === 0` -> `completedReason: 'full_coverage'`). Guard không auto-complete campaign rỗng/chưa gán target groups hoặc accounts.
     - Gọi `checkWarmingCampaignAutoCompletion` trong `warm-join-run-callback/route.js` khi finalize run.
     - Khắc phục Warming cron starvation trong `_acquireWarmJoinRunLock`: khi `campaignId` null, ưu tiên campaign có `MAX(completed_at) ASC NULLS FIRST` và filter `(end_date IS NULL OR end_date >= CURRENT_DATE)`. Đồng thời loại trừ disabled warming accounts.
     - Cập nhật `createCampaign`, `updateCampaign`, `getCampaigns`, `getCampaignDetail` xử lý `max_posts_per_run` (clamp 1..15) và trả về `totalGroupsInPool`.
  3. **PHẦN C (UI `CampaignEditModal.js`)**:
     - Thay input `targetQuota` bằng `maxPostsPerRun` (min 1, max 15, default 15) kèm mô tả an toàn timeout 55 phút.
     - Cập nhật mô tả Auto-Scheduler checkbox.
  4. **PHẦN D (UI `page.js` & `CampaignDispatchPreviewModal.js`)**:
     - Overview Panel hiển thị "Target Pool (toàn bộ nhóm mục tiêu)" với `totalGroupsInPool`.
     - Stats bar trong Preview Modal đổi label thành "Dispatched This Run".
     - Webhook routes `auto-run-eligible-campaigns` & `campaign-data` chọn `max_posts_per_run` và kiểm tra pool completion.
- Verify:
  - `node --check` PASS 100% 7 files.
  - Turbopack `npm run build` PASS 100% 31/31 routes.
  - Sandbox test suite (`scratch/test_stop_conditions_and_circuit_breaker.mjs`): 6/6 test cases PASS (Job posting pool batching, Per-nick circuit breaker, Warming auto-completion, Warming cron rotation, End date stop condition, `max_posts_per_run` clamp & DB check constraint).
  - API regression test: `/api/qa-test` (5/5 PASS), `/api/biz-test` (28/28 PASS).
- ✅ Không có sai lệch so với spec.

---

### [2026-09-15 00:10] Dùng after() (next/server) Cho Webhook Dispatch Trong _executeCampaignRunInternal & triggerWarmJoinRun (FIX_SPEC_2026-09-15)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/campaign_actions.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  - **Mục đích**: Khắc phục sự cố campaign dispatch webhook n8n bị rớt ngẫu nhiên trên Vercel Serverless Function (sự cố sản xuất campaign "WINPRO - 3 JOBS" `public` id `01a09fcb-4715-70ef-9729-c15b0fb53105` bị kẹt ở status `Running` do Serverless Function trả response và đóng băng container trước khi unawaited `fetch()` kịp gửi ra khỏi server).
  - **Giải pháp**:
    - `src/app/campaign_actions.js`: Import `after` từ `next/server`.
    - **Chỗ 1 — trong `_executeCampaignRunInternal`**: Trích xuất `process.env.N8N_CAMPAIGN_TRIGGER_WEBHOOK_URL`, `process.env.INTERNAL_WEBHOOK_SECRET`, `process.env.DB_SCHEMA` ra biến cục bộ trước callback. Bọc toàn bộ khối `fetch()` gọi webhook n8n vào `after(async () => { ... })` (`waitUntil` trên Vercel). Thêm `signal: AbortSignal.timeout(8000)` chống treo vô thời hạn. Giữ nguyên `try...catch` bên trong callback để log cảnh báo và không làm ảnh hưởng response gốc.
    - **Chỗ 2 — trong `triggerWarmJoinRun`**: Áp dụng đúng pattern y hệt Chỗ 1: trích xuất các biến môi trường ra biến cục bộ, bọc khối `fetch()` vào `after(async () => { ... })`, bổ sung `signal: AbortSignal.timeout(8000)`, và bảo toàn `try...catch` xử lý lỗi.
- Verify:
  - `node --check src/app/campaign_actions.js` -> PASS 100%.
  - Turbopack `npm run build` -> Compiled successfully in 30.0s, 31/31 routes static page generation PASS 100%.
  - Trigger test thật trong sandbox UI (`Test Job posting`): Server Action dispatch thành công, tạo `campaign_runs` id `01a0a0e4-c52b-7ea0-909c-e85f21c3dbee`, UI cập nhật "Running" và "Just now", sau đó reset test data về Ready.
  - `/api/biz-test` (27 tests) + `/api/qa-test` (5 tests) -> PASSED 100% (0 failures).
- ✅ Không có sai lệch so với spec.

---

### [2026-09-14 23:45] Search Column Responsive Fix & Campaigns Detail Panel Tablet Audit (Spec 2026-09-14 Batch 2)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/search/page.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  - **Việc A (FIX_SPEC - Search Column Responsive Fix)**:
    - Sửa `src/app/search/page.js` (dòng 623): Xoá `max-w-[260px]` khỏi container badge social, giữ nguyên `flex flex-wrap items-center gap-1.5`. Giúp container tự nhiên tận dụng độ rộng cột `<td>` (đã có `min-w-[260px]` ở `<th>`).
    - Verify trên Chrome DevTools MCP:
      - Ứng viên có ≥3 social profiles (Tran Duc Anh: LinkedIn, GitHub, Facebook) trên desktop (1440px): 3 badge nằm thẳng hàng trên 1 dòng duy nhất (`top: 134.65px`), chiều cao hàng chuẩn 31px (khớp đồng đều với các hàng 0, 1, 2 profiles).
      - Khi thu hẹp về 768px tablet (`tdWidth: 260px`): badges tự động wrap sang dòng 2 một cách tự nhiên chỉ khi tổng chiều rộng badge vượt quá kích thước thực tế của cột.
  - **Việc B (QA SPEC - Campaigns Detail Panel Tablet Audit)**:
    - Audit tablet 4 mốc độ rộng chuẩn qua Chrome DevTools MCP trên dev server thực tế:
      - **1440px**: `scrollWidth: 1440` vs `clientWidth: 1440` (khớp 100%), 0 console error.
      - **1024px**: `scrollWidth: 1024` vs `clientWidth: 1024` (khớp 100%), 0 console error.
      - **834px**: `scrollWidth: 834` vs `clientWidth: 834` (khớp 100%), 0 console error.
      - **768px**: `scrollWidth: 768` vs `clientWidth: 768` (khớp 100%), 0 console error.
    - Test chi tiết 2 Popover mới ở 768px tablet (hàng gần đáy bảng):
      - **Group Type Tag Editor**: Cuộn tới đáy bảng Target Groups -> click trigger -> Popover portal ra `document.body`, tự động lật `data-side="top"` (tránh tràn đáy màn hình, `top: 540px`, `bottom: 905px`), hiển thị đầy đủ không bị che khuất; click ngoài đóng popover chính xác.
      - **Join Status Accounts Breakdown**: Cuộn tới đáy bảng -> click ratio badge `0/2` -> Popover portal ra `document.body` (`data-align="end"`), nằm gọn trong viewport (`bottom: 1012px` <= `1024px`), hiển thị danh sách accounts chi tiết; click ngoài đóng popover chính xác.
    - Không phát hiện bất kỳ lỗi/regression nào trong suốt quá trình audit.
- Verify:
  - `npm run build` -> Compiled successfully in 17.2s, 31/31 routes static generation PASS 100%.
- ✅ Không có sai lệch so với spec.

### [2026-09-14 23:15] Batch Fix: Blacklist Sync, Job Branch Location, Portal Popovers & Unified Contacts (FIX_SPEC_2026-09-14 - Round 1 & 2)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/actions.js`, `src/app/candidates/page.js`, `src/app/search/page.js`, `src/app/jobs/page.js`, `src/app/components/JoinStatusBadge.js`, `src/app/components/GroupTypeTagEditor.js`, `src/app/hitl_actions.js`, `src/app/api/biz-test/route.js`
- Nội dung:
  - **Hạng mục 1: Bỏ Blacklist rồi nhưng Search vẫn hiện "Black List"**:
    - `src/app/actions.js` (`updateCandidateProfile`): Tự động đặt `blacklist_note = ''` khi `blocked` chuyển sang `false`.
    - `src/app/candidates/page.js`: Khi uncheck checkbox "Blacklisted", tự động đặt `formData.blacklist_note = ''`.
    - `src/app/search/page.js`: Sửa biến `isBlocked = !!c.blocked`, tách biệt hoàn toàn khỏi sự tồn tại của `c.blacklist_note`.
  - **Hạng mục 2: Đổi địa chỉ Branch thì Job Location tự động ăn theo (`branch_id` Dynamic Lookup)**:
    - Chạy migration DDL thêm cột `branch_id text` vào bảng `jobs` ở cả 2 schema `sandbox` và `public` trên Supabase (project `mock-supabase-project`).
    - `src/app/actions.js`: Bổ sung `branch_id` vào whitelist trường được phép sửa trong `updateJobField`, bổ sung `branch_id` vào SELECT và GROUP BY của `getClientWorkbenchData` và `getJobs`.
    - `src/app/jobs/page.js`: Thêm helper `getJobLocationDisplay(job, branches)` tự động tra cứu địa chỉ mới nhất của chi nhánh nếu có `job.branch_id`; thêm handler `handleUpdateJobFields` cập nhật nguyên tử `location` và `branch_id`; sửa Popover chọn Location lưu `branch_id` tương ứng với HQ/Branch hoặc `null` với custom address/Clear; hiển thị địa chỉ resolved ở bảng Job Orders.
  - **Hạng mục 3: Popover bị cắt cụt do container cha `overflow-hidden` & Chống Double-Toggle (Round 2)**:
    - Thay thế toàn bộ popup inline custom `absolute/fixed` bằng portal-based Popover từ `src/components/ui/popover.jsx` (dựa trên `@base-ui/react/popover`) giúp render ra `document.body` thoát khỏi khung chứa bị `overflow: hidden`.
    - `src/app/components/JoinStatusBadge.js`: Accounts Breakdown Popover & Admin Question Popover. Xoá bỏ hoàn toàn các dòng tự toggle `setShow...Popover(!...)` trong `PopoverTrigger onClick`, để `Popover` tự quản lý state mở/đóng một chiều qua `open`/`onOpenChange`, chỉ giữ `e.stopPropagation()` và mutual exclusion.
    - `src/app/components/GroupTypeTagEditor.js`: Filter Rename Popover & Badge Tag Editor Dialog.
  - **Hạng mục 4: Thống nhất logic tổng hợp `candidate.phones` / `candidate.emails`**:
    - `src/app/actions.js`: Export `syncCandidateAggregatedContacts(candidateId, dbClient = sql)` hỗ trợ db client tuỳ chọn (phục vụ transaction), tái cấu trúc `addContactPoint` và `updateContactPoint` gọi trực tiếp hàm này.
    - `src/app/hitl_actions.js`: Import và tái cấu trúc khối MERGE gọi `syncCandidateAggregatedContacts(targetId, tx)` để bảo toàn logic dedup, chuẩn hoá số điện thoại (`normalizeContactValue`) và ưu tiên contact chính (`is_primary`/active).
  - **Kiểm thử tích hợp & Hồi quy (Round 2)**:
    - Bổ sung `BIZ-24`: Verify uncheck Blacklisted profile -> `blacklist_note` tự động reset về rỗng `''` trong DB.
    - Bổ sung `BIZ-25`: Verify tạo Client -> Add Branch -> Gán Job `branch_id` -> Cập nhật địa chỉ Branch -> `branch_id` round-trip chính xác và data đồng bộ.
- Verify:
  - `node --check` toàn bộ các files modified -> PASS 100%.
  - Turbopack `npm run build` -> Compiled successfully in 21.4s, 31/31 routes static page generation PASS 100%.
  - `/api/biz-test` -> Toàn bộ 28 tests (BIZ-01..25, PERF-01..03) PASSED 100% (0 failures).
- ✅ Không có sai lệch so với spec.

---

### [2026-09-13 23:25] Dùng after() (next/server) cho webhook dispatch trong syncCandidatesToGoogleContacts (FIX_SPEC_2026-09-13)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/actions.js`
- Nội dung:
  - **Mục đích**: Khắc phục lỗi 80% lần dispatch "Save to Google Contacts" bị rớt ngẫu nhiên trên Vercel Serverless Function do container đóng băng/ngắt ngay sau khi hàm `return` trước khi `fetch()` không `await` hoàn tất.
  - **Giải pháp**:
    - Import `after` từ `next/server`.
    - Trong hàm `syncCandidatesToGoogleContacts`, bọc logic `fetch()` webhook n8n vào callback `after(async () => { ... })` (sử dụng `waitUntil` dưới nền trên Vercel) để đảm bảo invocation kéo dài cho đến khi promise gửi request hoàn tất mà không chặn phản hồi trả về cho User.
    - Đọc các biến môi trường (`N8N_GOOGLE_CONTACTS_SYNC_WEBHOOK_URL`, `INTERNAL_WEBHOOK_SECRET`, `DB_SCHEMA`) ra biến cục bộ trước khi truyền vào closure.
    - Thêm `signal: AbortSignal.timeout(8000)` bảo vệ tài nguyên serverless không bị treo vô thời hạn nếu VPS n8n gặp sự cố.
    - Giữ nguyên `try...catch` bên trong callback để log cảnh báo và không làm ảnh hưởng response gốc.
- Verify:
  - `node --check src/app/actions.js` -> PASS 100%.
  - `git diff --stat` -> `src/app/actions.js` (21 lines changed: 16 insertions(+), 5 deletions(-)).
  - `npm run build` (Turbopack) -> Compiled successfully in 44s, 31/31 routes static page generation PASS 100%.
- ✅ Không có sai lệch so với spec.

---

### [2026-09-13 21:05] Chỉ cho phép Delete CV version, bỏ hẳn Add/Edit trực tiếp trên Candidate 360 (FIX_SPEC_2026-09-13-e)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/actions.js`, `src/app/candidates/page.js`, `.env.local`
- Nội dung:
  - **Mục đích**: Rút gọn tính năng Add/Edit CV version trực tiếp trên Candidate 360 (từ SNAP-162) theo quyết định của PO, sau khi test thật phát hiện rủi ro bảo mật/dữ liệu: người dùng có thể dán nhầm link CV của candidate khác mà không có bước đối chiếu/xác nhận. Từ nay CV mới CHỈ được nạp qua luồng "+ Parse CV (AI)" đi qua hàng đợi HITL review. Candidate 360 CHỈ còn giữ quyền Delete 1 version cụ thể.
  - **Server Actions (`src/app/actions.js`)**:
    - Xoá hoàn toàn 3 hàm: `appendCvVersion(...)`, `updateCvVersion(...)`, `triggerCvReparse(...)`.
    - Giữ nguyên `deleteCvVersion(candidateId, index)` để phục vụ việc xoá version không cần thiết.
  - **Candidate 360 UI (`src/app/candidates/page.js`)**:
    - Xoá import 3 hàm đã bỏ khỏi `../actions`.
    - Xoá 5 state: `showAppendCvModal`, `appendCvUrl`, `appendCvFilename`, `editingCvVersionIndex`, `isAppendingCv`.
    - Xoá hàm `handleAppendCvVersion`.
    - Sửa `handleSaveProfile`: bỏ biến `oldCvUrl` và khối gọi `triggerCvReparse`.
    - Khoá ô input "CV Link" trong Personal Information thành `readOnly` + `disabled`, xoá nút "Add Version", cập nhật ghi chú hướng dẫn dùng luồng "+ Parse CV (AI)".
    - Cập nhật empty state trong Embedded CV Viewer: xoá nút "Add CV Version", thay bằng hướng dẫn trỏ tới nút "+ Parse CV (AI)".
    - Cập nhật toolbar Embedded CV Viewer: xoá nút "Edit" và nút "Add Version", chỉ giữ nút "Delete" (cho version trong mảng `cv_urls`) và "Open in New Tab".
    - Xoá toàn bộ modal markup "Append New CV Version Modal".
  - **Cấu hình môi trường (`.env.local`)**:
    - Xoá biến môi trường `N8N_CV_REPARSE_WEBHOOK_URL` không còn sử dụng.
- Verify:
  - `git grep -E "appendCvVersion|updateCvVersion|triggerCvReparse" src/` -> 0 kết quả (PASS 100%).
  - `node --check src/app/actions.js src/app/candidates/page.js` -> PASS 100%.
  - `npm run build` (Turbopack) -> Compiled successfully in 31.2s, 31/31 routes static page generation PASS 100%.
- ✅ Không có sai lệch so với spec.

---

### [2026-09-13 20:05] Edit & Delete từng CV version cụ thể (Candidate 360°)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/actions.js`, `src/app/candidates/page.js`
- Nội dung:
  - **Server Actions (`src/app/actions.js`)**:
    - Thêm `updateCvVersion(candidateId, index, { url, filename })` cập nhật url và filename tại index cụ thể trong mảng `cv_urls`, đồng bộ `cv_url` nếu là latest version.
    - Thêm `deleteCvVersion(candidateId, index)` xoá phần tử tại index khỏi mảng `cv_urls`, chặn xoá nếu `cv_urls.length <= 1`, cập nhật lại `cv_url` trỏ về version cuối cùng nếu version bị xoá trùng với `cv_url`.
  - **Candidate 360 UI (`src/app/candidates/page.js`)**:
    - Import `updateCvVersion` và `deleteCvVersion`.
    - Thêm state `editingCvVersionIndex` (null cho Add mode, number cho Edit mode).
    - Cập nhật `allCvUrls` bổ sung thuộc tính `arrayIndex` (index trong mảng `cv_urls` hoặc null) và `filename`.
    - Cập nhật `handleAppendCvVersion` gọi `updateCvVersion` khi ở Edit mode (và không kích hoạt re-parse), gọi `appendCvVersion` khi ở Add mode.
    - Thêm handler `handleDeleteCvVersion` xác nhận trước khi xoá và cập nhật lại danh sách.
    - Thêm nút Edit (`Pencil`) và Delete (`Trash2`) cạnh dropdown chọn CV version, chỉ hiển thị với version có `arrayIndex !== null`.
    - Cập nhật modal Add/Edit CV Version hỗ trợ động tiêu đề ("Edit CV Version" / "Add New CV Version"), nút submit ("Save Changes" / "Append Version"), và reset `editingCvVersionIndex` khi đóng/hủy modal.
- Verify:
  - `node --check src/app/actions.js src/app/candidates/page.js` PASS 100%.
  - Next.js Turbopack `npm run build` PASS 100% 31/31 routes.
- ✅ Không có sai lệch so với spec.
...
### [2026-09-13 19:30] Tự động re-parse CV khi cập nhật (qua HITL) + Báo lỗi rõ ràng (Spec #14)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/actions.js`, `src/app/candidates/page.js`, `.env.local`
- Nội dung:
  - **Server Action (`src/app/actions.js`)**: Thêm `triggerCvReparse(candidateId)` bọc security guard `assertRealRequestContext`, trích xuất `driveFileId` từ `cv_url`, tạo persistent notification khởi tạo (`cv_reparse`, severity `info`), định tuyến `baseUrl` sandbox/production, và bắn webhook n8n tới `N8N_CV_REPARSE_WEBHOOK_URL`.
  - **Candidate 360 UI (`src/app/candidates/page.js`)**:
    - Import `triggerCvReparse` từ `../actions`.
    - Trong `handleAppendCvVersion`: tự động gọi `triggerCvReparse(candidate.id)` ngay sau khi append version thành công và nạp lại dữ liệu.
    - Trong `handleSaveProfile`: lưu `oldCvUrl`, tự động gọi `triggerCvReparse(candidate.id)` khi `formData.cv_url` thực sự thay đổi so với `oldCvUrl` và có giá trị mới.
  - **n8n Workflow ("A5: CV Re-parse (Existing Candidate)" - ID `8sdwRFG354PTL8Oj`)**:
    - Tạo workflow mới trong thư mục "ATS 3.0" (`y4ZDeiEYOE0cUDFZ`) với 8 nodes: `Webhook: CV Reparse Trigger` (headerAuth `Je1dHcRXyZhrXODl`), `Prepare Batch Context`, `Create Batch Record`, `Insert Batch Item`, `Build SubWorkflow Input`, `Process Single Item` (`WfSingle00000001`), `Build Reparse Notification`, `POST Final Notification`.
    - Publish active version `b0cf34ac-7158-4532-bc00-819ab7471fe8`.
    - Cấu hình notification phân loại rõ ràng theo `match_status`: `UPDATE` (success, có bản cập nhật cần duyệt trong Notification Center), `ERROR`/fail (error, báo lỗi để nạp lại file), `CONFLICT` (warning), `NEW` (warning).
  - **Môi trường**: Bổ sung `N8N_CV_REPARSE_WEBHOOK_URL=https://n8n.example.com/webhook/cv-reparse-trigger` vào `.env.local`.
- Verify:
  - `node --check src/app/actions.js src/app/candidates/page.js` PASS 100%.
  - `n8n validate_workflow` & `get_workflow_details` xác nhận workflow 8 nodes active trên VPS.
  - Next.js Turbopack `npm run build` PASS 100% 31/31 routes.
- ✅ Không có sai lệch so với spec.

---

### [2026-09-13 19:25] Click Notification dẫn thẳng tới nơi cần điền (mục #8) & Focus / Ghim Social Group trên Library
| **`SNAP-20260913-159`** | 13/09/2026 19:15 | `v3.0-RC187` | 📐🎛️ **4 Fix Độc Lập: Campaign Details Layout, Timeline Follow Status Khi Inline Edit, Planning Date Default & Dedup Contact Point**: (1) **Campaign Details Layout (`src/app/campaigns/page.js`)**: Sửa `overflow-hidden` → `overflow-x-hidden` trên Master Campaigns Table để loại bỏ xung đột với `overflow-y-auto`; giới hạn chiều cao cột Linked Job `max-h-16 overflow-y-auto`; đặt sàn chiều cao tối thiểu cho Detail Expandable Panel `min-h-[360px]`; (2) **Timeline Follow Status (`src/app/page.js`)**: Cập nhật `handleInlineUpdate` luôn gọi `selectRow({ application_id: applicationId })` khi đổi status ("theo con trỏ") bất kể dòng có đang được chọn hay không; (3) **Planning Date Default (`src/app/actions.js`)**: Bổ sung `planning_date` với giá trị mặc định `CURRENT_DATE` trong câu INSERT activity của `createCandidateWithApplication()`; (4) **Dedup Contact Point**: Đưa helper chuẩn hoá `normalizeContactValue` vào `src/lib/utils.js` (tránh lỗi Next.js Server Actions cấm export sync helper từ file `'use server'`), import và chuẩn hoá 2 phía trong `src/app/hitl_actions.js`, mở rộng danh sách `checkValues` bắt các biến thể định dạng SĐT (`0xxx`, `+84xxx`, `84xxx`) trong `cv-import/route.js`; (5) **Verify**: `node --check` PASS 100%, Turbopack `npm run build` PASS 100% 31/31 routes. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260913-158`** | 13/09/2026 19:00 | `v3.0-RC186` | 📄✨ **Kích Hoạt Dropdown Chọn Version CV Cũ (Embedded CV Viewer) Trong Candidate 360° (`src/app/candidates/page.js`)**: Bổ sung gán trường `cv_urls: cand.cv_urls || []` vào `setFormData` trong hàm `loadCandidateData()`, khai thông luồng nạp mảng `cv_urls` vào state `formData`, giúp useMemo `allCvUrls` và dropdown chọn CV version hiển thị đầy đủ danh sách các version CV đã lưu; `node --check` PASS 100%, Turbopack `npm run build` PASS 100% 31/31 routes. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260913-157`** | 13/09/2026 18:55 | `v3.0-RC185` | 📱🏢 **Header Responsive (CRM-ATS 3.0 Text Hidden on Mobile) & Lọc Trùng Địa Chỉ HQ Trong Dropdown Job Location**: (1) **Header Responsive (`src/app/layout.js`)**: Thêm class `hidden md:inline` vào `<span>CRM-ATS 3.0</span>` để ẩn chữ ở màn hình hẹp, chỉ hiển thị từ breakpoint `md` trở lên, icon "ATS" luôn hiển thị; (2) **Deduplicate Job Location Dropdown (`src/app/jobs/page.js`)**: Cập nhật bộ lọc `.filter()` của `clientForm.branches` loại bỏ branch có địa chỉ trùng với `clientForm.address` (Main Headquarters cấp Client), ngăn chặn hiển thị trùng 2 dòng "Main Headquarters" khi branch bị un-set HQ; (3) **Verify**: `node --check` PASS 100%, `git diff` khớp 100% spec, Turbopack `npm run build` PASS 100% 31/31 routes. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260913-156`** | 13/09/2026 18:23 | `v3.0-RC184` | 📇✨ **Bỏ Dấu "+" Thừa Trong Nút "Assign to Job" (Candidate 360°)**: Bỏ ký tự "+" literal trong thẻ `<span>` của nút "Assign to Job" tại `src/app/candidates/page.js` (dòng 1200), giữ lại icon `<Plus size={13} />`, đồng bộ hoàn toàn với pattern của nút "Assign to Job Order" (`src/app/candidates/page.js` dòng 1661); `node --check` PASS 100%, Next.js Turbopack `npm run build` PASS 100% 31/31 routes. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260913-155`** | 13/09/2026 18:18 | `v3.0-RC183` | 📇✨ **Đổi Tên "LinkedIn Job Post" → "LinkedIn Jobs" (`src/constants/enums.js`)**: Cập nhật giá trị chuỗi của `LINKEDIN_JOB_POST` và phần tử tương ứng trong `SOURCE_CHANNELS_LIST` từ `"LinkedIn Job Post"` sang `"LinkedIn Jobs"` để đồng bộ với tên gọi phổ biến thực tế trên production (74 dòng "Linkedin Jobs"); giữ nguyên key `LINKEDIN_JOB_POST`, vị trí trong danh sách và 10 giá trị còn lại; `node --check` PASS 100%, Next.js Turbopack `npm run build` PASS 100% 31/31 routes. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260913-154`** | 13/09/2026 18:12 | `v3.0-RC182` | 📇✨ **Chuẩn Hoá Danh Sách Source Channels (`src/constants/enums.js`)**: Cập nhật `SOURCE_CHANNELS` và `SOURCE_CHANNELS_LIST` với 11 kênh tuyển dụng chuẩn hoá theo audit thực tế trên production: bổ sung `ITVIEC` ("ITViec"), tách riêng `LINKEDIN_JOB_POST` ("LinkedIn Job Post") khỏi `LINKEDIN` ("LinkedIn"), rút gọn `FACEBOOK_GROUP` -> `FACEBOOK` ("Facebook"), `REFERRAL` -> "Referral", `DIRECT_SOURCING` -> "Direct Sourcing"; `node --check` PASS 100%, Next.js Turbopack `npm run build` PASS 100% 31/31 routes. Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260913-153`** | 13/09/2026 18:05 | `v3.0-RC181` | 🎛️📅 **Action Menu: Chuyển Đổi Candidate Source Sang Dropdown Chặt (9 Lựa Chọn) & Sắp Xếp Mặc Định Planning Date Mới Nhất (DESC)**: (1) **Candidate Source Dropdown (`src/app/page.js`)**: Thay ô input free-text bằng `<select>` chuẩn với 9 lựa chọn từ `SOURCE_CHANNELS_LIST`, bổ sung nhánh fallback `(legacy)` cho dữ liệu cũ ngoài danh mục, xoá bỏ `<datalist id="source-channels-list">` không còn dùng; (2) **Default & Auto Re-sort Planning Date (`src/app/page.js`)**: Khởi tạo state `planningDateSort = 'desc'` (ưu tiên Planning Date mới nhất trước khi load trang), bổ sung nhánh `else if (field === 'planning_date')` trong `handleInlineUpdate` tự động sắp xếp lại danh sách cục bộ theo chiều sort hiện tại (`NULLS LAST`) ngay khi recruiter chỉnh sửa ngày mà không cần bấm lại icon sort; (3) **Verify**: `node --check src/app/page.js` PASS 100%, Next.js Turbopack `npm run build` PASS 100% 31/31 routes (4.2s, 0 errors). Theo Implementation Spec từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260913-152`** | 13/09/2026 17:42 | `v3.0-RC180` | 🌐🔀 **Environment-Aware Routing Cho Workflow C (FB Auto-Warm & Group Auto-Joiner — Nhánh Webhook)**: (1) **`src/app/campaign_actions.js` (`triggerWarmJoinRun`)**: Bổ sung trường `environment: process.env.DB_SCHEMA || 'public'` vào payload dispatch gửi n8n webhook; (2) **`src/app/api/webhooks/warm-join-run-callback/route.js` & `warm-join-run-progress/route.js`**: Import `sqlSandbox`, định tuyến `sqlClient = body.environment === 'sandbox' ? sqlSandbox : sql`, thực thi `sqlClient.begin(...)` bảo toàn schema isolation; (3) **n8n Workflow C (`L8QdckqW7FDwanRq`)**: Cập nhật 5 Code nodes (`Validate Internal Secret`, `Prepare Cron Context` [hardcode `environment: 'public'`], `Smart Group Allocator & Dispatcher`, `Process Bridge Warm Results`, `Build Final Warm Run Summary`), publish active version `e8875941-4b87-410e-b874-e6fd7ef09946`; (4) **Verify**: `node --check` PASS 100% 3 files, `npm run build` PASS 100% 31/31 routes Turbopack, n8n `get_workflow_details` xác nhận 5 node cập nhật và active version đồng bộ 100%. Theo Implementation Spec Giai đoạn 2b từ Claude (Architect/QA). | ✅ Stable | Repo + n8n |
| **`SNAP-20260913-151`** | 13/09/2026 16:05 | `v3.0-RC179` | 🌐🔀 **Environment-Aware Routing Cho Google Contacts Sync (Next.js Dispatch, Notifications Webhook & n8n Workflow A2)**: (1) **`src/app/actions.js` (`syncCandidatesToGoogleContacts`)**: Bổ sung trường `environment: process.env.DB_SCHEMA || 'public'` vào body payload gửi n8n webhook; (2) **`src/app/api/webhooks/notifications/route.js`**: Xác định `targetSchema = body.environment === 'sandbox' ? 'sandbox' : 'public'`, hỗ trợ ghi động `${sql(targetSchema)}.notifications` cho cả UPDATE và INSERT, bổ sung log schema; (3) **n8n Workflow A2 (`O659fZyN2uGaaLyL`)**: Cập nhật 3 Code node (`Build Create Payload`, `Build Update Payload`, `Build Conflict Message`) trích xuất `environment` từ `Save Contact Webhook` và truyền vào `notificationPayload`, publish active version `5bb2fa7c-0a69-4cf0-b1a0-dce7b708238c`; (4) **Verify**: `node --check` PASS 100% 2 files, `npm run build` PASS 100% 31/31 routes Turbopack, n8n active version verify PASS 100%. Theo Implementation Spec Giai đoạn 1 từ Claude (Architect/QA). | ✅ Stable | Repo + n8n |
| **`SNAP-20260913-150`** | 13/09/2026 14:48 | `v3.0-RC178` | 🔔📇 **Bổ Sung Persistent Notification "Đã Gửi Yêu Cầu" Ngay Khi Dispatch Google Contacts Sync (`src/app/actions.js`)**: (1) Thêm câu lệnh `INSERT INTO notifications` ngay trước khối dispatch webhook n8n trong `syncCandidatesToGoogleContacts()`, ghi nhận thông báo loại `google_contacts_sync` severity `info` kèm danh sách tên ứng viên (rút gọn nếu >3 tên) và metadata `{ candidateIds, action: 'dispatched' }`; (2) Tách biệt việc ghi nhận ý định và kết quả dispatch webhook, đảm bảo Notification Center luôn lưu vết yêu cầu đồng bộ tức thì; (3) Verify: `node --check src/app/actions.js` PASS 100%, `npm run build` PASS 100% 31/31 routes Turbopack. Theo yêu cầu bổ sung từ Claude (Architect/QA). | ✅ Stable | Repo |
| **`SNAP-20260913-149`** | 13/09/2026 14:25 | `v3.0-RC177` | 📇🔄 **Tích Hợp Google Contacts Sync (Thay Thế Workflow Notion Legacy) — Server Action, Single/Bulk UI Sync & Cập Nhật Workflow n8n ATS 3.0**: (1) **Server Action (`src/app/actions.js`)**: Thêm `assertRealRequestContext` security guard và hàm `syncCandidatesToGoogleContacts(candidateIds)` truy vấn candidate + contact points từ Postgres và dispatch sang `N8N_GOOGLE_CONTACTS_SYNC_WEBHOOK_URL` kèm header `x-internal-secret`; (2) **Candidate 360 UI (`src/app/candidates/page.js`)**: Thêm nút "Save to Google Contacts" với icon `Contact` bên cạnh nút "+ Parse CV (AI)"; (3) **Search Menu UI (`src/app/search/page.js`)**: Thêm `selectedCandidateIds` state, banner bulk action ("Save to Google Contacts (N)") khi có candidate được chọn, checkbox trên header `<th>` và row `<td>` cho riêng tab Candidate; (4) **n8n Workflow (`O659fZyN2uGaaLyL`)**: Cập nhật thành "A2: Save Contacts → Google Sync (ATS 3.0)", thêm HeaderAuth `Je1dHcRXyZhrXODl`, gỡ 3 Notion nodes & 3 Telegram nodes, tách `Get All Google Contacts` trước loop, xử lý lặp batch candidates qua `Loop: Candidate Batch` (`splitInBatches` v3), cập nhật `Normalize Contacts`, `Match Logic`, `Route by Match Count`, `Build Create/Update/Conflict Payload` thay Notion bằng ATS 3.0 URLs và gửi thông báo qua 3 node HTTP Request tới `/api/webhooks/notifications`; (5) **Verify**: `node --check` PASS 100% 3 files, Next.js Turbopack `npm run build` PASS 100% 31/31 routes. Theo `FIX_SPEC_2026-09-13-d`. | ✅ Stable | Repo + n8n |
| **`SNAP-20260913-148`** | 13/09/2026 13:50 | `v3.0-RC176` | 🔒🛡️ **Rotate Secret `BRIDGE_INTERNAL_SECRET` Thật Trên VPS Production TinoHost**: (1) Sinh secret 32-byte hex ngẫu nhiên mới qua `openssl rand -hex 32` trực tiếp trên VPS; (2) Cập nhật dòng `BRIDGE_INTERNAL_SECRET` trong file cấu hình `/opt/n8n/facebook auto posting 2.0/.env`; (3) Restart service `fb-bridge` qua `pm2 restart fb-bridge`, xác nhận trạng thái `online`; (4) Verify: Gọi HTTP POST `/api/facebook-post-v2` với secret cũ bị từ chối `401 Unauthorized`, secret sai bị từ chối `401 Unauthorized`, secret mới xác thực thành công. Theo `FIX_SPEC_2026-09-13-c`. | ✅ Stable | VPS Production |
| **`SNAP-20260913-147`** | 13/09/2026 13:45 | `v3.0-RC175` | 🔒⚡ **Dọn 2 Phát Hiện Bảo Mật Nhỏ Từ QA Security Audit (`scripts/`)**: (1) `scripts/.env.bridge.example`: Thay thế giá trị token secret giả định bằng generic placeholder `replace-with-a-strong-random-secret-generated-on-the-vps`; (2) `scripts/run-batch.js`: Chuyển đổi 2 vị trí sleep đồng bộ qua `execSync` spawn node child process sang `await new Promise(resolve => setTimeout(resolve, ...))` trong async IIFE runner; (3) Verify: `node --check scripts/run-batch.js` PASS 100%, `git diff --stat scripts/` chỉ 2 files. Theo `FIX_SPEC_2026-09-13-b`. | ✅ Stable | Git pending QA |
| **`SNAP-20260913-146`** | 13/09/2026 10:15 | `v3.0-RC174` | 📐🚀 **Tăng Tỷ Lệ Responsive Expand Activity Timeline Lên 60vh (`src/app/page.js`) Theo Phản Hồi Trải Nghiệm Thực Tế**: (1) **Expanded Drawer Height**: Tăng clamp từ `42vh` (320px-560px) lên `h-[clamp(380px,60vh,680px)]` giúp tối đa hóa không gian quan sát khi expand; (2) **Expanded Log List Max-Height**: Tăng clamp từ `34vh` (260px-460px) lên `max-h-[clamp(300px,50vh,580px)]` hiển thị số lượng dòng log vượt trội; (3) **Verify**: `node --check` PASS 100%, `npm run build` PASS 100% 31/31 routes Turbopack. Theo `FIX_SPEC_2026-09-13 (round 3)`. | ✅ Stable | Git pending QA |
| **`SNAP-20260913-145`** | 13/09/2026 10:10 | `v3.0-RC173` | 📐💻 **Chuyển Đổi Chiều Cao Expand Activity Timeline Sang Responsive Viewport (`clamp`) & Khóa Sàn An Toàn Master Table (`src/app/page.js`)**: (1) **Responsive Drawer Height**: Đổi chiều cao expand sang `h-[clamp(320px,42vh,560px)]` tự động co giãn 42% viewport height với sàn an toàn 320px và trần 560px; (2) **Responsive Log List Max-Height**: Đổi `logsMaxHeightClass` sang `max-h-[clamp(260px,34vh,460px)]` chừa khoảng đệm header/padding 60-100px; (3) **Sàn Tối Thiểu Candidates Table**: Thêm `min-h-[140px]` bảo đảm bảng danh sách trên không bao giờ bị bóp biến mất; (4) **Verify**: `node --check` PASS 100%, `npm run build` PASS 100% 31/31 routes Turbopack. Theo `FIX_SPEC_2026-09-13 (round 2)`. | ✅ Stable | Git pending QA |
| **`SNAP-20260913-144`** | 13/09/2026 10:00 | `v3.0-RC172` | 🔍📐 **Triển Khai Nút "Expand Activity Timeline" Cho Action Menu Dashboard (`src/app/page.js`) & Tham Số `logsMaxHeightClass` Cho `ActivityLogPanel` (Mục 11 Còn Treo Từ PLAN 11-Item)**: (1) **`ActivityLogPanel.js`**: Thêm prop `logsMaxHeightClass` (mặc định `"max-h-[220px]"`) bảo toàn 100% backward-compatibility cho Candidates Hub và Jobs Workbench; (2) **`page.js`**: Thêm icon `Maximize2`/`Minimize2`, state `isDetailExpanded`, điều khiển chiều cao drawer 3 trạng thái (`480px` khi expand, `260px` bình thường, `0px` khi ẩn), thêm nút toggle Expand/Collapse trước nút Hide, tự động reset `isDetailExpanded(false)` khi bấm Hide, và truyền `logsMaxHeightClass` (`max-h-[380px]` / `max-h-[220px]`); (3) **Docs**: Cập nhật mục 1.2 `USER_MANUAL_DRAFT.md` và `DEVELOPMENT_LOG.md`; (4) **Verify**: `node --check` PASS 100%, `npm run build` PASS 100% 31/31 routes Turbopack. Theo `FIX_SPEC_2026-09-13`. | ✅ Stable | Git pending QA |
...
| **`BACKFILL-20260909-01`** | 09/09/2026 23:06 | `v3.0-RC170b` | 📏🪛 **[BACKFILL 2026-09-13] Widen Inline Planning Date Field để Fit `dd-MMM-yyyy` Trên 1 Dòng**: `DateInputField` (`src/app/candidates/page.js`) hiển thị ‘Tháng - Ngày - Năm’ kèm icon clear/calendar trong 1 flex row `justify-between`; ô `w-24` (96px, `text-[11px]`) không đủ chỗ khiến ngày wrap 2-3 dòng dù còn nhiều khoảng trống ngang trên card ứng tuyển; mở rộng lên `w-36` (`px-2 py-1`) để hiển thị trên 1 dòng, đồng bộ với fix tương tự đã áp dụng cho Start/End Schedule time input ở `campaigns/page.js`. **Ghi chú:** entry này backfill ngày 2026-09-13 (audit mục 10.5 phát hiện gap) — commit gốc làm trực tiếp qua Claude Code (không qua bridge AG) nên bị bỏ sót log tại thời điểm đó. | ✅ Stable | Git `ec96415` |
...
| **`SNAP-20260907-125`** | 07/09/2026 08:25 | `v3.0-RC153` | 🛡️🔒 **Cấu Hình Vercel Firewall — Giới Hạn `/api/webhooks/*` Chỉ Nhận Request Từ IP VPS n8n (`127.0.0.1`)**: (1) **Vercel Firewall Configuration**: User cấu hình trực tiếp trên Vercel Dashboard (project `crm-ats-web`) 2 Custom Rules theo đúng thứ tự ưu tiên: Rule 1 `Allow n8n VPS` (`IP Address equals 127.0.0.1` -> `Bypass`), Rule 2 `Block Non-n8n on Webhooks` (`Request Path starts with /api/webhooks/` AND `IP Address does not equal 127.0.0.1` -> `Deny`); (2) **Verify 3/3 Tiêu Chí Bắt Buộc**: (a) **Chặn IP ngoài**: Gọi từ IP ngoài (kể cả khi gửi kèm `x-internal-secret` hợp lệ) vào `/api/webhooks/notifications` bị Vercel Firewall chặn ngay tại Edge với HTTP `403 Forbidden` (`x-matched-path: null`); (b) **Bypass n8n VPS**: n8n VPS (`127.0.0.1`) upload và xử lý CV thành công 100%, webhook callback và progress nhận 200 OK, tự động dọn sạch 100% test data; (c) **Auth & App an toàn**: `/api/auth/providers` (Google OAuth) và trang `/login` trả về 200 OK, phiên người dùng không bị ảnh hưởng. Theo `docs/testing/FIX_SPEC_2026-09-07_vercel-firewall-restrict-webhooks-to-n8n-ip.md`. | ✅ Stable | Vercel Dashboard + Git `98f6176` |
...
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/components/PendingCVClientWrapper.js`, `src/app/campaign_actions.js`, `src/app/campaigns/page.js`, `src/app/components/JoinStatusBadge.js`
- Nội dung:
  - **Notification Center Navigation (`src/app/components/PendingCVClientWrapper.js`)**: Kích hoạt `router.push(notif.link)` khi click vào notification item có `notif.link`, tự động đánh dấu đã đọc `handleMarkRead(notif.id)`, đóng panel `setIsOpen(false)`, thêm cursor pointer và hover border, bổ sung `e.stopPropagation()` cho nút Mark as read để không kích hoạt điều hướng nhầm khi chỉ muốn đánh dấu đã đọc.
  - **Social Groups Library Pinning (`src/app/campaign_actions.js`)**: Thêm tham số `focusGroupId = null` vào `getSocialGroupsLibrary(filters)`, bọc điều kiện filter cũ trong ngoặc và thêm `OR sgu.id = ${focusGroupId}` trong WHERE để group chỉ định luôn xuất hiện bất kể filter hiện tại, sắp xếp ưu tiên `ORDER BY (sgu.id = ${focusGroupId}) DESC, sgu.created_time DESC NULLS LAST` đưa group lên đầu trang 1.
  - **Campaigns Deep Linking & State Flow (`src/app/campaigns/page.js`)**: Sửa `useEffect` đọc `searchParams.get("group_id")` lưu vào state mới `focusGroupId` (loại bỏ lỗi nhét UUID vào ô search), truyền `focusGroupId` làm tham số thứ 7 xuống `fetchSocialGroupsLibrary` ở cả 3 vị trí (`loadSocialGroupsLibrary`, debounce `useEffect`, `handleLibraryPageChange`), truyền prop `autoOpenAnswer={g.id === focusGroupId}` xuống `JoinStatusBadge` trong bảng Social Groups Library.
  - **Auto Open Answer Popover & Smooth Scroll (`src/app/components/JoinStatusBadge.js`)**: Nhận prop `autoOpenAnswer = false`, khởi tạo `showAnswerPopover(autoOpenAnswer)`, thêm `useEffect` tự động cuộn `containerRef.current.scrollIntoView({ behavior: "smooth", block: "center" })` khi component mount với `autoOpenAnswer = true`.
- Verify:
  - `node --check` PASS 100% cả 4 files đã sửa.
  - `git diff --stat` xác nhận đúng 4 file code.
  - `npm run build` PASS 100% 31/31 routes Turbopack.
- ✅ Không có sai lệch so với spec.

---

### [2026-09-13 19:15] 4 Fix độc lập: Campaign Details layout, Timeline follow status, Planning Date default & Dedup contact point
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/campaigns/page.js`, `src/app/page.js`, `src/app/actions.js`, `src/app/hitl_actions.js`, `src/app/api/webhooks/cv-import/route.js`, `src/lib/utils.js`
- Nội dung:
  - **Phần 1 (Campaign Details Layout - `src/app/campaigns/page.js`)**:
    - Sửa `overflow-hidden` → `overflow-x-hidden` trên Master Campaigns Table container, triệt tiêu xung đột với `overflow-y-auto`, giữ bo góc ngang.
    - Giới hạn chiều cao cụm badge cột "Linked Job" với `max-h-16 overflow-y-auto` khi chiến dịch liên kết nhiều vị trí tuyển dụng.
    - Đặt sàn chiều cao tối thiểu `min-h-[360px]` cho Detail Expandable Panel, đảm bảo không gian thao tác không bị bóp nghẽn.
  - **Phần 2 (Timeline Follow Status - `src/app/page.js`)**:
    - Sửa nhánh `field === "status"` trong `handleInlineUpdate`: khi đổi status của bất kỳ dòng nào, luôn gọi `selectRow({ application_id: applicationId })` để Interview & Activity Timeline tự động chuyển tiêu điểm theo con trỏ thao tác.
  - **Phần 3 (Planning Date Default - `src/app/actions.js`)**:
    - Bổ sung cột `planning_date` với giá trị `CURRENT_DATE` vào câu lệnh INSERT activity trong hàm `createCandidateWithApplication()`.
  - **Phần 4 (Dedup Contact Point & Phone Format Variants)**:
    - Đưa hàm thuần túy `normalizeContactValue` vào `src/lib/utils.js` và export dùng chung cho toàn bộ dự án (`src/app/actions.js`, `src/app/hitl_actions.js`).
    - Nâng cấp deduplication trong nhánh MERGE của `src/app/hitl_actions.js`: chuẩn hoá giá trị cả 2 phía và so khớp lowercase type `(type || '').toLowerCase()`.
    - Nâng cấp `src/app/api/webhooks/cv-import/route.js`: bổ sung helper `phoneFormatVariants(raw)` sinh các biến thể `0xxx`, `+84xxx`, `84xxx` đưa vào `checkValues` để câu truy `= ANY(${checkValues})` bắt trúng mọi định dạng SĐT đã lưu trong DB.
- Verify:
  - `node --check` PASS 100% tất cả 6 files.
  - `npm run build` PASS 100% 31/31 routes Turbopack.
- ⚠️ Sai lệch so với spec:
  - Spec gốc yêu cầu: Phần 4.1 yêu cầu `export function normalizeContactValue` trực tiếp từ `src/app/actions.js` và Phần 4.2 import từ `./actions`.
  - Lỗi gặp phải: Next.js Turbopack compiler báo lỗi `Error: Server Actions must be async functions` tại `src/app/actions.js:954:17` khi build, vì `src/app/actions.js` là file Server Actions có chỉ thị `"use server"`. Hàm `normalizeContactValue` là synchronous pure utility, nếu chuyển sang `async` sẽ làm các biểu thức template string `${normalizeContactValue(...)}` và predicate trong `.filter()` bị chuyển thành `[object Promise]`, gây sai lệch logic deduplication.
  - Giải pháp thay thế đã dùng: Đặt `export function normalizeContactValue` trong `src/lib/utils.js` (nơi chứa các helper tiện ích dùng chung như `stripAccents`, `formatDateVN`), import vào `src/app/actions.js` và `src/app/hitl_actions.js`.
  - Đã báo Claude/User: Có, trong báo cáo này và mục 10.7 log.

---

### [2026-09-13 18:55] Header responsive & Lọc trùng địa chỉ HQ trong dropdown Job Location
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/layout.js`, `src/app/jobs/page.js`
- Nội dung:
  - Header responsive (`src/app/layout.js`): Thêm class `hidden md:inline` vào `<span>CRM-ATS 3.0</span>` để ẩn chữ ở màn hình hẹp, chỉ hiển thị từ breakpoint `md` trở lên; icon "ATS" giữ nguyên luôn hiển thị.
  - Deduplicate Job Location Dropdown (`src/app/jobs/page.js`): Cập nhật bộ lọc `.filter()` của `clientForm.branches` loại bỏ branch có địa chỉ trùng với `clientForm.address` (Main Headquarters cấp Client), ngăn chặn việc cùng 1 địa chỉ hiển thị 2 lần trong dropdown "Job Location/Address" khi branch bị un-set HQ nhưng chưa đồng bộ lại trường address của client.
- Verify:
  - `node --check src/app/layout.js src/app/jobs/page.js` PASS 100%.
  - `git diff --stat` xác nhận chỉ sửa đúng các file yêu cầu.
  - `npm run build` PASS 100% 31/31 routes Turbopack.
- ✅ Không có sai lệch so với spec.

---

### [2026-09-13 18:23] Bỏ dấu "+" thừa trong nút "Assign to Job" (Candidate 360°)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/candidates/page.js`
- Nội dung:
  - Bỏ ký tự "+" literal trong thẻ `<span>` của nút "Assign to Job" (dòng 1200) tại `src/app/candidates/page.js` thành `<span>Assign to Job</span>`.
  - Giữ nguyên icon `<Plus size={13} />`, class CSS và logic `onClick`.
  - Đồng bộ chuẩn UI với nút "Assign to Job Order" ở dòng 1661 trong cùng file.
- Verify:
  - `node --check src/app/candidates/page.js` PASS 100%.
  - `git diff --stat src/app/candidates/page.js` xác nhận chỉ sửa đúng file yêu cầu.
  - `npm run build` PASS 100% 31/31 routes Turbopack.
- ✅ Không có sai lệch so với spec.

---

### [2026-09-13 18:18] Đổi tên "LinkedIn Job Post" → "LinkedIn Jobs" (src/constants/enums.js)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/constants/enums.js`
- Nội dung:
  - Cập nhật giá trị chuỗi của `LINKEDIN_JOB_POST` trong `SOURCE_CHANNELS` từ `"LinkedIn Job Post"` sang `"LinkedIn Jobs"`.
  - Cập nhật phần tử tương ứng trong `SOURCE_CHANNELS_LIST` từ `"LinkedIn Job Post"` sang `"LinkedIn Jobs"`.
  - Giữ nguyên key `LINKEDIN_JOB_POST`, vị trí trong danh sách và 10 giá trị kênh còn lại.
  - Lý do: Đồng bộ với tên gọi phổ biến thực tế trên production (74 dòng "Linkedin Jobs" so với 1 dòng "Linkedin Job Posting").
- Verify:
  - `node --check src/constants/enums.js` PASS 100%.
  - `git diff --stat src/constants/enums.js` xác nhận chỉ sửa đúng `src/constants/enums.js`.
  - `npm run build` PASS 100% 31/31 routes Turbopack.
- ✅ Không có sai lệch so với spec.

---

### [2026-09-13 18:12] Chuẩn hoá danh sách Source Channels (SOURCE_CHANNELS & SOURCE_CHANNELS_LIST)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/constants/enums.js`
- Nội dung:
  - Cập nhật object `SOURCE_CHANNELS` và mảng `SOURCE_CHANNELS_LIST` trong `src/constants/enums.js` theo kết quả audit trực tiếp trên production (`public.activity.source_channel`).
  - Bổ sung kênh ITViec (`ITVIEC: "ITViec"`).
  - Tách riêng kênh LinkedIn Job Post (`LINKEDIN_JOB_POST: "LinkedIn Job Post"`) khỏi LinkedIn (`LINKEDIN: "LinkedIn"`).
  - Rút gọn 3 kênh có ngoặc / tiếng Việt về dạng tiếng Anh chuẩn ngắn gọn: `FACEBOOK: "Facebook"`, `REFERRAL: "Referral"`, `DIRECT_SOURCING: "Direct Sourcing"`.
  - Giữ nguyên các kênh chuẩn còn lại: TopCV, VietnamWorks, CareerBuilder, Company Website, Other.
- Verify:
  - `node --check src/constants/enums.js` PASS 100%.
  - `npm run build` PASS 100% 31/31 routes Turbopack.
  - `git diff --stat src/constants/enums.js` xác nhận chỉ sửa đúng `src/constants/enums.js`.
...
### [2026-09-13 18:05] Action Menu: Candidate Source dropdown chặt + Default/Auto sort theo Planning Date
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/page.js`
- Nội dung:
  - **Phần 1: Candidate Source dropdown chặt (9 lựa chọn)**:
    - Chuyển ô nhập Candidate Source từ `<input type="text" list="source-channels-list" ...>` sang `<select>` chặt chỉ gồm 9 giá trị trong `SOURCE_CHANNELS_LIST`.
    - Bổ sung fallback option `{app.source_channel} (legacy)` hiển thị an toàn khi bản ghi cũ trong DB có giá trị ngoài 9 kênh chuẩn.
    - Xoá phần tử `<datalist id="source-channels-list">` không còn sử dụng.
  - **Phần 2: Default sort & Auto re-sort theo Planning Date**:
    - Đổi giá trị khởi tạo state `planningDateSort` từ `null` thành `'desc'` để mặc định load bảng sắp xếp ngày mới nhất lên đầu.
    - Bổ sung nhánh `else if (field === "planning_date")` trong hàm `handleInlineUpdate` để tự động sắp xếp lại danh sách ứng viên cục bộ ngay khi người dùng chỉnh sửa Planning Date inline (bảo toàn quy tắc `NULLS LAST`).
- Verify:
  - `node --check src/app/page.js` PASS 100%.
  - `npm run build` PASS 100% 31/31 routes Turbopack.
  - `git diff --stat` xác nhận chỉ có `src/app/page.js` và `docs/DEVELOPMENT_LOG.md` thay đổi.
- ✅ Không có sai lệch so với spec.

---

### [2026-09-13 17:42] Environment separation Giai đoạn 2b: Workflow C (FB Auto-Warm & Group Auto-Joiner, nhánh Webhook)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/campaign_actions.js`, `src/app/api/webhooks/warm-join-run-callback/route.js`, `src/app/api/webhooks/warm-join-run-progress/route.js`, n8n workflow `L8QdckqW7FDwanRq` (nodes `Validate Internal Secret`, `Prepare Cron Context`, `Smart Group Allocator & Dispatcher`, `Process Bridge Warm Results`, `Build Final Warm Run Summary`)
- Nội dung:
  - **Phần 1: Next.js (2 route + 1 hàm dispatch)**:
    - `src/app/campaign_actions.js`: Trong `triggerWarmJoinRun()`, bổ sung `environment: process.env.DB_SCHEMA || 'public'` vào payload dispatch gửi tới n8n webhook.
    - `src/app/api/webhooks/warm-join-run-callback/route.js`: Import `sqlSandbox` từ `src/lib/db.js`, định tuyến `sqlClient = body.environment === 'sandbox' ? sqlSandbox : sql`, chạy `sqlClient.begin(async (sqlTx) => ...)` đóng run và cập nhật notification/campaign status đúng schema.
    - `src/app/api/webhooks/warm-join-run-progress/route.js`: Import `sqlSandbox`, định tuyến `sqlClient = body.environment === 'sandbox' ? sqlSandbox : sql`, chạy `sqlClient.begin(...)` ghi nhận kết quả per-account, per-group và cập nhật milestone notification đúng schema.
  - **Phần 2: n8n workflow C ("C: FB Auto-Warm & Group Auto-Joiner", ID `L8QdckqW7FDwanRq`)**:
    - Node `Validate Internal Secret`: Thêm `environment: body.environment || 'public'` vào JSON output.
    - Node `Prepare Cron Context`: Hardcode `environment: 'public'` cố định cho nhánh Schedule Trigger để luôn chạy production thật.
    - Node `Smart Group Allocator & Dispatcher`: Thêm `environment: triggerContext.environment || 'public'` vào kết quả phân bổ.
    - Node `Process Bridge Warm Results`: Đọc `allocatorData.environment || 'public'` và gắn vào từng item JSON output (`runId`, `environment`, `completedAccount`).
    - Node `Build Final Warm Run Summary`: Đọc `environment` từ node trước và đưa vào payload callback cuối cùng.
    - Workflow đã được publish active (`activeVersionId: e8875941-4b87-410e-b874-e6fd7ef09946`, `sameAsDraft: true`).
- Verify:
  - `node --check` PASS 100% trên `src/app/campaign_actions.js`, `src/app/api/webhooks/warm-join-run-callback/route.js`, `src/app/api/webhooks/warm-join-run-progress/route.js`.
  - `npm run build` PASS 100% 31/31 routes Turbopack.
  - n8n `get_workflow_details` xác nhận đúng 5 Code nodes đã cập nhật chính xác (đặc biệt `Prepare Cron Context` hardcode `'public'`), không thay đổi connections hay node khác, active version đã được kích hoạt.
- ✅ Không có sai lệch so với spec.

---

### [2026-09-13 16:35] Environment separation Giai đoạn 2: Workflow A (FB Group Auto-Post) & CV Parser (tunnel baseUrl)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/lib/db.js`, `src/app/campaign_actions.js`, `src/app/api/webhooks/campaign-run-callback/route.js`, `src/app/api/webhooks/campaign-run-progress/route.js`, `src/app/api/cv-upload-proxy/route.js`, n8n workflow `9W588GooZeZhiSKm` (nodes `Process Bridge Results`, `Build Final Run Summary`), n8n workflow `fofSZKkdyhlVd9Lc` (node `Config`)
- Nội dung:
  - **Phần 1: Workflow A - FB Group Auto-Post (environment-flag pattern)**:
    - `src/lib/db.js`: Thêm named export `sqlSandbox` kết nối trực tiếp với search_path `sandbox,public,extensions` (max pool 3, prepare: false) dùng cho webhook callbacks khi chạy môi trường sandbox.
    - `src/app/campaign_actions.js`: Bổ sung `environment: process.env.DB_SCHEMA || 'public'` vào payload dispatch gửi n8n webhook.
    - `src/app/api/webhooks/campaign-run-callback/route.js` & `src/app/api/webhooks/campaign-run-progress/route.js`: Định tuyến `sqlClient = body.environment === 'sandbox' ? sqlSandbox : sql` và gọi `sqlClient.begin(...)` cùng `checkCampaignAutoCompletion(campaignIdToVerify, sqlClient)` đảm bảo toàn bộ ghi nhận tiến trình/kết quả chiến dịch cập nhật đúng schema tương ứng.
    - n8n workflow `9W588GooZeZhiSKm`: Cập nhật node `Process Bridge Results` và `Build Final Run Summary` truyền tiếp field `environment` qua các webhook callback/progress.
  - **Phần 2: CV Parser (tunnel baseUrl pattern)**:
    - `src/app/api/cv-upload-proxy/route.js`: Tự động đính kèm `baseUrl: 'https://ats-dev.thucnguyen8n.space'` vào formData gửi sang n8n webhook khi `DB_SCHEMA === 'sandbox'`.
    - n8n workflow `fofSZKkdyhlVd9Lc`: Cập nhật node `Config` (`main_config`) trích xuất `incomingBaseUrl = item.baseUrl || item.body.baseUrl`, fallback về `https://crm-ats-web-hazel.vercel.app`, cho phép sub-workflow và toàn bộ chuỗi webhook cv-batch / notifications trỏ về đúng dev tunnel khi upload từ môi trường sandbox.
- Verify:
  - `node --check` PASS 100% trên cả 5 file JS trong `src/`.
  - `npm run build` PASS 100% 31/31 routes Turbopack.
  - n8n: `get_workflow_details` xác nhận activeVersionId của cả 2 workflow (`9W588GooZeZhiSKm` và `fofSZKkdyhlVd9Lc`) khớp versionId hiện tại và `sameAsDraft: true`.
- ✅ Không có sai lệch so với spec.

---

### [2026-09-13 16:05] Environment-aware routing cho Google Contacts Sync (Giai đoạn 1)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/actions.js`, `src/app/api/webhooks/notifications/route.js`, n8n workflow `O659fZyN2uGaaLyL` (nodes `n009`, `n012`, `n015`)
- Nội dung: 
  - `src/app/actions.js`: Bổ sung field `environment: process.env.DB_SCHEMA || 'public'` vào payload dispatch của `syncCandidatesToGoogleContacts()` gửi tới n8n webhook.
  - `src/app/api/webhooks/notifications/route.js`: Đọc `targetSchema` từ `body.environment` (`sandbox` hoặc `public`), áp dụng identifier động `${sql(targetSchema)}.notifications` cho cả 2 câu truy vấn UPDATE và INSERT, ghi log schema mục tiêu.
  - n8n Workflow "A2: Save Contacts → Google Sync (ATS 3.0)" (`O659fZyN2uGaaLyL`): Cập nhật 3 node Code (`Build Create Payload`, `Build Update Payload`, `Build Conflict Message`) lấy `environment` từ `Save Contact Webhook` và gắn vào `notificationPayload`, sau đó publish active version mới.
- Verify:
  - `node --check src/app/actions.js` và `node --check src/app/api/webhooks/notifications/route.js` PASS 100%.
  - `npm run build` PASS 100% 31/31 routes Turbopack.
  - n8n `get_workflow_details` xác nhận 3 node đã cập nhật `environment`, workflow publish thành công (`activeVersionId: 5bb2fa7c-0a69-4cf0-b1a0-dce7b708238c`).
- ✅ Không có sai lệch so với spec.

---

### [2026-09-13 14:48] Bổ sung Persistent Notification "Đã gửi yêu cầu" ngay khi dispatch Google Contacts Sync
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/actions.js`, `docs/DEVELOPMENT_LOG.md`
- Nội dung: 
  - Trong `syncCandidatesToGoogleContacts()` (`src/app/actions.js`), bổ sung câu lệnh `INSERT INTO notifications` ngay sau khi build payload và trước khi gọi `fetch` webhook.
  - Cấu trúc notification: `type: 'google_contacts_sync'`, `severity: 'info'`, `title` và `message` hiển thị tên ứng viên đơn lẻ hoặc tổng hợp danh sách (rút gọn "tên 1, tên 2, tên 3 và N candidate khác" nếu >3 ứng viên), `link` trỏ tới ATS URL candidate nếu là single sync, và `metadata: { candidateIds, action: 'dispatched' }`.
  - Đặt trước khối gọi webhook n8n đảm bảo notification lưu vết dispatch luôn được ghi nhận trong Notification Center dù webhook có phản hồi hay không.
- Verify: `node --check src/app/actions.js` PASS 100%, `npm run build` Turbopack PASS 100% 31/31 routes.

---

### [2026-09-13 14:25] Tích hợp Google Contacts Sync (Server Action, Single/Bulk UI & Workflow n8n ATS 3.0)
- Viết bởi: Antigravity (Implementer)
- Commit: pending (`git log --oneline -1`)
- Files: `src/app/actions.js`, `src/app/candidates/page.js`, `src/app/search/page.js`, `.env.local`, `docs/DEVELOPMENT_LOG.md`
- Nội dung: 
  - **Nhóm 1 (Code `src/`)**: 
    - Thêm guard bảo mật `assertRealRequestContext` và Server Action `syncCandidatesToGoogleContacts(candidateIds)` trong `src/app/actions.js`.
    - Thêm nút "Save to Google Contacts" trên header Candidate 360 (`src/app/candidates/page.js`).
    - Thêm multi-select checkboxes (header + row) và banner bulk action trên tab Candidate của Search Menu (`src/app/search/page.js`), giữ nguyên `▶` single selector và không ảnh hưởng tab Client/Job.
    - Thêm placeholder `N8N_GOOGLE_CONTACTS_SYNC_WEBHOOK_URL=` vào `.env.local`.
  - **Nhóm 2 (Workflow n8n `O659fZyN2uGaaLyL`)**: 
    - Đã cập nhật thành công qua `n8n-mcp-vps` (đổi tên thành "A2: Save Contacts → Google Sync (ATS 3.0)").
    - Thêm `headerAuth` dùng credential "ATS 3.0 Internal Webhook Secret" (`Je1dHcRXyZhrXODl`).
    - Gỡ bỏ 3 node Notion (`Extract Page ID`, `Get Candidate`, `Query Contact Points`) và 3 node Telegram.
    - Tách `Get All Google Contacts` ra trước vòng loop xử lý batch candidates.
    - Nối Webhook → Get All Google Contacts → Extract Candidates → Loop: Candidate Batch (`splitInBatches` v3) → Normalize Contacts → Match Logic → Route by Match Count → Build Payload → Create/Update Google Contact → 3 node HTTP Request gửi Notification Center (`/api/webhooks/notifications`) → Loop back.
    - Cập nhật toàn bộ link/message format từ Notion CRM sang ATS 3.0. Giữ workflow ở trạng thái `active: false`.
- Verify: `node --check` PASS 100% trên 3 file `actions.js`, `candidates/page.js`, `search/page.js`; `npm run build` Turbopack PASS 100% 31/31 routes.

---

### [2026-09-13 13:50] Rotate BRIDGE_INTERNAL_SECRET Thật Trên VPS Production
- Viết bởi: Antigravity (Implementer)
- Commit: N/A (VPS Production Operation)
- Files: /opt/n8n/facebook auto posting 2.0/.env (VPS), docs/DEVELOPMENT_LOG.md
- Nội dung:
  1. Đăng nhập SSH VPS TinoHost theo thông tin bảo mật, xác nhận vị trí file cấu hình `/opt/n8n/facebook auto posting 2.0/.env`.
  2. Sinh secret mới ngẫu nhiên 32-byte (64 hex characters) bằng lệnh `openssl rand -hex 32` trực tiếp trên VPS.
  3. Cập nhật duy nhất biến `BRIDGE_INTERNAL_SECRET` trong `/opt/n8n/facebook auto posting 2.0/.env` bằng secret mới, bảo toàn nguyên vẹn `BRIDGE_PORT=5680` và `BRIDGE_TIMEOUT_MS=3300000`.
  4. Khởi động lại service `fb-bridge` bằng `pm2 restart fb-bridge`, kiểm tra `pm2 status` xác nhận tiến trình `online` ổn định.
  5. Kiểm tra an toàn (Rule verify): Gửi request kèm secret cũ đã lộ -> trả về HTTP `401`; gửi request kèm secret sai -> trả về HTTP `401`; gửi request kèm secret mới -> vượt qua lớp xác thực thành công (200).
- Verify:
  - `grep -rl "BRIDGE_INTERNAL_SECRET" "/opt/n8n/facebook auto posting 2.0/"` -> đúng 2 file `bridge-server.js` và `.env`.
  - `pm2 status` -> `fb-bridge` status `online` (pid 91984).
  - Curl wrong secret -> HTTP code `401`.
  - Curl leaked old secret -> HTTP code `401`.
  - Curl new secret -> HTTP code `200` (authenticated).

---

### [2026-09-13 13:45] Dọn 2 Phát Hiện Bảo Mật Nhỏ Từ QA Security Audit Trong scripts/
- Viết bởi: Antigravity (Implementer)
- Commit: pending-qa (`git log --oneline -1`)
- Files: scripts/.env.bridge.example, scripts/run-batch.js, docs/DEVELOPMENT_LOG.md
- Nội dung:
  1. `scripts/.env.bridge.example`: Thay thế giá trị token secret giả định bằng generic placeholder rõ ràng `replace-with-a-strong-random-secret-generated-on-the-vps`, giữ nguyên 2 biến cấu hình `BRIDGE_PORT` và `BRIDGE_TIMEOUT_MS`.
  2. `scripts/run-batch.js`: Thay thế 2 vị trí dùng `execSync` spawn child node process để sleep (dòng 133 inter-account cooldown và dòng 240 inter-job delay) thành native promise-based delay `await new Promise(resolve => setTimeout(resolve, waitMs))` / `await new Promise(resolve => setTimeout(resolve, delayMs))` bên trong async IIFE runner, loại bỏ pattern string interpolation vào shell command và tối ưu tài nguyên thực thi.
- Verify:
  - `node --check scripts/run-batch.js` PASS 100%.
  - `git diff --stat scripts/` xác nhận chỉ 2 file trong phạm vi bị thay đổi.
  - Kiểm tra trực tiếp `scripts/.env.bridge.example` xác nhận chỉ chứa placeholder token an toàn.

---

### [2026-09-13 10:15] Tăng Tỷ Lệ Responsive Clamp Expand Activity Timeline Lên 60vh (Round 3)
- Viết bởi: Antigravity (Implementer)
- Commit: `0fc9cf4`
- Files: src/app/page.js, docs/DEVELOPMENT_LOG.md
- Nội dung:
  1. Tăng tỷ lệ responsive cho Expanded bottom drawer "Interview & Activity Timeline" trong `src/app/page.js` từ `h-[clamp(320px,42vh,560px)]` lên `h-[clamp(380px,60vh,680px)]` theo phản hồi trải nghiệm thực tế của Thức (PO), bảo đảm drawer mở rộng rõ rệt khi expand trên mọi kích thước màn hình.
  2. Tăng prop `logsMaxHeightClass` truyền vào `<ActivityLogPanel>` từ `max-h-[clamp(260px,34vh,460px)]` lên `max-h-[clamp(300px,50vh,580px)]` để danh sách nhật ký tận dụng trọn vẹn không gian mở rộng mới.
  3. Giữ nguyên trạng thái thu gọn `260px`/`220px`, trạng thái ẩn `h-0`, và sàn an toàn `min-h-[140px]` của Candidates Master Table.
- Verify:
  - `node --check src/app/page.js` PASS 100%.
  - `git diff --stat src/` xác nhận chỉ file `src/app/page.js` bị thay đổi trong round 3.
  - Turbopack `npm run build` PASS 100% 31/31 routes (0 errors).

---

### [2026-09-13 10:10] Responsive Clamp Chiều Cao Expand Activity Timeline & Sàn An Toàn Master Table (Round 2)
- Viết bởi: Antigravity (Implementer)
- Commit: `0fc9cf4`
- Files: src/app/page.js, docs/DEVELOPMENT_LOG.md
- Nội dung:
  1. Chuyển đổi chiều cao Expanded bottom drawer "Interview & Activity Timeline" trong `src/app/page.js` từ kích thước cố định `480px` sang responsive `h-[clamp(320px,42vh,560px)]` (chiếm 42% viewport height, tối thiểu 320px, tối đa 560px), giữ nguyên `260px` khi bình thường và `h-0` khi ẩn.
  2. Cập nhật prop `logsMaxHeightClass` truyền vào `<ActivityLogPanel>` sang `max-h-[clamp(260px,34vh,460px)]` khi expand, giữ khoảng đệm hợp lý 60-100px tránh tràn container.
  3. Thêm sàn an toàn `min-h-[140px]` cho Master Table container phía trên (dòng ~819) để bảo đảm danh sách ứng viên luôn hiển thị tối thiểu 1-2 dòng ngay cả khi drawer chạm trần trên màn hình độ phân giải thấp (1366×768).
- Verify:
  - `node --check src/app/page.js` PASS 100%.
  - `git diff --stat src/` xác nhận chỉ file `src/app/page.js` bị thay đổi trong round 2.
  - Turbopack `npm run build` PASS 100% 31/31 routes (0 errors).

---

### [2026-09-13 10:00] Triển Khai Nút Expand Activity Timeline Cho Action Menu Dashboard (Mục 11 PLAN 11-Item)
- Viết bởi: Antigravity (Implementer)
- Commit: `0fc9cf4`
- Files: src/components/ActivityLogPanel.js, src/app/page.js, docs/USER_MANUAL_DRAFT.md, docs/DEVELOPMENT_LOG.md
- Nội dung:
  1. Thêm prop `logsMaxHeightClass = "max-h-[220px]"` vào `src/components/ActivityLogPanel.js` cho phép tùy biến chiều cao scroll list log mà vẫn giữ nguyên 100% backward-compatibility cho 2 nơi dùng chung khác (`candidates/page.js`, `jobs/page.js`).
  2. Bổ sung state `isDetailExpanded` và import `Maximize2`, `Minimize2` trong `src/app/page.js`.
  3. Cấu hình container drawer "Interview & Activity Timeline" co giãn 3 trạng thái (`h-[480px]` khi expand, `h-[260px]` bình thường, `h-0` khi ẩn) với transition mượt mà.
  4. Thêm nút toggle "Expand" / "Collapse" trên header drawer (đặt trước nút "Hide"), tái sử dụng pattern chuẩn từ Jobs Workbench (`jobs/page.js`).
  5. Tự động reset `isDetailExpanded(false)` khi click nút "Hide" để bảo đảm lần mở peek lại sau luôn bắt đầu ở kích thước thu gọn chuẩn.
  6. Truyền `logsMaxHeightClass={isDetailExpanded ? "max-h-[380px]" : "max-h-[220px]"}` vào `ActivityLogPanel` để hiển thị nhiều dòng nhật ký hơn rõ rệt khi expand.
  7. Cập nhật tài liệu hướng dẫn sử dụng `docs/USER_MANUAL_DRAFT.md` mục 1.2.
- Verify:
  - `node --check src/components/ActivityLogPanel.js` PASS 100%.
  - `node --check src/app/page.js` PASS 100%.
  - `git diff --stat src/` xác nhận chỉ 2 file code được thay đổi.
  - Turbopack `npm run build` PASS 100% 31/31 routes (30.5s, 0 errors).

---

### [2026-09-09 23:06] [BACKFILL] Widen inline Planning Date field so dd-MMM-yyyy fits on one line
- Viết bởi: Claude (qua Claude Code, sửa trực tiếp — KHÔNG qua bridge Antigravity)
- Commit: `ec96415`
- Files: src/app/candidates/page.js
- Nội dung: `DateInputField` render “11 - Sep - 2026” + icon clear/calendar trong 1 flex row `justify-between`; ô `w-24` (96px) ở `text-[11px]` không đủ chỗ ngang, buộc ngày wrap thành 2-3 dòng dù còn nhiều khoảng trống trên card ứng tuyển (cạnh badge Source / Planning / Inbound Apply). Mở rộng lên `w-36` kèm `px-2 py-1` để hiển thị trên 1 dòng, khớp cách fix tương tự đã áp dụng cho Start/End Schedule time input ở campaigns/page.js.
- Verify: Thuần thay đổi class CSS, không đổi logic/DB — không có kết quả test tự động ghi lại tại thời điểm commit gốc.
- Ghi chú backfill: Phát hiện qua audit đối chiếu `git log` vs `DEVELOPMENT_LOG.md` (mục 10.5) ngày 2026-09-13 — commit `ec96415` nằm giữa 2 commit đều có entry (`2510f6c` → SNAP-142, `d5ee59e` → SNAP-143) nhưng không có entry riêng. Nguyên nhân: commit này làm trực tiếp qua phiên Claude Code (đồng tác giả `Claude Sonnet 5`, không dẫn FIX_SPEC nào), nằm ngoài luồng bridge Claude↔Antigravity nên không bị ràng buộc bởi nghĩa vụ ghi log “cùng lượt” mục 10.2 (vốn áp dụng cho Antigravity). Backfill sau khi được Thức xác nhận cho phép.
...
### [2026-09-07 08:25] Cấu Hình Vercel Firewall: Giới Hạn /api/webhooks/* Chỉ Nhận Từ IP VPS n8n (127.0.0.1)
- Viết bởi: Antigravity (Implementer)
...
- Nội dung:
  1. **Thiết lập 2 Custom Firewall Rules trên Vercel Dashboard**:
     - **Rule 1 (`Allow n8n VPS`)**: `If IP Address equals 127.0.0.1` -> `Then Bypass` (đảm bảo n8n VPS luôn thông suốt, không bị ảnh hưởng bởi các lớp bot challenge).
     - **Rule 2 (`Block Non-n8n on Webhooks`)**: `If Request Path starts with /api/webhooks/ AND IP Address does not equal 127.0.0.1` -> `Then Deny` (chặn 100% request từ Internet vào các endpoint webhook nếu không xuất phát từ IP VPS n8n).
  2. **Kiểm thử đối soát 3 tiêu chí bắt buộc**:
     ...
     - **Test 2 (Bypass n8n VPS)**: n8n VPS (`127.0.0.1`) chạy CV Parser upload thành công tạo batch và notification với HTTP `200 OK`, tự động dọn sạch dữ liệu test.
...
- Nội dung:
  1. **PHẦN 1 (Google OAuth + Email Allowlist qua Auth.js)**:
     ...
     - Triển khai callback `signIn`: Chuẩn hoá email chữ thường và đối chiếu với danh sách `ALLOWED_EMAILS` (chứa `demo@ats-prototype.local`), tự động chặn các email lạ và chuyển hướng về `/login?error=AccessDenied`.
...
- Verify:
  ...
  - n8n Webhook Connectivity Test: `POST https://n8n.example.com/webhook/cv-upload` trả về HTTP 200 OK `{"message":"Workflow was started"}`.
...
- Nội dung: Triển khai toàn bộ hotfix khẩn cấp theo spec `docs/testing/FIX_SPEC_2026-09-05_campaign-fb-autopost_warmjoin-cron-lock-and-secret-hotfix.md` dựa trên QA độc lập của Claude:
  ...
  - **PHẦN 3 & 4 (n8n Workflow C & Credentials)**:
    1. Tạo 2 credentials chuẩn kiểu `httpHeaderAuth` trực tiếp trên n8n VPS (`n8n.example.com`):
...
- Nội dung: Triển khai toàn diện hệ thống nuôi nick FB tuần tự (Sequential Warming & Auto-Join Groups) theo tài liệu kiến trúc `docs/architecture/BLUEPRINT_2026-09-04_fb-account-warming-and-rotation-strategy.md` xuyên suốt 4 Pha:
  ...
  - **Phase 2: n8n Workflow C & Dedicated Cloudflare Tunnel**:
    1. Xây dựng và kích hoạt workflow `C: FB Auto-Warm & Group Auto-Joiner` (ID: `L8QdckqW7FDwanRq`) nằm trong folder `ATS 3.0` (`y4ZDeiEYOE0cUDFZ`) trên VPS n8n (`n8n.example.com`), production webhook URL: `https://n8n.example.com/webhook/warm-join-trigger`.
  ...
  - **Phase 3: Server Actions & Giao Diện ATS 3.0 UI**:
    1. Cấu hình `.env.local`: bổ sung biến `N8N_WARM_JOIN_TRIGGER_WEBHOOK_URL=https://n8n.example.com/webhook/warm-join-trigger`.
...
- Verify:
  ...
  - `https://crm-ats-9sqdcld9c-demo-user-portfolio.vercel.app` ➔ HTTP 404 NOT FOUND (Offline 100%).
...
### [2026-09-13 19:00 ICT] Kích Hoạt Dropdown Chọn Version CV Cũ (Embedded CV Viewer) Trong Candidate 360° (`src/app/candidates/page.js`)
- Viết bởi: Antigravity (Implementer)
- Commit: <hash> (`git log --oneline -1`)
- Files:
  - `src/app/candidates/page.js`
  - `docs/DEVELOPMENT_LOG.md`
- Nội dung:
  - **Hiện tượng & Bối cảnh**: Dropdown chọn version CV cũ trong tab "Embedded CV Viewer" trên giao diện Candidate 360° đã được code sẵn đầy đủ (`allCvUrls` useMemo, dropdown UI) nhưng bị vô hiệu hoá hoàn toàn do hàm `loadCandidateData()` không gán field `cv_urls` từ object candidate trả về từ backend vào state `formData`.
  - **Khắc phục**: Bổ sung `cv_urls: cand.cv_urls || []` vào `setFormData` trong `loadCandidateData()` tại `src/app/candidates/page.js`.
  - **Không đụng chạm**: Giữ nguyên `allCvUrls` useMemo, dropdown UI, `getCandidateProfile()` trong `actions.js`, và modal "Add New CV Version".
- Verify:
  - `node --check src/app/candidates/page.js` → PASS 100%.
  - `npm run build` → PASS 100% 31/31 routes Turbopack.
