...
2. **Chạy thật script** `scripts/archive/data-mutating-oneoffs/2026-09-01_normalize-legacy-result-reason.mjs --apply` trên Supabase production đang dùng (project `mock-supabase-project`), dán lại nguyên văn log console (cả dry-run lẫn apply) và số record thực tế bị ảnh hưởng.
