...
**Bắt buộc chạy migration này lên đúng Supabase project production đang dùng** (project `your-project-ref`, schema `public` — chú ý bài học từ round 1 của spec Stage/Result/Reason: phải qualify đúng schema `public.activity_log`, không được để search_path mặc định trỏ nhầm `sandbox`).
