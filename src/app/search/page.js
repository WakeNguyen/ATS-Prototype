  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col h-full max-h-full bg-slate-950 text-slate-100 font-sans select-none overflow-hidden">
      ...
      <div className="flex-1 min-h-0 flex flex-col bg-slate-950 overflow-hidden">
        {activeTab === "candidate" && selectedCandidateIds.size > 0 && (
          <div className="px-4 pt-2">
            <div className="flex items-center justify-between px-4 py-2 bg-emerald-950/40 border border-emerald-500/40 rounded-lg text-xs">
              <span className="text-emerald-300 font-semibold">{selectedCandidateIds.size} candidate(s) selected</span>
              <div className="flex items-center gap-3">
                <button onClick={() => setSelectedCandidateIds(new Set())} className="text-slate-400 hover:text-slate-200 underline cursor-pointer">Clear selection</button>
                <button
                  onClick={async () => {
          ...
          {activeTab === "candidate" && (
            <table className="w-full text-xs text-left border-collapse select-text">
              <thead className="bg-slate-900 text-slate-300 font-extrabold sticky top-0 z-10 border-b border-slate-800 text-center uppercase text-[10px] tracking-wider">
                <tr>
                  <th className="w-12 p-1.5 border-r border-slate-800 text-center">
                    <input
                      type="checkbox"
                      checked={candidates.length > 0 && candidates.every(c => selectedCandidateIds.has(c.candidate_id))}
                      onChange={() => {
                        setSelectedCandidateIds(prev => {
                          const allSelected = candidates.length > 0 && candidates.every(c => prev.has(c.candidate_id));
                          if (allSelected) return new Set();
                          return new Set(candidates.map(c => c.candidate_id));
                        });
                      }}
