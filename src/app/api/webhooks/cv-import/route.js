...
export async function POST(req) {
  try {
    ...
    // Prepare list of contact values to check for duplicates (bao gồm cả biến thể định dạng số
    // điện thoại 0xxx / +84xxx / 84xxx, vì DB có thể lưu bất kỳ định dạng nào tuỳ lần nhập trước đó)
    function phoneFormatVariants(raw) {
      const digits = (raw || '').replace(/[^\d+]/g, '');
      const variants = new Set([digits.toLowerCase()]);
    ...
    for (const cp of data.contactPoints) {
      const v = cp.value.trim().toLowerCase();
      checkValues.push(v);
      if ((cp.type || '').toLowerCase().includes('phone')) {
        phoneFormatVariants(cp.value).forEach(variant => checkValues.push(variant.toLowerCase()));
      }
    }
