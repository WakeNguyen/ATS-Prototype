...
export async function POST(req) {
  try {
    ...
    const targetSchema = body.environment === 'sandbox' ? 'sandbox' : 'public';

    if (body.id) {
      const [row] = await sql`
        UPDATE ${sql(targetSchema)}.notifications
        SET
    ...
    const [row] = await sql`
      INSERT INTO ${sql(targetSchema)}.notifications (type, title, message, severity, link, metadata)
      ...
      RETURNING id
    `;
    console.log('[notifications] INSERTED notification id:', row?.id, 'title:', body.title, 'schema:', targetSchema);
