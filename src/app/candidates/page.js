...
function Candidate360Content() {
  ...
  const [showAppendCvModal, setShowAppendCvModal] = useState(false);
  const [appendCvUrl, setAppendCvUrl] = useState("");
  const [appendCvFilename, setAppendCvFilename] = useState("");
  const [isAppendingCv, setIsAppendingCv] = useState(false);
  ...
  // Append New CV Version Handler
  async function handleAppendCvVersion(e) {
    if (e) e.preventDefault();
    if (!candidate?.id || !appendCvUrl.trim()) return;

    setIsAppendingCv(true);
    try {
      const res = await appendCvVersion({
        candidateId: candidate.id,
        cvUrl: appendCvUrl.trim(),
        originalFilename: appendCvFilename.trim()
      });
      if (res.success) {
        notify("✓ New CV version added successfully!");
        setShowAppendCvModal(false);
        setAppendCvUrl("");
        setAppendCvFilename("");
        await loadCandidateData(candidate.id);
        setActiveTab("cv_viewer");
      } else {
        notify("Failed to add CV version: " + (res.error || "Unknown error"));
      }
    } catch (err) {
      notify("Error adding CV version: " + err.message);
    } finally {
      setIsAppendingCv(false);
    }
  }
  ...
  const allCvUrls = useMemo(() => {
    ...
    if (Array.isArray(formData.cv_urls) && formData.cv_urls.length > 0) {
      ...
      if (formData.cv_url && !formData.cv_urls.some(c => c.url === formData.cv_url)) {
        list.push({ url: formData.cv_url, label: 'Current CV (Direct Link)' });
      }
    } else if (formData.cv_url) {
      list.push({ url: formData.cv_url, label: 'Primary CV (v1)' });
    }
    return list;
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setShowAppendCvModal(true)}
                    className="text-[10px] text-emerald-400 hover:text-emerald-300 bg-emerald-950/40 hover:bg-emerald-900/50 border border-emerald-800/60 px-2 py-0.5 rounded-lg flex items-center gap-1 font-semibold transition-colors"
                    title="Append a new CV version without losing previous versions"
                  >
                    <Plus size={10} />
              ...
              <input
                ...
                className="w-full px-3 py-1.5 text-xs font-mono bg-slate-950 border border-slate-700 rounded-xl text-slate-300 focus:outline-none focus:border-emerald-500"
                placeholder="https://drive.google.com/file/d/..."
              />
              <p className="text-[10px] text-slate-500 mt-1">
                Direct edits here auto-save old CV to version history. Or click <strong className="text-emerald-400">+ Add Version</strong> to append explicitly.
              </p>
            </div>
              ...
              {!embeddableCvUrl ? (
                <div className="flex-1 flex flex-col items-center justify-center text-slate-500 gap-3">
                  ...
                  <p className="text-[11px] text-slate-400 max-w-sm text-center">
                    Paste a Google Drive share link into the "CV Link" field on the left form and click Save, or click below to add a version.
                  </p>
                  <button
                    type="button"
                    onClick={() => setShowAppendCvModal(true)}
                    className="mt-2 px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold flex items-center gap-1.5 shadow-lg shadow-emerald-950 transition-colors"
                  >
                    <Plus size={13} />
                    <span>Add CV Version</span>
                  </button>
                </div>
              ) : (
                  ...
                  <div className="px-3 py-1.5 bg-slate-950 border-b border-slate-800 flex items-center justify-between text-xs text-slate-400">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => setShowAppendCvModal(true)}
                        className="px-2.5 py-1 rounded-lg bg-emerald-950/60 hover:bg-emerald-900/60 border border-emerald-700/50 text-emerald-300 text-[11px] flex items-center gap-1 font-semibold transition-colors"
                      >
                        <Plus size={12} />
                        <span>+ Add Version</span>
                      </button>
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-purple-300 text-[11px] flex items-center gap-1 font-semibold transition-colors"
                      >
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 bg-slate-950">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-100">Add New CV Version</h3>
              </div>
              <button
                type="button"
                onClick={() => setShowAppendCvModal(false)}
            ...
            <form onSubmit={handleAppendCvVersion} className="p-5 space-y-4">
              <div className="p-3 bg-slate-950/80 border border-slate-800 rounded-xl text-xs space-y-1">
                <p className="text-slate-400">
                  Adding version for: <strong className="text-slate-200">{candidate?.full_name}</strong> (#{candidate?.display_number})
                </p>
                {allCvUrls.length > 0 && (
                  <p className="text-[11px] text-slate-500">
                    Existing versions: <span className="text-emerald-400 font-semibold">{allCvUrls.length} file(s)</span>
                  </p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-300">
                  New CV URL (Google Drive / Shareable Link) <span className="text-rose-400">*</span>
                </label>
                <input
                  type="url"
                  required
                  value={appendCvUrl}
                  onChange={e => setAppendCvUrl(e.target.value)}
                  placeholder="https://drive.google.com/file/d/..."
                  className="w-full px-3 py-2 text-xs font-mono bg-slate-950 border border-slate-700 rounded-xl text-slate-200 focus:outline-none focus:border-emerald-500"
                  autoFocus
                />
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-300">
                  Original File Name <span className="text-slate-500 text-[10px] font-normal">(Optional)</span>
                </label>
                <input
                  type="text"
                  value={appendCvFilename}
                  onChange={e => setAppendCvFilename(e.target.value)}
                  placeholder="e.g. CV_JohnDoe_2026_Updated.pdf"
                  className="w-full px-3 py-2 text-xs bg-slate-950 border border-slate-700 rounded-xl text-slate-200 focus:outline-none focus:border-emerald-500"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-800">
                ...
                <button
                  type="submit"
                  disabled={isAppendingCv || !appendCvUrl.trim()}
                  className="px-4 py-2 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 rounded-xl flex items-center gap-1.5 shadow-lg shadow-emerald-950 transition-colors"
                >
                  {isAppendingCv ? (
                    <>
                      <Loader2 size={13} className="animate-spin" />
