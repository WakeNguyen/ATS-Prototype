...
export default function JoinStatusBadge({
  ...
  const [showAnswerPopover, setShowAnswerPopover] = useState(autoOpenAnswer);
  ...
  useEffect(() => {
    if (autoOpenAnswer && containerRef.current) {
  ...
  const renderAnswerPopoverContent = () => (
    <PopoverContent
      align="end"
      side="bottom"
      sideOffset={6}
      className="w-80 p-3 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl text-slate-200 text-xs space-y-2.5"
  ...
  const renderRatioBadge = () => {
    ...
    return (
      <Popover open={showAccountsPopover} onOpenChange={setShowAccountsPopover}>
        <PopoverTrigger
          type="button"
          onClick={(e) => {
            e.stopPropagation();
        ...
        <PopoverContent
          ...
          side="bottom"
          sideOffset={6}
          className="w-72 p-3 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl text-slate-200 text-xs space-y-2"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center justify-between border-b border-slate-800 pb-1.5">
  ...
  const renderLegacyStatusBadge = () => {
    switch (status) {
      ...
      case "Needs Custom Answer":
        return (
          <Popover open={showAnswerPopover} onOpenChange={setShowAnswerPopover}>
            <PopoverTrigger
              type="button"
              onClick={(e) => {
                e.stopPropagation();
