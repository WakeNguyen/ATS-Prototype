...
export default function CampaignEditModal({
  ...
  const [maxPostsPerRun, setMaxPostsPerRun] = useState(15);
      ...
      try {
        ...
        if (isEdit && campaignId) {
          ...
          if (detailRes.success && detailRes.data) {
            ...
            const breakerLog = detailRes.data.circuitBreakerLog || [];
            ...
            setCircuitBreakerLog(breakerLog);
            ...
            const runningRun = Array.isArray(runs) && runs.some((r) => r.status === "Running");
            setStatus(currentStatus);
            setInitialStatus(currentStatus);
            setIsRunInProgress(runningRun);
            ...
            setMaxPostsPerRun(c.max_posts_per_run || 15);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        max_posts_per_run: Math.min(15, Math.max(1, parseInt(maxPostsPerRun, 10) || 15)),
      ...
      if (isEdit && isRunInProgress && initialStatus === "Active" && status !== "Active") {
        warningInfo = {
          warning: "Note: the run currently in progress will continue to completion — pausing only prevents future runs."
        };
      }

      if (onSuccess) onSuccess(warningInfo);
      onClose();
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4 text-xs">
          {loading ? (
            ...
            <>
                  ...
                  <div className="p-3 rounded-lg bg-slate-950 border border-slate-800/80 flex items-start gap-2.5">
                    ...
                    <label htmlFor="autoRunEnabled" className="text-[11px] text-slate-300 cursor-pointer leading-tight select-none">
                      ...
                      <span className="text-[10px] text-slate-400 block mt-0.5">
                        Tự động điều phối các đợt đăng bài ngẫu nhiên cho đến khi đăng hết toàn bộ nhóm mục tiêu hoặc hết thời gian hiệu lực (end date).
                      </span>
                    </label>
                  </div>
              ...
              {/* Max Posts / Joins Per Run (Applies to both Job Posting and Warming) */}
              <div className="space-y-1">
                <label className="text-[11px] font-semibold text-slate-300">
                  {campaignType === "Warming" ? "Max Joins Per Run" : "Max Posts Per Run"} (tối đa {selectedGroupIds.size ? `${selectedGroupIds.size} ` : ''}nhóm/lượt)
                </label>
                <input
                  type="number"
                  min="1"
                  ...
                  onChange={(e) => setMaxPostsPerRun(Math.min(15, Math.max(1, parseInt(e.target.value, 10) || 15)))}
                ...
                <span className="text-[10px] text-slate-500 block leading-tight">
                  {campaignType === "Warming"
                    ? "Số nhóm tối đa mỗi tài khoản tự động join trong 1 lượt chạy (trần cứng 15, mặc định 2 nếu không đổi)."
                    : "Số nhóm tối đa đăng trong 1 lượt chạy (trần cứng 15, an toàn trước giới hạn timeout hạ tầng 55 phút/lượt). Campaign sẽ tự chạy nhiều lượt cho tới khi phủ hết toàn bộ nhóm đã gán."}
                </span>
              </div>
