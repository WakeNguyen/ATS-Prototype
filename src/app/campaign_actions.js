    ...
    const [newCampaign] = await sql.begin(async (sqlTx) => {
      const [inserted] = await sqlTx`
        INSERT INTO campaigns (
          ...
          auto_spin_content, allow_post_without_join, auto_run_enabled, target_quota, job_id, created_time, last_updated
        ) VALUES (
          ${campaign_name.trim()}, ${normalizedType}, ${channel}, ${post_language}, ${post_image_url || null}, ${content || ''},
          ${target_criteria || null}, ${start_date || null}, ${end_date || null}, ${start_time || '00:00'}, ${end_time || '23:59'}, ${is_active}, 'Ready',
          ${Boolean(auto_spin_content)}, ${Boolean(allow_post_without_join)}, ${Boolean(auto_run_enabled)}, ${target_quota ? Number(target_quota) : null}, ${primaryJobId}, now(), now()
        )
        RETURNING id
      `;
...
// ==========================================
// 2. SMART DISPATCHER & PREVIEW ENGINE
// ==========================================
...
const SAFE_DISPATCH_BATCH_SIZE = parseInt(process.env.SAFE_DISPATCH_BATCH_SIZE || '18', 10);
...
export async function computeCampaignDispatchPreview(campaignId) {
  try {
    const [campaign] = await sql`
      SELECT id, campaign_name, is_active, status, target_quota, allow_post_without_join
      FROM campaigns 
      WHERE id = ${campaignId}
    `;
    ...
    // Calculate processed_count towards target_quota (Sent + Failed from valid non-systemic runs)
    ...
    // Slicing: quota remaining capped by SAFE_DISPATCH_BATCH_SIZE
    let finalDispatch = dispatchedJobs;
    const targetQuota = campaign.target_quota != null ? Number(campaign.target_quota) : null;
    const totalSentOrProcessed = Number(processed_count || 0);
    const quotaRemaining = targetQuota !== null
      ? Math.max(0, targetQuota - totalSentOrProcessed)
      : finalDispatch.length;
    ...
    const previewResult = {
      ...
      quotaRemaining,
      safeDispatchBatchSize: SAFE_DISPATCH_BATCH_SIZE,
...
export async function _executeCampaignRunInternal({
  ...
  if (process.env.N8N_CAMPAIGN_TRIGGER_WEBHOOK_URL && runResult?.runId && finalValidatedDispatch) {
    ...
    fetch(process.env.N8N_CAMPAIGN_TRIGGER_WEBHOOK_URL, {
      method: 'POST',
      headers: {
...
/**
 * Check Campaign Auto-Completion against Target Quota:
 * Evaluates whether total processed groups (Sent + Failed) has reached the target quota.
 ...
 * @returns {Promise<{success: boolean, completed?: boolean, totalProcessed?: number, targetQuota?: number, error?: string}>}
 */
export async function checkCampaignAutoCompletion(campaignId, sqlClient = sql) {
  try {
    const [campaign] = await sqlClient`
      SELECT id, campaign_name, target_quota, is_active, auto_run_enabled, status
      FROM campaigns
      WHERE id = ${campaignId}
    `;
    ...
    if (targetQuota !== null && totalProcessed >= targetQuota) {
      await sqlClient`
        UPDATE campaigns SET
          is_active = false,
          auto_run_enabled = false,
      ...
      await sqlClient`
        INSERT INTO notifications (
          ...
          'campaign_completed',
          'Chiến dịch đã hoàn tất chỉ tiêu',
          ${`Chiến dịch "${campaign.campaign_name}" đã hoàn tất Target Quota (${totalProcessed}/${targetQuota} nhóm) và tự động kết thúc.`},
          'success',
          ${'/campaigns?campaign_id=' + campaignId},
          ${sqlClient.json({
            campaignId,
      ...
      revalidatePath('/campaigns');
      return { success: true, completed: true, totalProcessed, targetQuota };
    }

    return { success: true, completed: false, totalProcessed, targetQuota };
  } catch (error) {
  ...
  await sql.begin(async (sqlTx) => {
    ...
    if (campaignId) {
      ...
      const [activeCamp] = await sqlTx`
        SELECT id, campaign_name, campaign_type, is_active
        FROM campaigns
        WHERE campaign_type = 'Warming' AND is_active = true
...
export async function triggerWarmJoinRun(campaignId, params = {}) {
  ...
  const { accountIds = [], maxGroupsPerAccount = 2 } = params || {};
  ...
  try {
    ...
    const webhookUrl = process.env.N8N_WARM_JOIN_TRIGGER_WEBHOOK_URL || 'https://your-n8n-instance.com/webhook/warm-join-trigger';
    const payload = {
      runId: lockRes.runId,
      accountIds: lockRes.accountIds,
      campaignId: lockRes.campaignId,
      targetGroups: lockRes.targetGroups,
      maxGroupsPerAccount: Number(maxGroupsPerAccount) || 2
    };
