...
export default function ActionMenuPage() {
  ...
  const [planningDateSort, setPlanningDateSort] = useState(null); // null | 'desc' | 'asc'
    ...
    if (res.success) {
      ...
      if (field === "status" && statusFilter !== "ALL" && value !== statusFilter) {
        setApplications(prev => {
          const currentIndex = prev.findIndex(a => a.application_id === applicationId);
          const remaining = prev.filter(a => a.application_id !== applicationId);
          if (selectedAppId === applicationId) {
            if (remaining.length > 0) {
              ...
              const nextApp = remaining[nextIndex];
              setTimeout(() => {
                selectRow(nextApp);
              }, 0);
            } else {
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col h-full max-h-full bg-slate-950 text-slate-100 font-sans select-none overflow-hidden">
      ...
      {/* ======================================================== */}
      <div className="flex-1 min-h-0 flex flex-col bg-slate-950 border-b-2 border-slate-800 overflow-hidden">
        ...
        >
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-1 border-r border-slate-800 text-xs" onClick={(e) => e.stopPropagation()}>
                        <input
                          type="text"
                          list="source-channels-list"
                          value={app.candidate_source || app.source_channel || ""}
                          onChange={(e) => handleInlineUpdate(app.application_id, "source_channel", e.target.value)}
                          placeholder="Select or type source..."
                          className="w-full px-1.5 py-0.5 text-xs bg-slate-900 border border-slate-700 rounded text-slate-200 placeholder-slate-600 focus:outline-none focus:border-emerald-500"
                        />
                      </td>
          ...
          {/* Datalist for Standard Sourcing Channels */}
          <datalist id="source-channels-list">
