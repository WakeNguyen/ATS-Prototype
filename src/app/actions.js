...
import { stripAccents, normalizeContactValue } from '../lib/utils.js';
...
// 1.6 Thêm Action Note / Activity Log mới (Hỗ trợ cả Object lẫn Positional Arguments)
...
export async function syncCandidateAggregatedContacts(candidateId, dbClient = sql) {
  if (!candidateId) return;
  try {
    const contacts = await dbClient`SELECT type, value FROM contact_points WHERE candidate_id = ${candidateId}`;
    const phones = [];
    const emails = [];
    const socials = [];
    const texts = [];
    ...
    for (const cp of contacts) {
      ...
      if (t.includes('phone') || t.includes('zalo') || t.includes('whatsapp') || t.includes('mobile') || t.includes('tel')) {
        phones.push(v);
      } else if (t.includes('mail') || t.includes('email')) {
        emails.push(v);
      } else {
  ...
  try {
    ...
    await sql`
      ...
      SET 
        ...
        blocked = ${isBlocked},
        blacklist_note = ${isBlocked ? (blacklist_note || '') : ''},
        notes = ${notes || ''},
        last_updated = NOW()
      WHERE id = ${candidateId}
    `;
...
// 2.2.4 Xoá 1 version CV cụ thể khỏi mảng cv_urls (theo index) — không cho xoá version cuối cùng còn lại
export async function deleteCvVersion(candidateId, index) {
  await assertRealRequestContext('deleteCvVersion');
  try {
    const [cand] = await sql`SELECT cv_url, cv_urls FROM candidates WHERE id = ${candidateId}`;
    if (!cand) return { success: false, error: 'Candidate not found' };

    const cvUrls = Array.isArray(cand.cv_urls) ? [...cand.cv_urls] : [];
    if (index < 0 || index >= cvUrls.length) {
      return { success: false, error: 'Invalid CV version index' };
    }
    if (cvUrls.length <= 1) {
      return { success: false, error: 'Cannot delete the only remaining CV version' };
    }

    const removedUrl = cvUrls[index].url;
    cvUrls.splice(index, 1);
    const newCvUrl = removedUrl === cand.cv_url ? (cvUrls[cvUrls.length - 1]?.url || '') : cand.cv_url;

    await sql`
      UPDATE candidates SET cv_url = ${newCvUrl}, cv_urls = ${sql.json(cvUrls)}, last_updated = NOW()
      WHERE id = ${candidateId}
    `;
    revalidatePath(`/candidates/${candidateId}`);
    revalidatePath('/candidates');
    return { success: true, cv_url: newCvUrl, cv_urls: cvUrls };
  } catch (error) {
    console.error('[deleteCvVersion] Error:', error);
    return { success: false, error: error.message };
  }
}
...
export async function addContactPoint(candidateId, type, value) {
  ...
  try {
    ...
    await sql.begin(async (tx) => {
      ...
      await syncCandidateAggregatedContacts(candidateId, tx);
    });
...
export async function getClientWorkbenchData({ clientId = null, clientIndex = 0, searchTerm = "", jobId = null, clientName = null } = {}) {
  try {
    ...
    const jobs = await sql`
      ...
      GROUP BY j.id, j.display_number, j.job_title, j.location, j.branch_id, j.status, j.working_mode, j.notes, j.jd_url, j.jd_text, j.created_time
...
export async function updateJobField(jobId, field, value) {
  if (!jobId) return { success: false, error: "Missing Job ID" };
  try {
    const allowed = ['job_title', 'title', 'location', 'status', 'working_mode', 'notes', 'jd_url', 'jd_text', 'branch_id'];
    if (!allowed.includes(field)) {
      return { success: false, error: `Trường ${field} không hợp lệ` };
    }
...
export async function getJobs() {
  try {
    const data = await sql`
      ...
      GROUP BY j.id, j.client_id, j.job_title, j.status, j.location, j.branch_id, j.display_number, cl.name, j.created_time
      ORDER BY j.created_time DESC NULLS LAST, j.display_number DESC NULLS LAST
    `;
...
export async function createCandidateWithApplication(formData) {
  try {
    ...
    const [app] = await sql`
      INSERT INTO activity (id, candidate_id, job_id, current_stage, status, planning_date, created_time)
      VALUES (gen_random_uuid(), ${candidate.id}, ${job_id}, 'New', 'In progress', CURRENT_DATE, NOW())
      RETURNING id
    `;
...
/**
 * Dispatch candidates data to n8n webhook for Google Contacts sync.
 * @param {string[]} candidateIds - Array of candidate UUIDs.
 * @returns {Promise<{success: boolean, count?: number, error?: string}>}
 */
export async function syncCandidatesToGoogleContacts(candidateIds) {
  await assertRealRequestContext('syncCandidatesToGoogleContacts');
  try {
    if (!Array.isArray(candidateIds) || candidateIds.length === 0) {
      return { success: false, error: 'candidateIds must be a non-empty array' };
    }

    const [candidateRows, contactRows] = await Promise.all([
      sql`
        SELECT id, display_number, full_name, prefix,
               TO_CHAR(dob, 'YYYY-MM-DD') AS dob, notes
        FROM candidates WHERE id = ANY(${candidateIds})
      `,
      sql`
        SELECT candidate_id, type, value
        FROM contact_points WHERE candidate_id = ANY(${candidateIds})
      `
    ]);

    const contactsByCandidate = {};
    for (const row of contactRows) {
      (contactsByCandidate[row.candidate_id] ||= []).push({ type: row.type, value: row.value });
    }

    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://crm-ats-web-hazel.vercel.app';
    const payloadCandidates = candidateRows.map(c => ({
      candidateId: c.id,
      fullName: c.full_name,
      prefix: c.prefix,
      dob: c.dob,
      notes: c.notes,
      candidateDisplayNumber: c.display_number,
      atsUrl: `${baseUrl}/candidates?id=${c.id}`,
      contacts: contactsByCandidate[c.id] || []
    }));

    const candidateNames = payloadCandidates.map(c => c.fullName || 'Unnamed');
    const namesSummary = candidateNames.length <= 3
      ? candidateNames.join(', ')
      : `${candidateNames.slice(0, 3).join(', ')} và ${candidateNames.length - 3} candidate khác`;

    await sql`
      INSERT INTO notifications (type, title, message, severity, link, metadata)
      VALUES (
        'google_contacts_sync',
        ${payloadCandidates.length === 1 ? `Đang đồng bộ Google Contacts: ${candidateNames[0]}` : `Đang đồng bộ ${payloadCandidates.length} candidate tới Google Contacts`},
        ${payloadCandidates.length === 1
          ? `Yêu cầu đồng bộ candidate ${candidateNames[0]} tới Google Contacts đã được gửi, đang chờ xử lý.`
          : `Yêu cầu đồng bộ ${payloadCandidates.length} candidate (${namesSummary}) tới Google Contacts đã được gửi, đang chờ xử lý.`},
        'info',
        ${payloadCandidates.length === 1 ? payloadCandidates[0].atsUrl : null},
        ${JSON.stringify({ candidateIds: payloadCandidates.map(c => c.candidateId), action: 'dispatched' })}
      )
    `;
    ...
    console.error('Error in syncCandidatesToGoogleContacts:', error);
    return { success: false, error: error.message };
  }
}
