...
import { normalizeContactValue } from '../lib/utils.js';
import { syncCandidateAggregatedContacts } from './actions.js';
      ...
      if (action === 'REJECT' || action === 'DISMISS') {
        ...
        if (newContactPoints.length > 0) {
          ...
          const existingSet = new Set(
            existingContacts.map(c => `${(c.type || '').toLowerCase()}::${normalizeContactValue(c.type, c.value)}`)
          );

          const dedupedContactPoints = newContactPoints.filter(cp => {
            if (!cp || !cp.value) return false;
            const normVal = normalizeContactValue(cp.type, cp.value);
            if (!normVal) return true; // Không chuẩn hoá được (vd số điện thoại không hợp lệ) — giữ nguyên hành vi cũ, không tự ý loại bỏ
            return !existingSet.has(`${(cp.type || '').toLowerCase()}::${normVal}`);
          });
            ...
            await syncCandidateAggregatedContacts(targetCandidateId, sqlTx);
          }
        }
